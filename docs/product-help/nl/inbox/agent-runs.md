...
4. Klik op een rij om het run-gesprek erachter te openen. Voor items die op een ja of nee wachten, open de paarse sub-weergave **Beslissingen** onder Alle communicatie — dezelfde lijst als Overview **Wacht op beslissing**. Gearchiveerde agents staan niet meer onder [Agents](/docs/ai/agents); wijs Agenda-wakes die nog naar hen wijzen opnieuw toe.

## Bekijk het werklog van een agent

1. Vouw in de sectie **Agents** van de zijbalk de map van een agent uit.
2. Open de sub-weergave **Activiteit**. Die opent dezelfde tijdlijn, gefilterd op die agent — je chats met de agent blijven in zijn map, het werklog blijft in Activiteit, nooit gemengd in een lijst.
