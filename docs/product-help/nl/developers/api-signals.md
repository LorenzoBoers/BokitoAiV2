...
Vereist `signals:write`. `subject` en `body` zijn verplicht; `priority` is een van `low`, `normal`, `high`, `urgent` (standaard `normal`); maximaal 10 tags.

Het signal landt in de inbox op het `api`-kanaal, agents en routeringsregels behandelen het als elk ander inkomend bericht, en er vuurt een `signal.created`-webhook. Dit is de standaardmanier om alerts, formulierinzendingen of events uit andere systemen in dezelfde flow als klantmail te krijgen.
