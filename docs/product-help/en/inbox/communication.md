...
1. Open **Communication**. Drafts and approvals waiting on you live under **Agents** → [Agent runs](/docs/inbox/agent-runs) (**Decisions**) and on [Cockpit](/docs/getting-started/cockpit) **Awaiting decision**. At the top, **All communication** is a folder like the rest of the sidebar: click it to expand **Open**, **Mine**, **Unassigned** and **Closed** (plus **Snoozed** and **Spam**). The first expand opens the default sub-view from **Settings** → **Email & messages** (Folders and tags) — usually **Open**, or **Mine** if you set that. **Outbound** is mail you started.
...
3. Switch to **Note** for an internal comment the customer never sees (hover the tab for the reminder). Notes sit as quiet left-rule text in the timeline — distinct from customer messages, without a yellow bubble. Notes still work if no mailbox can send. Closed or spam threads keep notes too — a **Reopen** button sits on the composer so you do not hunt for it.
...
2. Choose **Send as:** **You** or the agent — the signature that will be appended appears under that choice before you send. Edit the wording, then send — or choose **Not now** / **I'll handle it myself**. Sending or approving one draft dismisses leftover suggestion cards. Older dismissed drafts collapse to one line (**Earlier draft — dismissed**).
