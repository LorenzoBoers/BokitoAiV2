...
export default function MemberManagement() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const [rawMembers, rawInvites] = await Promise.all([
          appScopedGet<unknown[]>(appRoutes.workspaces.members(workspaceId), token).catch(() => []),
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      const inviteList = await appScopedGet<unknown[]>(appRoutes.workspaces.invites(workspaceId), token).catch(() => [])
