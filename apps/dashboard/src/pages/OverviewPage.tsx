  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      listGovernChanges('pending_review')
        ...
        .catch(() => 0),
      listThreads(token, { view: 'awaiting_decision', perPage: 6 })
        .then((r) => r.items)
        .catch(() => [] as InboxThread[]),
      bokitoGetCockpitActivity(token, 30).catch(() => [] as CockpitActivityEvent[]),
              ...
              <>
                {attentionThreads.map((thread) => (
                  <Link
                    ...
                    to={`/communication/${thread.folder === 'internal' ? 'agents' : 'customers'}/awaiting-decision/t/${thread.id}`}
