  ...
  if (!projectId) {
    return (
      <PageContent width="sm" className="py-1">
        <p className="text-sm text-text-muted">{t('project.connect.notFound')}</p>
      </PageContent>
    )
  }
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-lg border border-border/60 bg-bg-surface p-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-medium text-text-primary">{t('project.connect.githubTitle')}</h2>
          </div>
          <p className="mt-2 text-sm text-text-muted">{t('project.connect.githubDescription')}</p>
          <div className="mt-4">
            <ConnectRepoPanel
              projectId={projectId}
        ...
        <div className="rounded-lg border border-dashed border-border/60 bg-bg-elevated/40 p-5 opacity-80">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-medium text-text-primary">{t('project.connect.managedTitle')}</h2>
            <Badge variant="neutral">{t('project.connect.comingSoon')}</Badge>
          </div>
          <p className="mt-2 text-sm text-text-muted">{t('project.connect.managedDescription')}</p>
        </div>
      </div>
