...
- **Dashboard bokito mode:** `VITE_API_MODE=bokito` and `VITE_BOKITO_API_URL=http://127.0.0.1:8000` in `apps/dashboard/.env.local`. Vite proxies all `/api/*` to FastAPI. Admin rail shows Cockpit (`/home`), Orchestra (`/orchestra`), Agenda (`/agenda`), Projects, Inbox, Workforce, Database (`/database`), Integrations, Settings. Staff users (`is_staff` on `/auth/me`) see a tenant switcher in the header (`GET /auth/tenants`, `POST /auth/switch-tenant`); seed staff login `staff@bokito.ai` / `bokito-test-password`. Multichannel inbox uses FastAPI `/api/integrations/inbox/*` (same contract as legacy Xano). Workspace Blueprint editor uses FastAPI `/api/workforce/workspace/doc/*` backed by `blueprint_*` tables (page tree, block batch ops, revisions, change-requests). Integrations hub (marketplace, connected, MCP, GitHub, e-mail OAuth) uses FastAPI `/api/integrations/integrations/*`, `/api/integrations/github/*`, `/api/integrations/email/*` with a static provider catalog (`integrations_catalog.py`) and mock OAuth redirects that append success query params to `return_url`. Project hub uses FastAPI `/api/workforce/projects/*` (CRUD, repo connect/reindex, orchestration, notification prefs, usage budget/summary, workstreams, PO agent). Workforce runtime uses FastAPI `/api/workforce/agents`, `/api/workforce/work_logs`, `/api/workforce/messages`, and `/api/workforce/workforce/*` (config, status, trigger-agent, timeline) backed by `agents`, `agent_runs`, `run_events`, and `workforce_messages` tables. Custom database (Database page) uses FastAPI `/api/app/custom-tables/*`, `/api/app/standard-tables/*`, records, views, CSV import/export, activity, and comments via `custom_*` SQL tables. Settings control plane (P7): FastAPI `/api/app/workspaces`, `/api/app/workspaces/{id}/members`, `/api/app/workspaces/{id}/invites`, `POST /api/app/workspace-invites`; auth group `PATCH /api/auth/profile`, `POST /api/auth/change-password`, `POST /api/auth/workspaces/{id}/branding` (multipart), `POST /api/auth/users/me/avatar`, session stubs (`/api/auth/auth/password-reset-request`, etc.). Tenant maps 1:1 to workspace (`id` = tenant UUID string); branding/logo/favicon stored in `tenants.settings_json` and `logo_url` (data URLs in dev).
- **Local dev:** `docker compose -f docker-compose.dev.yml up`; seed `admin@bokito.ai` / `bokito-test-password`.
- **Tests:** pytest (27 tests, mock LLM). CI: `api` (ruff + pytest), `dashboard` + `messenger` builds (`VITE_API_MODE=bokito`), `e2e` (Playwright: dashboard login/cockpit/inbox/integrations/database/orchestra/agenda + messenger login). Local: `npm run verify:bokito` and `npm run verify:bokito:e2e` (see `docs/AI-OS-DEV.md`).
...
- **Primary rail (admin, icon-only):** Home (`/home`) → Project hub (`/projects`) → Inbox → **Workforce** (`/workforce/overview`, Bot-icoon; merged Assistent + Agent library) → Integraties → Data; **Instellingen** onderaan (`/settings/profile`). Geen aparte Assistent- (Sparkles) of Agenten-rail meer.
- **Workforce-zijbalk (hybrid IA):** drie lagen: (1) Workforce-links (`/workforce/overview`, `/workforce/agents`), (2) dynamische **agent-secties** — **Orkestrators** en **Worker agents** (uit `GET /agents`, links naar `/ai/agents/:id`; orkestrator-rijen tonen optioneel gekoppeld project), (3) **Platform agents** (Assistant agent → `/ai/assistent/...`, Communication agent → `/ai/communicatie`). Geen projectenlijst in Workforce; orkestrator-setup blijft in Project hub sidebar. Orkestrator-dekking op `/workforce/overview` blijft als cockpit-informatie.
