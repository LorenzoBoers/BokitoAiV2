...
- **Execution ladder (trading stack):** `virtual` < `shadow` < `live`. Prod autotrading draait **live** (`EXECUTION_MODE=live`, `DEGIRO_ALLOW_LIVE_ORDERS=1`, `config.yaml execution.mode: live`). MCP `risk_status` moet `execution_mode: live` en `degiro_allow_live_orders: true` tonen. Enable script: `scripts/vps-enable-trading-live.py`; rollback = zet env terug naar shadow + recreate worker/MCP. **Netwerk:** na recreate altijd `scripts/vps-ensure-trading-network.py` (aliases `bokito-api`, `trading-exec-mcp` op `bokito_shared`).
