...
- **Marketplace UX**: kaartraster met type-badge **Communication**, **Repository** of **MCP**. Klik op een kaart opent een **detailmodal** (beschrijving, capabilities, verbindingstelling). **Nieuwe verbinding instellen** gaat naar een **setup-stap** in dezelfde modal: OAuth (GitHub, Outlook, Gmail) met return naar `/integrations/marketplace?connect=<id>`, of API-key/MCP-formulier (Bjorn Lunden, Custom MCP). Verbonden: **Bekijk bij Verbonden**. Setup-configuratie via `integration-setup.ts` (`oauth2` | `api_key` | `custom_mcp`), gestuurd door **`apps/dashboard/src/lib/integrations/registry.ts`** (per provider: setup mode, OAuth-strategie `github` | `inbox` | `platform`, connection-count bron). OAuth-start in de UI via `integration-oauth-flow.ts` (`startProviderOAuth`).
...
- **Catalogus**: `GET /integrations/providers` uit `integration_providers` (seed: github, outlook, gmail, bjorn_lunden_mcp, custom_mcp in `xano-patches/v1/integration-providers-seed.md`). De marketplace toont alleen deze live providers; geen placeholder-catalogus meer. Fallback-metadata staat in `integrations-data.ts`.
- **Host-logo's**: tabel `integration_hosts` bevat merklogo's (Xano image-velden `logo` / `logo_dark`). Providers verwijzen via `host_id`. De providers-API levert per provider een genest `host`-object met `logo_url` / `logo_dark_url`. De portal gebruikt `IntegrationHostLogo`: Xano-URL indien geupload, anders alleen **officiële** statische SVG's voor GitHub / Microsoft (Outlook) / Google (Gmail) in `public/brands/`, anders gekleurde initialen-placeholder (o.a. Bjorn Lunden MCP, Custom MCP, SMTP). Zie `xano-patches/v1/integration-hosts-seed.md`.
...
- **GitHub (legacy + nieuw)**: `GET /github/connections` (lijst), `GET /github/repos?connection_id=`, repo koppelen via `PATCH /projects/{id}/repo` met `connection_id` + `integration_bindings` type `project_repo`. Worker: `POST /integrations/worker/credentials` (fallback `POST /github/worker/token`).
...
- **Admin rail**: Home, Inbox, **Workspace** (`/projects`), Assistent, Integraties, Data (geen aparte Agenten-rail). Workforce-agents en per-agent run-geschiedenis staan onder Assistent → **Agents** (`/ai/agents`).
...
- **Primary rail (admin, icon-only):** Home (`/home`) → Inbox → Workspace (`/projects`) → Assistent (`/ai/assistent`) → Integraties → Data; **Instellingen** onderaan (`/settings/profile`). Geen aparte Agenten-rail.
...
- **Workspace (`/projects`):** context sidebar: **Overzicht**, **Communicatie** en **Documentatie** (enkele link naar `/projects/docs`); **Achtergrondworkers** (`ProjectHubBackgroundWorkersNav`) met compacte rijen: gekleurde dot + teller, status als gekleurde tekst onder de titel. Paginaboom staat in de content op `/projects/docs` (`PageTree` variant `minimal`), niet in de sidebar. Per-project canvas: `WorkerStatusStrip` boven `ProjectTabNav` (operationele status los van Communicatie-tab); status afgeleid via `deriveWorkerStatus` uit berichten (`decision_request` = geblokkeerd, overige `awaiting_human` = aandacht), budget, orchestration, work logs en repo-index.
- **Centrale documentatie:** `/projects/docs[/:slug]` is tenant-breed (workspace-only). `WorkspaceDocNavContext` laadt `GET /workspace/doc`; bij een lege boom seed de client acht standaard hoofdstukken (`workspace-doc-scaffold.ts`). Bestaande pagina's zonder blocks krijgen bij openen starter-blocks als de slug in de scaffold staat (bijv. `overview`). Als die seed-call tijdelijk faalt, toont `ProjectHubDocs` direct lokale scaffold-blocks zodat de pagina leesbaar blijft (geen foutbanner). Geen fallback naar per-project docs.
- **Workspace doc Xano-schema:** live tabellen `workspace_doc_pages` en `workspace_doc_blocks` mogen geen PostgreSQL-default `""` op enum/json/uuid-kolommen hebben (inserts geven anders `SQL 22P02`). Blocks sorteren op `position` alleen (`workspace_doc_blocks` heeft geen `created_at`). API-response gebruikt `workspace_doc`, niet `doc` (Xano coerceert `doc` naar integer). Op `workspace_doc_blocks` nooit `return = {type: "list"}` in `db.query` — kolom `type` botst met de return-key (`HTTP 400 Text filter requires ...`). `BlockEditor`, `RevisionPanel` en sidebar `PageTree` gebruiken `docScope: 'workspace'` met CRUD via `/workspace/doc/pages` en block batch via `/workspace/doc/pages/{id}/blocks`. Hoofdstukken zijn top-level `workspace_doc_pages` met kinderen; secties zijn blocks (`heading_*`). Hub-knop **Vraag agent** plaatst `POST /workspace/doc/change-requests` voor de actieve pagina.
- **Workspace doc hardening (blocks canonical + projections):** blocks/pages blijven source of truth. `workspace_doc_pages` heeft `content_version` (optimistic concurrency), `rendered_markdown`, `rendered_plaintext`, `content_hash`, `last_indexed_at`. User/agent batch endpoints weigeren writes op `is_locked` pages. Batch POST accepteert `expected_version`; conflict geeft foutmelding en frontend herlaadt. Runtime coalesced reindex via `POST {WORKER_BASE_URL}/workspace/doc/reindex-page` (15s debounce); indexeert primair `rendered_plaintext` chunks (`source_type=workspace_doc_page`) en slaat projections terug via worker `POST /workspace/doc/worker/pages/{page_id}/projections`. Agent batch ondersteunt optionele `idempotency_key` (`workspace_doc_write_idempotency`). Workspace editor gebruikt flat top-level blocks only (geen nested editor).
- **Per-project config (Xano):** `project_orchestration_config` + `GET/PATCH /projects/{id}/orchestration`; `project_notification_preferences` + `GET/PATCH /projects/{id}/notifications/preferences`; `GET /projects/{id}/usage/summary` en `GET /projects/{id}/usage/budget`. Patches in `xano-patches/v1/` + `PROJECT-HUB-BACKEND.md`. Portal: `ProjectOrchestration`, `ProjectNotifications`, `ProjectUsage` laden en slaan op (geen blueprint-banners meer zodra backend deployed is).
- **Fallback als config-tabellen ontbreken:** als `project_orchestration_config` en `project_notification_preferences` nog niet bestaan in de tenant, draaien de endpoints tijdelijk stateless met veilige defaults zodat `/project/:id/orchestration`, `/project/:id/notifications` en `/project/:id/usage` wel renderen zonder 4xx/5xx.
- **Per-project cockpit (`/project/:id/*`):** focust op agent-orchestratie. Sidebar-items: **Overzicht → Orchestratie → Communicatie → Workforce-geschiedenis → Tokenverbruik → Notificaties → Wijziging aanvragen → Instellingen**. Stubs voor Orchestratie (PO wake-frequency, autonomie-modus, HITL gevoeligheid), Notificaties (per-event matrix Desktop/E-mail/Mobiel), Workforce (gefilterde agent-runs voor dit project), Usage (totaal runs / lopend / tokens). `ProjectMessages` is hernoemd naar `ProjectCommunication` op `/project/:id/communication`; oude `/messages` redirect.
- **Assistent-zijbalk:** Widget (default customization) + AI communicatie (`/ai/communicatie`) + **Agents** (`/ai/agents`; admin-only lijst, detail met gefilterde runs, live log op `/ai/agents/:agentId/runs/:workLogId`). Geen globale “alle runs”-pagina onder AI.
...
- **Agent runs (waar):** (1) per agent onder `/ai/agents/:agentId`, (2) per project op `/project/:id/workforce` met detail `/project/:id/workforce/:workLogId`, (3) recente runs op project hub `/projects` (links naar project-run-detail). Legacy `/admin/runs` → `/projects`; `/admin/runs/:id` → project-run-detail indien vindbaar; `/workforce` → `/ai/agents`.
...
- Legacy redirects: `/integrations/sources` en `/datasources` → `/data/sources`; `/settings/data/*` → data/users-routes; `/workforce` → `/ai/agents`; `/admin/runs` → `/projects`; `/project/:id/doc[/:slug]` en `/project/:id/pkb` → `/projects/docs`; `/project/:id/messages` → `/project/:id/communication`.
- **Navigatie-badges:** `NavBadgeProvider` in de portal-shell pollt inbox-threads (ongelezen bij mij + niet toegewezen) en workforce-berichten (`awaiting_human` voor admins). Tellers op rail (Inbox, **Workspace**), Assistent-sidebar **Agents** (`agentsAttention`), inbox-submenu (Open, Mijn, Niet toegewezen) en Workspace Communicatie-tab; refresh na mark-read/unread in `Communication`.
