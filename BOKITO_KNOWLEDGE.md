...
- **Admin rail**: Home, **Projecten** (`/projects`), Inbox, Assistent, Integraties, Data (geen aparte Agenten-rail). Workforce-agents en per-agent run-geschiedenis staan onder Assistent → **Agents** (`/ai/agents`).
...
- **Primary rail (admin, icon-only):** Home (`/home`) → Project hub (`/projects`) → Inbox → Assistent (`/ai/assistent`) → Integraties → Data; **Instellingen** onderaan (`/settings/profile`). Geen aparte Agenten-rail.
