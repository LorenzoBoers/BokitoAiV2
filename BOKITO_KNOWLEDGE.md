...
- **Reply identity + clean drafts (aug 2026):** voorgestelde antwoorden zijn puur klantgericht. `services/suggestion_format.split_suggestion()` splitst rauwe LLM-output in draft-body en interne notitie: `INTERNAL_NOTE:`-sentinel (afgedwongen in de suggest-prompt), legacy `**Interne notitie:**`-blokken, research-preambles vóór een `---`+groet, en model-geschreven afsluitingen ("Met vriendelijke groet, …") worden gestript — het handtekeningsysteem voegt server-side precies één handtekening toe. De interne notitie wordt bewaard als `internal_note`-SignalMessage op de thread én als `internal_note` in de send-option payload (decision card toont hem ingeklapt, geel, "geen onderdeel van de e-mail"). **Send-as:** goedkeuren van een suggestie verstuurt standaard als de goedkeurende user (timeline-attributie `role="user"` + `author_user_id`, `metadata.send_as`), wisselbaar naar de agent via een segmented selector op de decision card ("Versturen als: Jij / <agent>"); keuze wordt onthouden in localStorage (`bokito.replySendAs`), tenant-default via `reply_send_as` in `PUT /api/settings/ai-modes` (instelbaar op `/settings/communication`). `ResolveBody.send_as` → `resolve_message_decision` → `send_reply`-tool. Auto-mode blijft agent-identiteit. **Handtekeningen:** per user (`User.settings_json.email_signature_html`, editor in Profiel), per agent (`Agent.settings_json.email_signature_html`, card op agent-detail), mailbox `signature_html` als terugval; resolutie in `services/signatures.resolve_signature_html` (user → agent → mailbox), doorgegeven als `signature_html`-override aan `deliver_outbound`/`format_outbound`. Handmatige replies en geplande outbound-messages gebruiken dezelfde resolutie op basis van auteur.
