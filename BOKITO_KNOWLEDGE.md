...
- **Catalogus**: `GET /integrations/providers` uit `integration_providers` (seed: github, outlook, gmail, bjorn_lunden_mcp, custom_mcp, plus remote MCP OAuth slugs in `xano-patches/v1/integration-providers-seed.md`). Marketplace groepeert providers per host; fallback-metadata in `integrations-data.ts` en `mcp-remote-providers.ts`.
...
- **Primary rail (admin, icon-only):** Home (`/home`) → Project hub (`/projects`) → Inbox → **Workforce** (`/workforce/agents`, Bot-icoon; merged Assistent + Agenten) → Integraties → Data; **Instellingen** onderaan (`/settings/profile`). Geen aparte Assistent- (Sparkles) of Agenten-rail meer.
- **Workforce-zijbalk:** Platform agents (Assistant agent → widget `/ai/assistent/...`, Communication agent → `/ai/communicatie`, PO agent uit `GET /agents`) + dynamische **Your agents**-lijst (custom agents via `WorkforceSidebarNav`). Workforce bevat geen workstream-pagina meer; deze agents zijn inzetbaar in project-workstreamstappen.
...
- **Per-project cockpit (`/project/:id/*`):** focust op projectuitvoering. Sidebar-items: **Workstreams → Orchestratie → Communicatie → Workforce-geschiedenis → Tokenverbruik → Notificaties → Wijziging aanvragen → Instellingen**. `ProjectOverview` op `/project/:id/overview` is nu een workstreams-wireframe met stream-submenu, input/output placeholders en stapkaarten (agent, instructie, tools). `ProjectCommunication` op `/project/:id/communication` ondersteunt projectbreed + optionele `?stream=` filterweergave (mixed model).
...
- Legacy redirects: `/integrations/sources` en `/datasources` → `/data/sources`; `/settings/data/*` → data/users-routes; `/workforce` → `/workforce/agents`; `/admin/runs` → `/workforce/agents`; `/project/:id/doc[/:slug]` en `/project/:id/pkb` → `/projects/docs`; `/project/:id/messages` → `/project/:id/communication`.
...
- **Workstreams wireframe (mei 2026):** workstream-editor is verplaatst naar `/project/:id/overview` en gebruikt een stream-submenu met meerdere mock streams. Canvasflow: `Input (PO invoke)` → vaste stappen (agent, instructie, tools uit tenant-integraties of mock fallback) → `Output (PO report)`. Knoppen **Add step** en **Drag preview** zijn UI-only en disabled in deze fase; nog geen backend-opslag of workflow-executie.
...
- **Project workstreams (backend):** Workstreams zijn persisted entities per project (`project_workstreams` tabel) met API `GET/POST/PATCH /projects/:id/workstreams`. Eerste GET seed drie default streams als de lijst leeg is. Sidebar toont workstreams van het geselecteerde project (niet meer projectnamen). Response bevat ook gekoppelde `po_agent` (agent entity met `role=po`, via `projects.po_agent_id` of fallback `agents.project_id`).
- **Project PO in hub:** Gekoppelde PO agent wordt getoond in Project Hub sidebar en op `/project/:id/overview`, met PO type-label en link naar agent detail. Koppelen via `PATCH /projects/:id/po-agent` (valideert PO type).
- **Projectinstellingen (sidebar):** Onderaan de Project Hub-zijbalk staat **Projectinstellingen** (`/project/:id/settings`) zodra een project geselecteerd is. De pagina bevat algemene projectvelden, codekoppeling en een danger zone om het project te verwijderen. Verwijderen vereist dubbele bevestiging: de gebruiker moet de exacte projectnaam typen. Backend: `DELETE /projects/{id}` met body `{ confirm_name }` (verwijdert workstreams/orchestration/notificaties, ontkoppelt agents, verwijdert project).
