...
- **Setup Hub (2026-08)**: `/settings/setup` (redirect vanaf legacy `/integrations/setup`; oude `IntegrationSetupPage` verwijderd). Vijf pijlers met live voortgangsdetectie (niets opgeslagen): 1) Communication (mailbox verbonden via onboarding-status, Gmail/Outlook merk-logo's), 2) Intelligence (company.md gevuld + assistant-gesprek + agents aanwezig; violet brein-icoon), 3) Automations (triggers > 0), 4) Branding & widget (workspace-logo of brand_color gezet), 5) KPIs & metrics (custom metrics > 0). Elke pijler heeft actie-links; header-CTA "Start with the assistant" opent `/communication/new?prefill=` met een setup-prompt. `ONBOARDING_SYSTEM_PROMPT` (tenant_bootstrap) beschrijft dezelfde vijf pijlers zodat de assistant de setup begeleid uitvoert.
...
- **Composer:** **Templates** is a visible label. Note vs Reply shows **The customer sees this** / **Only your team sees this**. **Send and snooze** is the park-after-send action. New conversation has **Back to Messages** and a To-field hint.
...
- **New conversation** treats **Email a customer** and **Chat with assistant** as equal chips. Reply composer CC/BCC placeholders are i18n. Inbox shortcut footer is **Press ? for shortcuts**. Agent-runs chip row has a muted subtitle. Govern Agent access tab has a one-line hint. Agent library model chips keep the pretty label and put the raw id in the tooltip.
