...
- **Page guide banners (2026-08):** main rail pages (Cockpit, Contacts, Agenda, Agents, Projects, Knowledge) plus Govern show a dismissible intro banner (`PageGuideBanner`) above the page content. Communication has no banner yet (video walkthrough later). "Read the full explanation" opens `/learn/:slug` (`LearnPage`) with written sections and a reserved video slot. Dismiss is stored in `localStorage` as `bokito-page-guide-dismissed:{slug}`. Copy lives in `nav.json` under `pageGuides`.
...
- **Snooze:** backend blijft bestaan (`snoozed_until` op Signal, `wake_snoozed_threads` scheduler). De Communication-UI toont snooze voorlopig niet (geen header-actie, geen "Send & snooze", geen Snoozed-queue). Oude `/communication/inbox/snoozed`-URLs redirecten naar All. Closed/Spam blijven onder More.
