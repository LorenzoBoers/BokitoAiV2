...
- **Streaming:** `AgentLoop.stream_chat` roept echte `llm.stream_chat` aan (Anthropic + OpenAI). Gateway op topic `signal:{id}` publiceert `message.delta` (token-tekst), `agent.step` (tool_call / tool_result / think), en `message` (final). Dashboard (`ThreadDetail`, `DirectChatPanel`) en mobiel (`useSignalStream`, `StreamingBubble`) tonen live deltas en tool-stappen.
