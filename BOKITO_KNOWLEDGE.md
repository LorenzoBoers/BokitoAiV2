...
- **Bokito bridge (trading lab → MMXM Trader):** Webhook trigger `MMXM pipeline webhook` (`f3e95fe7-c201-4ef3-bbac-50de9309cb2d`) op tenant `autotrading`, agent MMXM Trader. Trading stack env (`/opt/trading/.env`): `BOKITO_ENABLED=1`, `BOKITO_BASE_URL=http://bokito-api:8000`, `BOKITO_TRIGGER_ID`, `BOKITO_WEBHOOK_SECRET`. Setup: `scripts/vps-setup-trading-bokito-bridge.py`; smoke: `scripts/vps-trading-webhook-smoke.py`. DNS: `bokito-api` alias op `bokito_shared` (compose alias op prod `api` service); na `docker compose` recreate trading containers opnieuw verbinden via `scripts/vps-ensure-trading-network.py`.
- **Execution ladder (trading stack):** `virtual` < `shadow` < `live`. Prod draait `EXECUTION_MODE=shadow` (`DEGIRO_ALLOW_LIVE_ORDERS=0`) — MCP `risk_status` bevestigt dry-run. Live DeGiro vereist expliciet `execution.mode: live` in `config.yaml` én `DEGIRO_ALLOW_LIVE_ORDERS=1`.
