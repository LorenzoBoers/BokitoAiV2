...
- **Marketplace UX**: kaartraster met type-badge **Communication**, **Repository** of **MCP**. Niet verbonden: **Koppelen** (e-mail/repo) of **Instellen in MCP**; verbonden: **Bekijk bij Verbonden** (en voor MCP optioneel **Beheren in MCP**). Geen GitHub-beheer meer via marketplace-zijpaneel.
...
- **E-mail**: blijft op `email_oauth_connection`; marketplace telt Outlook/Gmail via `connection_counts.email_outlook` / `email_gmail` en linkt naar `/settings/inbox`.
- **MCP**: `POST /integrations/mcp/install`, `GET /integrations/mcp/bindings` voor tenant `mcp_server_ids`. Providers: `bjorn_lunden_mcp` (platform MCP server id 8), `custom_mcp` (URL + auth in connection metadata). Portal `/integrations/mcp` tab 1: enige install/beheer-UI (tabel uit `integration_connections`); catalogus Setup opent `?connect=bjorn_lunden_mcp` of `?connect=custom_mcp`. Tab 2: Bokito MCP client-instructies (UX-preview).
...
- **Admin rail**: item **Projecten** (`/projects`) naast Workforce.
...
- **Primary rail (icon-only, links):**
  - Inbox / Support (`/support/inbox/all`)
  - AI (standaardingang `/ai/assistent`; zijbalk: **Assistents** + **AI communicatie instellingen** `/ai/communicatie` — per-mailbox AI-config via `/email/connections/{id}/ai-config`)
  - Integraties (`/integrations` → connected; subnav: verbonden, marketplace, **bronnen** `/integrations/sources`, MCP)
  - Data (`/database`)
  - Workforce (binnenkort, niet klikbaar)
  - Instellingen staat onderaan de rail als vast icoon (`/settings/profile`)
- Er is geen aparte **Help**-rail meer en geen ingebouwde Swagger-**API Documentatie**-route (`/docs`); OpenAPI-export blijft beschikbaar op API-instellingen waar van toepassing.
...
- **Bronnen** (geindexeerde docs) staan onder Integraties; `/datasources` redirect naar `/integrations/sources`.
...
- GitHub repo connect (workforce): registreer een GitHub OAuth App met callback op `GET /github/oauth/callback` (integrations group). Xano env: `GITHUB_OAUTH_CLIENT_ID`, `GITHUB_OAUTH_CLIENT_SECRET`, `GITHUB_OAUTH_CALLBACK_URL` (exact gelijk aan geregistreerde redirect URI). Scopes: `read:user`, `repo`.
