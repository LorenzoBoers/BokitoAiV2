...
- **Björn Lundén sandbox**: MCP servers whose name matches bjorn/lunden/king/account get a mock accounting toolset (`search_customers`, `get_customer`, `list_invoices`, `get_invoice`, `list_ledger_entries`, `get_account_balance`, `list_vat_reports`) on `mock://` URLs, with realistic SEK mock responses in `mcp_client`. The BL connect dialog now has an optional server URL (empty = sandbox) and an optional API key, so the client can connect before real credentials exist.
