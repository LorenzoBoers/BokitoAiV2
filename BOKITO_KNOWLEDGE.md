...
- **Settings-menu IA (aug 2026):** vijf groepen in `SettingsLayout.tsx`: **Personal** (Profile & security, My assistant, Notifications), **Workspace** (General, Branding, Members, Setup guide), **Communication** (Email & messages, Chat widget, Communication agent, Help center), **Integrations** (Integrations, Developers), **AI** (Providers & models, Autonomy & approvals). De losse menu-items Setup/Connected/Marketplace/Connected tools zijn samengevoegd tot één "Integrations"-entry met een inner-tabbar (`components/shell/IntegrationsTabs.tsx`: Connected | Marketplace | Connected tools) bovenaan de drie bestaande routes — routes/deep-links (`/settings/marketplace?connect=...`) blijven ongewijzigd. `SettingsLink.match` accepteert nu ook een array van pad-prefixen. Het duplicaat "Knowledge"-item is verwijderd (zit in hoofdnav onder AI); "Document index" heet nu "Help center"; "Assistant widget" heet "Chat widget"; Autonomy is bij de AI-groep gevoegd.
...
- **AI-taalbeleid (2026-08):** twee onafhankelijke assen in `app/services/language.py`. (1) **Antwoordtaal** (`reply_language`): taal van conceptantwoorden aan de klant; default `auto` = spiegel de taal van het klantbericht (Engelse mail → Engels concept, ook op een NL-workspace); vast te pinnen per mailbox (`ai_config.reply_language`) of tenant-breed (`settings.ai_reply_language`); precedentie mailbox → tenant → auto. (2) **Teamtaal** (`ai_workspace_language`, default `en`): taal van AI-tekst gericht aan het team, zoals de `NO_REPLY_NEEDED:`-samenvatting. Beide worden als "Language rules" in de suggest- en auto-prompts geïnjecteerd. Instelbaar op `/settings/communication` (kaart "Taal" + per-mailbox override); API: `GET/PUT /api/settings/ai-modes` (velden `reply_language`, `workspace_language`) en per-mailbox `ai_config.reply_language` (lege string wist de override). Ondersteunde talen: nl, en, de, fr, es. Statische kaart-copy (titels, knoplabels als Send/Edit/Escalate/Close thread) komt niet van de backend maar wordt client-side vertaald via i18n (`communication.json` → `decisionCard.*`, mapping op optie-id/action_type in `DecisionRequestMessage.tsx`), zodat die de platformtaal van de gebruiker volgt.
