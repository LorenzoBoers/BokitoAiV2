...
- **Platform support (2026-08):** Bokito operators are `User.is_staff`. They authenticate with the normal login form (JWT `staff: true`); staff-only accounts (no membership) land in the first workspace that allows support. Operators enter a workspace via `POST /auth/switch-tenant` and act as themselves with effective admin — they do not impersonate a customer user. Each workspace can turn this off in Settings → General (`security.allow_platform_support`, default **on** when missing). Opted-out workspaces stay visible in the staff picker as locked and cannot be entered. Every enter/leave is written to `StaffAccessLog`. There is no anonymous or secret-URL access.
- **Workspace sessions (2026-07, cycle 1):** the JWT `tenant_id` is the single source of truth for the active workspace. `POST /api/auth/switch-workspace` re-scopes the token for any member with a membership (staff may enter any tenant, logged in `StaffAccessLog`); `POST /api/app/workspaces` returns a `session.access_token` scoped to the new tenant which the dashboard adopts with a full reload; `User.last_tenant_id` is persisted on signup/login/switch/create/invite-accept so login and refresh land in the last-used workspace (staff refresh honors it even without a membership). Frontend: `AuthContext.switchWorkspaceTenant`/`adoptWorkspaceSession` store the token in sessionStorage and `window.location.assign('/')`; `WorkspaceContext.switchWorkspace` only mutates local state when the target equals the JWT tenant. Fresh-tenant bootstrap is clean (assistant + personal agent, 4 workspace docs, runtime profiles, one Platform check-in conversation, enabled hourly heartbeat — no demo project/orchestrator/workstream). Recurring triggers reuse one internal Signal thread via `Trigger.signal_id`. Heartbeat findings always land in the tenant `operations_signal_id` thread (created at bootstrap). Existing tenants get the ops thread and a check-in row on startup via `ensure_platform_watch`, but a disabled heartbeat is never flipped on. Onboarding: `GET /api/app/onboarding` computes a checklist (email, company, assistant, watching, first_decision, team) from live data; `watching` is the enabled platform check-in. The Communication empty state renders `OnboardingChecklist` until complete. Agent tools `get_platform_watch` / `set_platform_watch` (ungated) let the assistant turn watching on from chat. Integrity tooling: `apps/api/tests/test_tenant_isolation.py` + `scripts/ops/audit-tenant.py` (cross-tenant FK audit + per-tenant row counts).
...
- **Prod deploy ops (2026-08-25):** Push to `master` runs CI, then Deploy builds GHCR images and SSHs to the VPS (`appleboy/ssh-action`) for staging then production. Production still needs the GitHub Environment approval (auto-approve via `scripts/dev/watch-deploy.ps1`). Live cutover of `5793bc4` succeeded on 2026-08-25: staging smoke + prod SSH + `https://app.bokito.ai/api/health` all green; VPS `/opt/bokito` and `BOKITO_API_IMAGE`/`BOKITO_WEB_IMAGE` match that SHA. Earlier runner `dial tcp <vps>:22: i/o timeout` failures are no longer the default path. Manual fallback if Actions SSH fails: deploy key `~/.ssh/bokito_vps_deploy` as `root@31.97.45.44`, then `python scripts/vps-prod-deploy-remote.py <sha>` after GHCR images exist (script logs the VPS into GHCR with a `gh` token). Building images on the VPS still needs BuildKit (`docker buildx`; legacy builder breaks `RUN --mount`). Prod env facts: Microsoft OAuth pair set (client `f44d9848-…`, tenant `common`), `LLM_MODE=live` + Anthropic key set, `WORKER_INBOUND_SECRET` added 2026-08-13 (new boot validator requires it), SMTP/VAPID/`BJORN_LUNDEN_MCP_URL` still unset. Staging runs `ENVIRONMENT=staging` with mock LLM (prod validators don't apply). Ruff is pinned to 0.15.17 in `apps/api` dev deps so CI (pip, fresh install) matches local lint.
...
- **Timeline group-chat model (2026-08):** the thread timeline (`TimelineItem.tsx`) renders four author types with distinct bubbles — external contacts (neutral card, favicon avatar, left), teammates (elevated bubble + "Team" chip + member avatar, left), AI agents (accent-washed bubble + Sparkles avatar + "AI" chip + agent name, left) and the signed-in user (accent-tinted bubble, avatar on the right, no name header, right-aligned — WhatsApp-style; failed sends show "Not delivered"). Internal notes stay yellow and follow the same left/right ownership rule. In email threads only inbound external mail keeps the full-width card (HTML mails need the width); own replies, teammate replies and AI messages use chat bubbles there too. `DirectChatPanel` follows the same convention. **Member recognition (2026-08):** inbound mail from a workspace member login email never creates a CRM `Contact`, stamps `SignalMessage.author_user_id`, skips inbound agent processing, and the timeline resolves authorship by `author_user_id` or `from_address` vs members (never as external). Contact panel shows a teammate card (no Block/Approve). AgentLoop injects `## Current operator` (name, email, role) for non-external trust. `GET /api/signals/members` includes `role`.
- **Mailbox disconnect semantics (2026-08):** `DELETE /api/email/connections/{id}` keeps conversation history: threads are detached (`Signal.channel_account_id = NULL`) while per-mailbox routing rules and channel bindings are deleted with the account. Orphaned email threads become read-only for outbound mail: `POST /api/email/send` returns 409 ("mailbox disconnected", or "disabled" for `is_enabled=False` accounts) and never falls back to another mailbox implicitly — only an explicit `connection_id` in the send body rebinds the thread to that mailbox. Real provider send failures return 502 instead of storing a message marked `sent` (dev store-only sends still report `sent`). The AI worker skips email signals without an account (`mailbox_disconnected`). The dashboard detects `email_connection_id == null` on email threads, shows a "mailbox disconnected" notice with a reconnect link in place of the reply composer (notes keep working), and disables Draft with AI (`ReplyComposer.replyDisabledNotice`).
- **Session agent picker, cancel, and in-thread abilities (2026-08):** the composer action is "Bring in an agent" (`agentSession.launcher`), or "Bring in {name}" for one-click when a single agent is relevant. Candidates come from `GET /api/signals/{id}/agent-candidates` (`agent_sessions.thread_agent_candidates`), ranked with a `reason` that labels the picker row: `channel` (the agent `resolve_agent_for_signal` picks for this thread) → `project` (the linked project's `po_agent_id`) → `company` (chat-permitted company agents). Personal assistants are not candidates. `POST /sessions` resolves through `agent_sessions.resolve_session_agent`, not `resolve_chat_target`: any candidate is allowed (403 otherwise), so a channel agent that is closed for direct chat can still be brought in; omitting `agent_id` picks the top candidate. A session becomes history at its first turn: `list_sessions` reports a live `message_count` for active sessions, the card shows **Cancel** until then and **End session** after, and `DELETE /api/signals/{id}/sessions/{sessionId}` (`discard_session`) deletes the conversation plus its `agent_session_started` event so nothing lands in the timeline (it falls back to a normal checkout when turns did arrive). Acting on the host thread: `AgentLoop(tool_signal_id=...)` separates the thread that tool calls hit from the conversation that streaming publishes to; `chat.py` passes `signal.context_signal_id`, so an agent brought into a thread runs its messaging tools on the customer thread. Two tools make that concrete — `suggest_thread_reply` (ungated: proposes a draft through `inbound_agent.create_reply_suggestion`, so it lands as the normal approve/edit/decline card) and `take_over_conversation` (pins `Signal.agent_id`, clears `ai_paused` + assignee, emits `ai_resumed`; refused for personal assistants). `resolve_agent_for_signal` honours an active company `Signal.agent_id` as a thread-level pin before channel bindings, which is what makes a takeover stick for the next inbound message.
- **Inline agent sessions (2026-08):** an operator can bring an agent into any external thread ("Ask agent" in the reply composer, agent picker lists chat targets). `POST /api/signals/{id}/sessions` creates an assistant `Signal` (channel="assistant", `source="agent_session"`, `context_signal_id` = host thread, `session_state="active"`) so the existing chat endpoints handle the conversation with full thread grounding, tools, and MCP access; starting twice reuses the caller's active session. `POST /api/signals/{id}/sessions/{sessionId}/close` is the checkout: it freezes the session (`session_state="closed"`), extracts an outcome (`session_outcome_json`: summary = last agent message, actions = consequential tool calls from message trace steps — read-only research tools like `search_index` are excluded, MCP calls surface the inner tool name) and writes `agent_session_started`/`agent_session_closed` SignalEvents on the host thread. `GET /api/signals/{id}` returns `sessions[]`; the dashboard renders an active session as a live inline chat card (`AgentSessionCard` embedding `DirectChatPanel`, per-answer "Use as reply" copies into the composer) and closed sessions as a collapsed summary row in the timeline that expands to actions + read-only transcript. Sessions are team-visible; the old `AskAssistantPanel` side panel is removed. Sessions cannot be started on assistant threads (400).
- **Bokito AI 3.1 virtual model (2026-08):** the platform ships its own branded model as a first-class catalog row: slug `bokito-ai-3-1`, `provider="bokito"`, `display_name "Bokito AI 3.1"`, no own `model_id`, `is_default_chat=True` (startup promotes it in pre-existing databases and demotes other chat defaults). Routing lives in `services/bokito_models.py` (`select_backing_slug`, static map → `claude-sonnet-4-6` today; `task_hint` param reserved for future task-weight routing/margin optimization). `resolve_model_call` detects the bokito provider and routes to the backing row: `provider_type`/`model_id`/API-key/providerkosten follow the backing model, while `slug` and the usage label `provider="bokito"` keep the Bokito identity — usage ledger, work logs and dashboard show Bokito everywhere. Billing: `ResolvedModelCall.bill_input/output_cost_per_mtok_cents` carry the Bokito list price; `compute_costs` bills customers on those while provider cost follows the backing model (cheaper backing = margin). Real models (Claude Sonnet 4.6 etc.) stay visible and selectable in the catalog; BYOK is untouched. Identity: agents whose call resolves to a bokito model get `BOKITO_MODEL_IDENTITY` (style.py) appended in `_build_system_prompt` — they present as "Bokito AI 3.1" and never say they are Claude/Anthropic; agents on real/BYOK models never get this line. Defaults: `Agent.model`, `RuntimeProfile.model` and platform-apply fall back to `bokito-ai-3-1`; startup migrates retired snapshot ids (`claude-sonnet-4-20250514`, `claude-haiku-4-20250514`, …) on agents + runtime profiles to current slugs (`LEGACY_AGENT_MODEL_REFRESH`). Dashboard: `model-label.ts` maps `bokito-ai*` → "Bokito AI 3.1"; LiveWorkLog humanizes the runtime model. Tests: `tests/test_model_catalog.py`.
- **LLM spend caps + alerts (2026-08):** Tenant caps live in `Tenant.settings_json.spend`: `daily_token_cap` (default 2M tokens/day) and `monthly_customer_micros_cap` (default $100/month, micro-USD); explicit `null`/0 means uncapped. Enforcement happens at the single choke point `resolve_model_call` (`services/spend_guard.check_tenant_budget`): only platform-key (billable, live) resolutions are checked, BYOK is never hard-blocked; over-cap raises `AppError` 402 `budget_exceeded`. Aggregates (platform tokens per UTC day, billable customer micros per month) are cached in-process for 60s. Alerts at 80%/100% fire from `record_usage` (fire-and-forget) through the ops-alert pipeline with pref category `billing-alerts`; dedupe via period-labeled titles + notification cooldown. API: `GET/PATCH /api/cockpit/budget` (PATCH owner/admin). UsagePage shows used-vs-cap bars, a blocked banner, and cap editing. The project `usage_budget` endpoint uses the tenant daily cap instead of the hardcoded 100k default when set.
- **Onboarding first-decision + demo thread + return nudge (2026-08):** The Getting Started checklist steps are `email`, `company`, `assistant`, `watching`, `first_decision`, `team` (the generic `channel` step was dropped). `watching` is done when the seeded heartbeat trigger is enabled. `first_decision` completes when any DecisionRequest on a signal has been resolved — the demo thread counts on purpose. The demo thread is opt-in: `POST /api/app/onboarding/demo-thread` (idempotent) seeds one email-style Signal with `source="demo"`, an inbound message and a pending decision card (`services/onboarding_demo.py`); `DELETE` removes it. Demo threads are deleted automatically when a real channel connects (channels router `create_account`, `ensure_email_account`) and never feed inbox-rule learning (`_record_no_reply_outcome` skips `source="demo"`). The checklist CTA seeds and navigates into the demo thread; a compact dismissible checklist card also renders on the Cockpit. Return nudge: an hourly scheduler pass (`send_channel_nudges`) mails owners of tenants that are 24h-14d old with no external channel, once — sent-at timestamp in `Tenant.settings_json.onboarding_channel_nudge_at`.
- **Platform check-in / watching (2026-08):** New workspaces get an enabled hourly heartbeat named Platform check-in plus one internal Messages thread stored as `operations_signal_id`. The assistant tools `get_platform_watch` and `set_platform_watch` toggle only that seeded trigger (ungated). Setup hub counts enabled automations only and offers Turn on check-in. Cockpit shows a watch banner when the check-in is paused. Existing disabled heartbeats stay off until an operator or the assistant enables them.
- **WhatsApp connect UI (2026-08-30):** `WhatsAppConnectForm` is a three-step guide (Prepare in Meta → Paste in Bokito → Register webhook) with a step rail, per-field hints (Phone number ID ≠ phone number), and a link to Meta’s Cloud API get-started docs. After **Connect number**, step 3 shows copyable Webhook URL + Verify token and the messages-subscribe / test checklist. `AddChannelDialog` uses a WhatsApp-specific subtitle instead of the generic “create a Bokito address” copy. Product-help `inbox/channels` documents the same three steps.
- **WhatsApp inbox channel (2026-08):** WhatsApp Business Cloud API is a native inbox channel. Adapter `apps/api/app/channels/whatsapp.py` mirrors the Slack adapter: `normalize_inbound` (Cloud API webhook `value` → InboundMessages; media types become placeholders like `[Image received]` + caption with the media id in metadata; `statuses[]` are ignored), `send_message` (Graph `POST /{phone_number_id}/messages`, text; error 131047/470 → `failed:outside_service_window` for Meta's 24h customer-service window, 190/401 → `failed:auth`), `verify_signature` (`X-Hub-Signature-256` HMAC with `META_APP_SECRET`). One app-level webhook serves all tenants (`GET/POST /api/channels/whatsapp/webhook`): GET answers Meta's handshake against `WHATSAPP_VERIFY_TOKEN`, POST routes each payload to the `ChannelAccount` with `channel="whatsapp"` and `address == metadata.phone_number_id`. Threading is one continuous Signal per customer number (`thread_external_id` = wa_id); dedupe on `wamid` via the shared `ingest_inbound`. Accounts are created via the generic `POST /api/channels/accounts` (`provider="whatsapp_cloud"`, `credentials={access_token, waba_id}`); `GET /api/channels/whatsapp/setup` (owner/admin) returns webhook URL + verify token for the connect card. V1 is bring-your-own Meta app (System User token pasted in `WhatsAppConnectCard` on `/settings/channels`); Embedded Signup / Tech Provider, template messages, media send are V2. Default AI mode is `suggest` (tenant-tunable on `/settings/communication`); human takeover toggle covers whatsapp threads. UI surfaces: Messages hub rail entry, `whatsapp` composer surface (no signature, Enter-to-send chat style), channel binding routing, marketplace tile that links to the channel settings. Env: `META_APP_ID`, `META_APP_SECRET`, `WHATSAPP_VERIFY_TOKEN` (pair-validated at prod startup). Tests: `tests/test_whatsapp_channel.py`, `src/lib/message-composer.test.ts`.
- **Slack decision-notify + digest mail (2026-08):** Decisions reach operators outside the dashboard. `services/slack_notify.py` posts a Block Kit card (Approve/Deny/Open buttons, decision id in the action value) from the same convergence point as push (`publish_decision` → `schedule_notify_decision_slack`). Targeting: thread assignee DM via `users.lookupByEmail` (opt-in through the `slack` channel on the `decisions` notification pref row; requires `users:read.email` scope), fallback the workspace channel in `ChannelAccount.settings_json.notify_channel_id`. Interactivity: one global `POST /api/channels/slack/interactions` — the payload `team.id` selects the ChannelAccount whose signing secret verifies the raw body; clicks resolve through `resolve_message_decision` (actor mapped via Slack profile email, `via: "slack:U..."` in the SignalEvent payload) and the original message is replaced through `response_url` so buttons cannot double-fire. **Digest mail:** opt-in pref rows `digest-daily`/`digest-weekly` (email-only categories); arq cron `send_tenant_digests_job` at 06:00 UTC (weekly fires on Mondays) → `services/digest_mail.py` aggregates open/unassigned threads, pending decisions, inbound messages and agent-run stats per tenant; empty digests are skipped. NotificationSettings UI renders a Slack column and dash-cells for channels that do not apply to a row.
- **Alembic-managed schema (2026-08):** Postgres schema is Alembic-managed since revision `003_baseline` (replays `create_all` + frozen `schema_patch` column patches + idempotent data repairs, so `upgrade head` converges fresh and existing databases without a manual `stamp`). `init_db()` on Postgres runs `alembic upgrade head` in-process at startup under advisory lock 729001 (api + worker can boot concurrently); SQLite (tests/local) keeps `create_all` + patches. `schema_patch.py` is frozen — every new schema change must be a new Alembic revision with explicit DDL. Because the baseline replays live model metadata, later revisions that add columns (e.g. `004_user_totp`) guard with an inspector check so fresh databases (where `create_all` already made the columns) do not fail. Docker image ships `alembic/` + `alembic.ini`. Runbook: `docs/ops/MIGRATIONS.md`. Since aug 2026 `init_db()` on Postgres runs `_assert_no_schema_drift` after `upgrade head`: every model column must exist in the live schema or startup raises (a model change without a revision then fails the deploy smoke test and triggers auto-rollback instead of surfacing as 500s on every select — this happened with `agents.settings_json`, fixed by revision `013_agent_settings_json`).
- **Human audit coverage + unified activity (2026-08):** Consequential human mutations write `AuditEvent` rows (actor_type `user`): thread status/assignee changes (`signal:updated` — tag/priority tweaks intentionally excluded), bulk actions (`signal:bulk_close|reopen|spam|assign`, one event per bulk with before-states; read/unread excluded), delete (`signal:deleted`), takeover/handback (`signal:takeover`/`signal:handback`), agent CRUD (`agent:created|updated|archived`), invites (`invite:created|resent|revoked|accepted`), member role/removal (`user:role_changed`, `user:member_removed`), workspace settings (`workspace:settings_updated`), tool overrides (`govern:tool_override_update`), budget caps (`billing:budget_updated`), password change (`user:password_changed`), 2FA (`user:2fa_enabled|2fa_disabled`), mailbox disconnect (`email:mailbox_disconnected`), integrations (`integration:connected|revoked`). `GET /api/cockpit/activity` is the unified timeline: RunEvents (kind `agent_run`) merged with user AuditEvents (kind `audit`, `actor_name` resolved from users) sorted newest-first; the Cockpit ActivityPage has an All/Agents/People source filter and shows the actor per audit row. Tests: `tests/test_human_audit.py`.
- **TOTP 2FA (2026-08):** RFC 6238 implemented stdlib-only in `services/totp.py` (SHA-1, 6 digits, 30s period, ±1 step window); secrets stored Fernet-encrypted on `User.totp_secret`/`totp_pending_secret` + `totp_enabled` (Alembic `004_user_totp`). Flow: `POST /api/auth/2fa/setup` (returns secret + otpauth URI) → `POST /api/auth/2fa/enable` (verifies first code, promotes pending secret, audits) → login on enrolled accounts returns `{requires_2fa, challenge_token}` (5-min JWT, type `totp_challenge`, no API access) instead of a session → `POST /api/auth/2fa/verify` exchanges challenge + code for the normal LoginResponse. `POST /api/auth/2fa/disable` requires the password (SSO-only accounts confirm with a current code). `totp_enabled` is exposed in `_user_dict` and `/me`. Frontend: Login page has a 6-digit code step (AuthContext `login` throws `TwoFactorRequiredError`, `verifyTotp` completes), Profile & security has an enroll/disable section (manual key + copy + otpauth link; no QR lib), and `TwoFactorBanner` in the shell soft-enforces the existing workspace `require_2fa` flag for non-enrolled users. Tests: `tests/test_totp_2fa.py`.
- **Learning loop v2 (2026-08):** Learning is on by default — `run_tenant_learning_cycle` / `run_learning_for_enabled_tenants` treat missing `learning_enabled` as true (tenants opt out with `learning_enabled: false`). Guardrails are asymmetric for safety: high escalation (>40%) on `autonomous` still tightens to `assisted` immediately but now writes a `learning:posture_tightened` AuditEvent (actor_type `system`); low escalation (<10%) on `manual` no longer silently eases — it creates a system-proposed `PlatformChange` (resource_type `autonomy_posture`, pending_review, deduped on open proposals) that a human accepts in Govern; accepting applies the posture via `apply_autonomy_posture_change` in `platform_apply.py`. Persona proposals: 3+ negative feedback entries in the past week create a `persona_review` PlatformChange (change_kind `review`, evidence samples in `after_json`, deduped); accepting acknowledges the review — nothing is auto-applied. `PLATFORM_RESOURCE_TYPES` gained `autonomy_posture`/`persona_review` and `CHANGE_KINDS` gained `review`; Govern UI labels added in `govern-labels.ts`. Digest mails include a Learning line (active rules, rule suggestions to review, open system proposals) via new `rules_active`/`rules_suggested`/`learning_proposals` digest keys; learning stats alone do not un-skip an otherwise empty digest. Tests: `tests/test_learning_v2.py`.
- **Mobile: stacked inbox + PWA + web push (2026-08):** Below `md` the inbox stacks: `ThreadList` fills the viewport (`w-full md:w-72`, hidden when a thread is selected), `ThreadDetail`/`DirectChatPanel` show full-width with a back button (`onBack` prop, `md:hidden`) that navigates to the list path; the "Select a thread" placeholder and `DirectChatEmptyState` only render from `md` up and `AgentThreadPanel` only from `lg`. PWA: `public/manifest.webmanifest` (standalone, start_url `/communication`, SVG icon) + theme-color/apple-touch meta in `index.html`; `public/sw.js` is a minimal service worker (no offline caching by design) that renders `push` events (title/body from the payload, tag-collapsed per `kind:signal_id`, click focuses/opens `/communication/inbox/all/t/{signal_id}`); registered in `main.tsx` in prod builds. Web push: backend already had `POST /api/push/subscribe` + `GET /api/push/vapid-public-key` + `send_push_to_user` (pywebpush VAPID + `expo:` endpoints) wired from gateway publishes; added `POST /api/push/unsubscribe` (delete by endpoint for current user). Frontend `lib/web-push.ts` handles register/permission/subscribe/unsubscribe; Notification settings has a per-device "Push notifications on this device" toggle reflecting the real browser subscription. Requires `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_CLAIMS_EMAIL` in prod env. Tests: `tests/test_push_notifications.py` (incl. subscribe/unsubscribe roundtrip).
- **Growing composer (2026-08):** `ComposerCard` auto-grows from content height (`scrollHeight`) instead of counting newlines. Chat starts at two lines (52px) and grows to 280px; email replies start at ~6 lines (132px) and grow to 440px; notes sit in between. Operators can drag the top edge (or use the expand control) for more room; the chosen floor persists in `localStorage` (`bokito.composer.floor.{chat|email|note}`). Double-click or Enter on the handle resets. Used in `ReplyComposer`, `DirectChatPanel`, and `/communication/new`.
- **Inbox operator parity (2026-08):** Email replies support CC/BCC (`ReplyBody.cc/bcc` → `deliver_outbound` → stored in `SignalMessage.metadata_json`, exposed via `serialize_message` as `cc`/`bcc`; ReplyComposer has a Cc/Bcc toggle). New outbound email starts from `ComposeEmailModal` (compose button in the thread-list quick filters, `POST /api/email/send`, multi-To/CC/BCC supported end-to-end — `deliver_outbound` accepts a `to_address` override with comma-separated recipients). Forward = compose prefilled with quoted history (`Fwd:` subject), sent as a new thread. **Scheduled send + soft undo:** `ReplyBody.send_after_seconds` (server-capped at 600) stores the message with `send_status="scheduled"` + `SignalMessage.send_after` without delivering; the 60s scheduler tick (`deliver_due_outbound_messages`) delivers due messages; `POST /api/signals/messages/{id}/cancel` deletes a still-scheduled message and returns the body for draft restore. The dashboard gives email replies a 15s undo window (sonner toast with Undo; timeline shows "Sending..." while scheduled). **Full-text search:** on Postgres `list_threads` matches message bodies via `to_tsvector('simple', subject || body_text) @@ plainto_tsquery` backed by expression GIN index `ix_signal_messages_fts` (created in `schema_patch._ensure_search_indexes`); SQLite/tests fall back to ILIKE.
...
- **First-run guided tour (2026-08):** brand-new users get a welcome intro + spotlight tour + assistant setup handoff. Framework: `apps/dashboard/src/components/tour/` — `tour-steps.ts` is the declarative definition (`TOUR_STEPS` with `[data-tour]` anchors, `TOUR_VERSION` for re-showing after major changes, `WELCOME_PILLARS`), `TourContext.tsx` renders three phases (welcome pillar modal, spotlight step overlay with animated cutout + pulse ring + progress dots + keyboard nav, finish card). Anchors: `data-tour="nav-{tab}"` on `ShellSidebar` NavLinks and `nav-group-ai` on the AI group; hidden/missing anchors degrade to a centered card (mobile-safe). Auto-start: tour state unset (or older version) AND onboarding checklist incomplete. State persists per user in `User.settings_json.tour` via `GET/PATCH /api/me/preferences` (`tour` dict, shallow-merged, scalars only, max 20 keys). Replay: "Product tour" in the topbar user menu. Finish CTA deep-links `/communication/new?prefill=<setup interview prompt>&autosend=1` — `NewConversationPage` auto-sends the prefill once the default assistant resolves, starting the conversational workspace setup. Copy in i18n namespace `tour` (EN/NL).
- **Agent-initiated human handoff (2026-08):** external conversations (widget/inbound, `trust="external"`) can always escalate to a human via the `handoff_to_human` tool (`tools/builtin.py`, category `messaging`, `gated=False` so escalation never waits on approval). The tool sets `Signal.ai_paused` + `has_unread`, writes an `ai_paused` SignalEvent (`via: handoff_to_human`), publishes a thread update, and alerts owners/admins through `notify_tenant_admins` (pref category `handoff`, in `DEFAULT_NOTIFICATION_ROWS`). Availability is guaranteed for external sessions: `AgentLoop` appends the tool when the passport allowlist omits it and `execute_tool` skips the passport check for `handoff_to_human` + external trust (operator sessions still enforce the passport). External sessions also get a `## Human handoff` system-prompt section instructing the agent to hand off instead of claiming it cannot reach a human. Operators take over via the existing thread-header toggle; the widget keeps polling so the takeover continues in the same conversation.
- **Policy (Phase 3, 2026-06):** unified tool layer in `apps/api/app/tools/` — `registry.py` (ToolSpec + categories), `policy.py` (allowance engine), `builtin.py` (all tools), `executor.py` (single execution path), `decision_copy.py` (human-readable gate titles/summaries). Each tool category (`messaging|workspace|agents|channels|triggers|integrations|govern`) has an allowance slider `deny|ask|allow` stored in tenant settings (`tool_allowances`, `tool_overrides`). Resolution layers: posture preset → category slider → agent passport (`autonomy_level` manual caps at ask, auto lifts ask→allow) → per-tool override ("Always allow" writes `tool_overrides`; for MCP that is `mcp:{server}:{tool}` so one allow does not unlock all `call_mcp_tool`) → trust clamp (external/widget sessions never auto-mutate; agents/channels/triggers/integrations/govern denied entirely). **MCP discovery** (`list_tools`, `list_resources`, `list_prompts`, `ping`, …) auto-allows without a DecisionRequest. Gated tool cards use plain-language titles like `Approve: List tools on mock-tools` instead of raw JSON; the dashboard also rewrites legacy JSON summaries via `tool-decision-copy.ts`. Legacy `ActionPolicy`/`ActionWhitelistEntry`/apply-modes deleted (tables dropped by `schema_patch`).
...
- **Custom metrics (2026-08):** tenant-defined KPIs next to the built-in Cockpit cards. Models `CustomMetric` (key/label/unit/target, max 24 per tenant) + `CustomMetricPoint` (append-only observations with source `agent|user|system`), Alembic `007_custom_metrics`. API `/api/metrics` (list with latest value + delta vs previous point, CRUD, `/{id}/points`); shared logic in `services/metrics.py`. Agent tools `record_metric` (creates the metric on first use, ungated) and `list_metrics` let agents fill KPIs universally; the onboarding assistant prompt tells it to set up metrics during pillar 5. Frontend: `metrics.routes.ts` + `lib/metrics-api.ts` (unit formatting: percent, EUR currency, minutes duration), Cockpit section `CustomMetricsSection` with metric cards (delta arrows, target, agent-source icon) and an add/edit dialog that can record a value inline.
- **Correction loop (2026-08):** operators can correct an agent's interpretation from the inbox. Agent messages keep thumbs (existing `POST /api/messages/{id}/feedback`) plus a **Correct** action; decision cards got thumbs (generic `POST /api/learning/feedback`, subject_type `decision`) and a "Correct {agent}'s interpretation" action. Both use `lib/correction-chat.ts` (`useCorrectionChat`): it records a down-sentiment feedback row, creates a chat conversation with the responsible company agent (`agent_id` from the message payload; if none, the operator must pick a company agent on New chat) grounded in the source thread via `context_signal_id` (the agent loop injects the full transcript), and auto-sends a correction prompt instructing the agent to ask clarifying questions and persist the learning (via `write_doc` and `suggest_inbox_rule`).
- **Truth fixes (gap-pass phase 1, 2026-08):** notification prefs gate every emission: `notification_channels` returns a `slack` key (gates decision Slack DMs), decision push respects the `decisions` desktop pref, and in-app decision Notification rows for a specific user are created with status from `decision_bell_status` (pref off = born `read`, so no bell noise/realtime ping while keeping the DB anchor). The unused `mobile` channel was removed from prefs. Cockpit/Usage format costs in USD (ledger currency); paused agents no longer fire triggers (`_resolve_agent` checks `is_active`, `last_status="agent_paused"` surfaced in Agenda); WhatsApp UI was hidden until the adapter shipped (2026-08, see WhatsApp inbox channel entry); messenger module toggles (home/messages/help/tools — changelog removed) actually drive widget tabs through the theme payload; marketplace honesty (shopify_mcp `coming_soon`, Outlook no calendar claim, mock OAuth success paths removed); /os leftovers retired (aios locales, stale hrefs).
- **Learning loop closes (gap-pass phase 2, 2026-08):** corrections and thumbs now feed real automation. (1) Agent tool `suggest_inbox_rule` (ungated, in `builtin.py`) lets agents propose an InboxRule (sender/domain/list_id x auto_close/auto_task/mute_ai) via `inbox_rules.suggest_rule` — the rule lands as `suggested` (source `agent`, audit `inbox_rule:suggest`) and a human activates it in Inbox settings; active/paused rules are never overwritten. (2) `learning.suggest_rules_from_feedback` (in the learning cycle) clusters 7-day thumbs-down on messages per email sender; 3+ distinct threads (PROMOTION_THRESHOLD) suggest a `mute_ai` rule for that sender. (3) Triage is visible: `serialize_thread` exposes `category`/`urgency`/`certainty`/`ai_summary`/`triaged_at`, rendered as a Sparkles chip in the thread meta row (tooltip = AI summary). (4) `certainty_threshold` (1-10, InboxSettings, used by `interpretation.py` to suppress priority raises below threshold*10 certainty) is configurable in Settings > AI > Communication (Triage card, `GET/PUT /api/inbox/settings`). Dead InboxSettings fields `rules_text`/`labeling_enabled` dropped (Alembic `009_inbox_settings_slim`).
- **Persona single-store (gap-pass phase 2, 2026-08):** the `assistant_personas` table is gone (Alembic `008_persona_doc` folds non-empty rows into the doc and drops the table). Persona lives only in the `persona.md` workspace doc: `services/persona.py` parses/renders structured `## Tone` / `## Do` / `## Don't` sections (unmanaged sections like appended feedback reviews are preserved on save); the Messenger-settings Tone/Do/Don't form round-trips through `GET/PUT /api/persona` backed by that doc; agents already read it via `build_workspace_context` (kind `persona`), so edits take effect on the next agent run. Govern `persona_review` approvals now apply something concrete: `apply_persona_review_change` appends the proposal's `proposed_addition` (built from negative-feedback comments in `learning.propose_persona_review`) as a dated `## Feedback review YYYY-MM-DD` section to persona.md.
- **Intelligence Stack (2026-06 restructure):** Product backbone mapped to Salim Ismail layers — visible in OS Graph pipeline UX (hybrid IA), not as separate rail items per layer.
  ...
  - **Email received-time fidelity + backfill window (2026-08):** inbound sync stores the provider's actual delivery time — Graph `receivedDateTime` (added to `$select`) and Gmail `internalDate` (epoch ms) — parsed to naive UTC and carried on `InboundMessage.received_at`; `ingest_inbound` stamps `SignalMessage.received_at`/`created_at` and only moves `Signal.last_message_at` forward (backfill never reorders threads). API timestamps from `signal_threads._iso` get a `Z` suffix so browsers stop parsing naive UTC as local time (this caused the "2h ago" offset on fresh mail). Each mailbox has a backfill window `settings_json.sync_window_days` (default 30, 0 = unlimited, max 3650) applied only on the initial fetch (no cursor): Graph adds `$filter=receivedDateTime ge ...` to delta + list, Gmail adds `q=after:{epoch}`; a client-side guard drops out-of-window items on paths without a server filter. Exposed as `sync_window_days` on `GET /api/email/accounts` and updatable via `PUT /api/email/connections/{id}/mailbox-settings` (which now only touches keys present in the body); the Email settings table has a "History" column (7/30/90 days, 1 year, Everything).
  - **Full email text for agents (2026-08):** HTML-only email (most support desks) has no `text/plain` MIME part; the Gmail parser previously fell back to the provider `snippet` (~200 chars) for `body_text`, so agents/triage saw mail "cut off mid-sentence" while the UI rendered the full `body_html`. The parser now derives text via `html_to_text` before falling back to the snippet (Graph already did this), and `services/signals.message_plain_text()` repairs already-synced rows at read time (prefers HTML-derived text when it is >120 chars fuller). Used by the inbound agent prompt (`process_inbound_signal`), triage, `serialize_message`, and the assistant transcript/history builders.
  ...
  - **Session refresh + 401 recovery (2026-08):** access tokens live 60 min (`access_token_expire_minutes`); the dashboard refreshes proactively 5 min before expiry. Sessions restored from `sessionStorage` schedule that refresh from the JWT `exp` claim (`jwtRemainingSeconds` in `lib/auth-retry.ts`) instead of silently never refreshing. A global fetch interceptor (`configureAuthRetry`, installed once by `AuthContext`) catches 401s on `/api/*` calls that carried a Bearer token (excluding `/api/auth/*`), performs one single-flight cookie refresh, and retries the request with the new token (marker header `x-bokito-auth-retry` prevents loops); when refresh fails the session is cleared so route guards redirect to login. The gateway WebSocket already backs off on `unauthorized` and reconnects with whatever token the auth layer holds.
  - **Integration connect suggestions with brand identity (2026-08):** `suggest_integration` decisions carry the provider slug in the connect option payload; titles use the catalog display name (`PROVIDER_BY_SLUG`, e.g. "Connect GitHub?"). Both decision surfaces render the provider brand logo via `IntegrationHostLogo` + `resolveProviderBrand` (inbox `DecisionRequestMessage` and assistant-chat `ChatDecisionCard`; the chat serializer `serialize_decision_for_chat` exposes `provider` per option). Approving "Connect now" resolves the decision and deep-links to `/settings/marketplace?connect={provider}`, which opens the connect hub for that integration.
  - **Automated / no-reply mail handling (2026-08):** the AI never drafts a reply to automated mail. Detection has two layers in `services/automated_mail.py`: (1) deterministic heuristics — sender local-part patterns (`noreply`, `donotreply`, `notifications@`, `alerts@`, `mailer-daemon`, `postmaster`, `bounce`, `newsletter`, …) plus RFC auto-mail headers captured at sync time (`auto_headers` in message metadata: `Auto-Submitted`, `Precedence: bulk/list`, `X-Auto-Response-Suppress`, `List-Id`/`List-Unsubscribe`, null `Return-Path`); Gmail supplies these from `format=full` headers, Graph only when `internetMessageHeaders` happens to be present; (2) a prompt guardrail — suggest and auto prompts instruct the model to return `NO_REPLY_NEEDED: <summary>` for automated notifications it recognizes (parsed by `extract_no_reply_summary`). **Heuristic path (2026-08 UX):** `acknowledge_automated_mail` writes a quiet `no_reply_noted` timeline event and leaves the thread open — it does **not** create an awaiting decision (busy personal mailboxes were flooding Agents / Cockpit with 100+ fake blockers). **Agent-judged path:** still calls `create_action_suggestion` ("No reply needed" card with Close / Create task / Keep open). Those tip cards are **excluded** from `agents_attention`, Cockpit `open_decisions`, `has_open_decision`, and the Decisions (`awaiting_decision`) queue; badge payload adds `no_reply_suggestions`. Operators clear a backlog via `POST /api/signals/dismiss-no-reply-suggestions` (Agents page banner). Active inbox rules still short-circuit before classification. Dashboard tip cards remain calm (BellOff). Preview excerpts use `clip_with_ellipsis`.
...
- **Widget branding + taal (2026-08):** de widgetnaam is dynamisch — keten `appearance.chatbot_name` (expliciete override) → companyagent-naam (mits aangepast; het generieke bootstrap-"Assistant"/"Assistent" telt niet) → `tenant.name`. "Bokito AI" verschijnt nooit meer op tenant-widgets. De theme-payload (`livechat_theme_from_tenant`, meegegeven `assistant_name` door `session/start`) bevat nu ook `locale` (uit `resolve_workspace_language`) en gelokaliseerde welcome-defaults wanneer titel/ondertitel leeg zijn (nl: "Welkom"/"Hoe kunnen we je helpen?", en: "Welcome"/"How can we help?"; overige talen → en). Tenant-bootstrap zaait geen Engelse welcome-teksten of `chatbot_name` meer: leeg = default tot de tenant het aanpast. Widget-chrome volgt `theme.locale` (fallback embed-attribuut/paginataal); `WIDGET_CHROME` (en/nl) dekt nu ook composer-placeholder, pre-chatformulier, CSAT, offline-/handoff-banners, lijst-states, chat-acties, sign-in-panel en "Powered by". De statische widget-HTML bevat geen hardcoded naam/welkomsttekst meer (geen taalflits); de Tools-tab is verborgen tot de sessie echt tools levert. `MessengerSettings`-placeholders tonen de resolved naam (`resolveWidgetName` in `messenger-appearance.ts`) en dezelfde gelokaliseerde welcome-defaults (teamtaal via AI-communication settings).
- **Widget unread badges (2026-08):** de livechat-lijstendpoints (`user/conversations`, `customer/conversations`) leveren `unread_count`: agent-berichten nieuwer dan het laatst-gezien-moment of de laatste eigen reply. "Gezien" wordt per viewer bijgehouden als geüpdatet `SignalEvent` `event_type="widget_seen"` (actor `user:{id}` of `customer:{id}`); het ophalen van `conversation/{id}/messages` geldt als leesmoment en reset de badge.
- **Widget image vision (2026-08 fix):** the widget sends full attachment metadata (`{id, url, mime, name, size}`) with `stream-chat`; the backend (`AgentLoop._apply_attachments_to_messages`) inlines images as base64 vision blocks. Because customer sites may cache old widget bundles that sent bare `{id, url}`, the loop is defensive: missing mime falls back to an extension guess (`_attachment_mime`) and, failing that, the bytes are fetched and magic-byte sniffed (`_sniff_image_mime`, jpeg/png/gif/webp) before deciding image vs "[Attached files: ...]" note. The inbox renderer (`MessageAttachments`) treats missing mime the same way (extension on name/url) so a stored `{id, url}` attachment no longer crashes the thread view with `mime.startsWith`. Widget uploads are image-only (10MB cap) via `/api/livechat/.../attachment`.
...
- **Outlook mailbox connect toont account-picker (2026-08-25):** `start_real_oauth` zet `prompt=select_account` op de Microsoft authorize-URL wanneer `flow="email"` (Connect Outlook via `/api/email/oauth/start`, `/api/email/outlook/oauth/start`, of `/api/integrations/oauth/start?provider=outlook`). Zonder die prompt hergebruikt Microsoft stil de browser-sessie, zodat de verkeerde mailbox wordt gekoppeld. Platform SSO (`flow="login"`, `/api/auth/microsoft/start`) krijgt géén prompt en mag de huidige Microsoft-sessie hergebruiken.
- **Prod redirect_uri geverifieerd (2026-08):** alle FastAPI OAuth-flows (Outlook mailbox, Microsoft SSO, Gmail, GitHub) delen exact één callback: `{PUBLIC_API_URL}/api/integrations/oauth/callback` (`config.Settings.oauth_redirect_uri`). Prod `.env.prod` heeft `PUBLIC_API_URL=https://app.bokito.ai`, dus prod stuurt letterlijk `https://app.bokito.ai/api/integrations/oauth/callback` (live geverifieerd via `/api/auth/microsoft/start` en `/api/email/outlook/oauth/start`). Krijg je `invalid_request … redirect_uri is not valid` op login.live.com, dan ontbreekt die URI in de Entra app registration (client `f44d9848-89f3-4e7d-8636-a0756ddc87d2`) onder Authentication → Web → Redirect URIs; de registration moet ook personal Microsoft accounts toestaan (tenant `common` → login.live.com valideert strikt).
- **Entra app registration "Bokito" gefixt (2026-08-17):** de registration (client `f44d9848-…`, tenant `4659e272-e64c-4522-ad10-31d66462df82`, eigenaar `lorenzo@bokito.ai`, `signInAudience=AzureADandPersonalMicrosoftAccount`) bevatte alleen twee legacy Xano-callbacks (`https://api.bokito.ai/api:app/email/outlook/oauth/callback` en `https://api.bokito.ai/api/auth/callback/azure-ad`). `https://app.bokito.ai/api/integrations/oauth/callback` is toegevoegd via de portal; daarna accepteren zowel login.live.com (MSA) als login.microsoftonline.com de authorize-URL. Diagnose-tip: met de prod client-credentials (`MICROSOFT_OAUTH_CLIENT_ID/SECRET` uit `.env.prod`) kan een app-only Graph-token de registration **lezen** (`GET /v1.0/applications(appId='…')?$select=web`), maar niet wijzigen (PATCH → 403 zonder Application.ReadWrite-permissie).
- **Gmail OAuth prod geconfigureerd (2026-08-17):** Google Cloud-project "Bokito ai" (`lorenzo@bokito.ai`) heeft OAuth-client "Bokito Production" (web application, redirect `https://app.bokito.ai/api/integrations/oauth/callback`); Gmail API enabled. `GOOGLE_OAUTH_CLIENT_ID/SECRET` staan in `/opt/bokito/.env.prod` (client-id `582551847741-…apps.googleusercontent.com`). Let op: de OAuth consent screen staat op **Testing** (External) — alleen testgebruikers kunnen een Gmail-mailbox koppelen; `lorenzo@bokito.ai` is toegevoegd als test user. Voor klanten moet de consent screen naar "In production" gepubliceerd worden (Gmail-scopes zijn restricted en vereisen dan Google-verificatie). "Internal" user type is niet beschikbaar: het project hangt niet onder een Google Workspace-organisatie. UI-valkuil: in het "Add users"-dialoog van de Google Auth Platform-console moet je na het typen van een e-mailadres **Enter** drukken zodat het een chip wordt; anders sluit "Save" zonder fout maar wordt er niets opgeslagen.
...
- **Catalogus**: `GET /integrations/providers` uit `integration_providers` (seed: github, outlook, gmail, king_accountancy, bjorn_lunden_mcp, custom_mcp, plus remote MCP OAuth slugs in `docs/archived/v1/integration-providers-seed.md`). Marketplace groepeert providers per host; fallback-metadata in `integrations-data.ts` en `mcp-remote-providers.ts`. Platform-tabellen (`integration_hosts`, `integration_providers`, `integration_connections`, `integration_bindings`, `github_connections`) en list-endpoints (API ids 307–312) zijn live op FastAPI workspace 1. Core hosts + providers seeden via `node scripts/seed-integration-providers.mjs` (hosts eerst, daarna providers met `host_id`).
...
- **Merklogos op connect-kaarten (aug 2026)**: koppelbare systemen tonen hun echte merklogo in plaats van generieke Lucide-iconen. `components/integrations/BrandMark.tsx` levert `BrandTile` (logo-tegel voor kaartheaders, via `IntegrationHostLogo`) en `BrandMark` (klein inline logo voor knoppen/accountrijen); beide resolven via `useIntegrationBrand` met statische fallback uit `brand-assets.ts`. `SettingsSection` heeft een optionele `icon`-prop voor de leading tegel. Toegepast op de WhatsApp- en Slack-connectkaarten in `/settings/channels` (mailboxrijen hadden al `ProviderLogo`). WhatsApp-logo toegevoegd: `public/brands/logo-whatsapp.svg` (Simple Icons, `#25D366`) + slug `whatsapp` in `BRAND_ASSET_PATHS` en `HOST_STATIC_BRAND_META`.
...
- **MCP**: `POST /integrations/mcp/install`, `GET /integrations/mcp/bindings` voor tenant `mcp_server_ids`. Providers: `king_accountancy` (`native://king-accountancy`), `bjorn_lunden_mcp` (platform MCP server id 8 / `native://bjorn-lunden`), `custom_mcp` (URL + auth in connection metadata). Marketplace setup-modal gebruikt gedeeld `McpConnectionForm`; `/integrations/mcp` blijft volledige beheer-UI.
...
- **Setup Hub (2026-08)**: `/settings/setup` (redirect vanaf legacy `/integrations/setup`; oude `IntegrationSetupPage` verwijderd). Zes pijlers met live voortgangsdetectie (niets opgeslagen): 1) Communication, 2) Intelligence (company.md + assistant-gesprek + agents; violet brein-icoon; extra regel + link naar modules), 3) Automations, 4) Branding & widget, 5) KPIs, 6) Projects. Header-CTA en setup-prefill openen `/communication/new?prefill=&kind=company` (bedrijfs-Assistant, niet de persoonlijke). `ONBOARDING_SYSTEM_PROMPT` beschrijft dezelfde pijlers plus `list_modules` / `recommend_module` wanneer het werk boekhouden raakt.
- **Eén setup-pad (2026-08 UX-pass):** first open → producttour (orientatie) → assistant setup-chat (tour-finish, primair) of **Setup guide** (`/settings/setup`, secundair). De Setup guide is de enige volledige checklist. Communication-empty en Cockpit `OnboardingCompactCard` zijn **Continue setup** (één volgende stap + Open setup guide + Talk to your assistant), geen tweede 5-stappenlijst. Landing blijft Communication (`HomeRoute`), niet Cockpit. SMTP/IMAP "Coming soon" is verborgen tot het werkt; gebruikers worden naar het Bokito-adres gewezen.
- **Connected tools naamgeving (2026-08)**: gebruikersgerichte "MCP"-labels zijn hernoemd naar **Connected tools** (NL: **Gekoppelde tools**) in settings-nav, integratie-filters, pageMeta en de servers-tab; "MCP" blijft alleen als technische toelichting tussen haakjes. Route blijft `/settings/mcp`.
- **Integratie-modules (2026-08)**: een **module** is een first-class product-hub (schema, verbs, skill, setup, sources), geen Settings-only power-switch. Een **connector** is het pakket eronder (KING, Moneybird). Inbox/GitHub blijven kanalen/kinds, geen modules. Enablement staat in `Tenant.settings_json.modules` (`{ "accounting": { "enabled": true, "default_connection_id": "…", "default_company_by_connection": {…} } }`). Expliciete flag wint; zonder flag blijft een live pakket de module aan (legacy). Nieuwe tenants starten uit. Uitzetten revoken geen koppelingen; een pakket koppelen zet de parent-module automatisch aan en seedt platformbronnen. Registry is `ModuleSpec` in `apps/api/app/modules/catalog.py` (accounting `available`; banking, investing, documents `coming_soon`). Hub: `/modules` (`ModulesPage`); module-home: `/modules/:slug` met tabs Overview / Connections / Sources / Setup. Legacy `/settings/modules*` redirect naar `/modules*`. Setup Hub (Intelligence) linkt naar `/modules`. Multi-registratie: meerdere `IntegrationConnection` / MCP-servers per provider; OAuth `bokito_create_new=1` op return URL; `GET/PATCH /integrations/modules/{slug}/connections|prefs`. ModuleSource (`module_sources`, migratie `019`): platformseeds Accounting (RJNet, NBA HRA, Belastingdienst) + tenant-URLs; worker `index_module_source_job` + wekelijkse cron. Agent tools: `list_modules`, `recommend_module`, `list_module_connections`, `set_module_default_connection`, `list_module_sources`, `search_module_sources`, `propose_module_source` (decision `add_module_source`). Frontend-fallback: `FALLBACK_MODULES` in `integration-modules.ts`.
- **Agent ziet modules alleen als ze aan staan**: tenant-snapshot `Modules:` toont `off` / `on, no package` / `connected`. Coming-soon in één regel “prepared, not connectable”. `filter_tools_for_modules` verbergt `accounting_*` (en later `banking_*` / `investing_*` / `documents_*`) tot de module aan staat. Skill-playbook alleen bij on + geen pakket; volle skill bij on + connected. `accounting_*` geeft eerst `module_off`, daarna `no_connection`; meerdere connections eisen `connection_id` of module-default. Tools `list_modules` en `recommend_module` blijven ungated. `recommend_module` maakt `enable_module` als de module uit staat, anders `setup_integration`. Goedkeuren van `enable_module` / `add_module_source` persist in `notifications.py`; de UI navigeert naar de modulepagina. Heartbeat-seed + bestaande heartbeats: accounting **off** → `recommend_module`. `ONBOARDING_SYSTEM_PROMPT`: eerst aanzetten, daarna een pakket koppelen.
- **Accounting-module**: één contract over KING Accountancy, Björn Lundén en Moneybird (Exact Online en SnelStart schema-ready, coming_soon). Canoniek schema `apps/api/app/modules/accounting/schema.py` (Company, Party, Document, Account, LedgerLine, Outstanding, BankMutation, TaxRate); capability-matrix `capabilities.py`; router `router.py` (connectie-discovery, verb-dispatch, capability-checks, dev-mocks, `summarize`); adapters onder `adapters/` (king, bjorn_lunden, moneybird). Agents gebruiken builtin `accounting_*` read-tools en `accounting_propose_*` write-tools (writes altijd via DecisionRequest). Moneybird is een OAuth-provider (`oauth_providers.py`; dev-mock-connectie zonder credentials); native client `services/moneybird.py`. Frontend-endpoint: `GET /integrations/modules/accounting/companies`.
- **Mailbox-gated compose (2026-08, Milan walk):** Write email / New email / Email a customer only appear when a mailbox can send. The built-in Bokito address counts (`isSendableMailbox`); Gmail/Outlook need `connected`/`active`. Without a sendable mailbox, New chat and the command palette offer **Connect a mailbox**, compose shows a connect empty state, and Contacts / Cockpit / the thread contact panel hide the write-email trap. Marketplace defaults to **Available** so coming-soon cards are off the first view (`status=all` still shows them). Page-guide banners are a single title + Learn more row. Times use a 24-hour clock in both EN and NL.
- **Communication reply fallback (2026-08):** Email threads without `channel_account_id` still send when the workspace has an enabled mailbox. `deliver_outbound` binds the first sendable account (Bokito, then Gmail/Outlook/mock). The composer blocks only when **no** sendable mailbox exists — not when the thread itself has a null `emailConnectionId`. Website chat (`widget` / `webchat` / `livechat`) stays on the chat surface even if the visitor left an email. Closed/spam composers keep notes and offer **Reopen**. **Quote** works on chat and email. **R** on a closed thread reopens then focuses the composer. Widget visitors with a placeholder address get an ask-for-email hint. Contacts search honors `?q=`. Empty inbox no longer requires a mailbox before New chat / widget setup.
- **Communication tags (2026-08):** Thread “labels” are **tags** end-to-end (UI copy + timeline). A thread can have multiple tags (`Signal.tags_json`); list filter `tag=` matches any of them, so the same thread appears under each Tags sidebar folder. The Tags rail shows pinned default views (`inbox_folders.sidebar_tags` in `/me/preferences`) unioned with the registry from `GET /api/signals/tags`. Tagging a thread refreshes the sidebar via `bokito:signal-tags-changed`. Invoice lookup / bookkeeping CTAs were removed from the contact panel, thread chrome, list badge, post-send toast, and command palette — not part of the Communication product surface. The old list “FACTUUR” / “Invoice” pill was a display-only heuristic, not a tag; AI triage category `billing` now registers and applies the real `billing` tag.
- **Thread list turn arrows (2026-08-28):** Customer rows no longer show the “Needs reply” / “Jij aan zet” text chip. An accent **left** arrow (tooltip: Needs reply / Jij aan zet) means the last real line is inbound or unread; a muted **right** arrow (Waiting on them / Wacht op antwoord) means open + last line outbound. The arrow sits at the far right of the preview row. Tags (and the needs-decision chip) sit under the contact title so the preview stays readable.
- **Tag registry (2026-08-28):** Tags are a first-class tenant vocabulary in table `signal_tags` (`SignalTag`: normalized lower-case `name`, `description` = when to use it, `created_by_user_id`; migration `018_signal_tags` backfills names found on `signals.tags_json`). `services/signal_tags.py` owns it: `normalize_tag(s)` (trim, collapse whitespace, lower, 40 chars, max 20 per thread), `ensure_tags` (auto-register on write), `allowed_tag_names` / `ai_catalog_lines` (AI vocabulary + hints), `catalog` (registry ∪ tags still on threads, with total/open counts), `create_tag`, `update_tag` (rename across registry + all threads, or set the description; renaming onto an existing tag merges), `delete_tag`. API: `GET/POST /api/signals/tags`, `PATCH/DELETE /api/signals/tags/{tag}` (create is member-level; rename/delete/describe are admin-only, one audit event per change). Every tag write path normalizes and registers: `patch_thread`, routing-rule labels (`apply_email_routing`), `apply_triage`, and the `set_thread_tags` agent tool. AI triage prompt lists `name - description` from the registry, so a tag created in settings is usable before any thread carries it.
  - **One surface per tag:** the only tag filter is the tag folder (`/communication/tag/{tag}[/{queue}]`). The old in-memory `tagFilter` in `Communication.tsx` overrode `leafFilters.tag` with `undefined`, so tag folders listed the entire inbox — removed. Clicking a tag chip in the list or on the thread navigates to that folder; the chip renders active inside it, and the folder banner leaves it.
  - **Frontend:** `useSignalTags` (module-cached, refreshes on `bokito:signal-tags-changed`) feeds the sidebar, the `TagPicker` (search the registry, see each tag's hint, create a new tag inline) and settings. `FoldersAndTagsManager` is one tag table: usage counts, pin/unpin to the rail, inline "when to use this tag", rename, delete, plus a create row. The section gear in the Messages rail only appears on hover/focus of that section (`group/section`).
  - **Removed:** the `billing` regex heuristic (`threadLooksFinancial`, `suggestedBillingTag`) and the dashed "Add billing" chip. Tag suggestions come from the registry and AI triage, not from keyword matching.
...
### 2.6d Conversation-driven project work (V1: queue + smart docs + resources)

Core principle: **the queue is the motor, the doc is the truth, the conversation is the source.** Projects in the V1 FastAPI track carry an implementation queue, project-scoped smart documentation, and generic resource slots.

- **Data model** (`apps/api/app/models/project_work.py`): `ProjectQueueItem` (kind feature/bug/task/idea/risk; priority; status machine `proposed → accepted → analyzing → planned → in_progress → verifying → done` plus `rejected`; provenance `origin_type`/`signal_id`/`message_id`; `impact_summary`), `ProjectDocSection` (status layer over `##` headings of a project doc: open/planned/in_progress/implemented/verified/deprecated; stable anchors survive saves), `QueueItemDocLink` (item ↔ section, relation implements/modifies/touches/documents; history persists after completion), `ProjectResource` (generic slot: repo/drive/notion/sheet/vibecode/site/other; `external_ref` + optional `connection_id`; sync fields). `WorkspaceDoc.project_id` scopes a doc to one project (`kind="project_doc"`); tenant Knowledge filters those out.
- **Repo fields migrated**: the GitHub fields formerly on `Project` live in a `ProjectResource(resource_type="repo", provider="github")`; `serialize_project` derives the legacy repo fields from that resource (alembic `016_project_work`).
- **Service** (`apps/api/app/services/project_work.py`): validated status transitions with AuditEvents and SignalEvents back to the source thread (`queue_item_created`, `queue_item_status`); section sync on every project-doc save (deprecates removed headings instead of deleting); section statuses land in `DocChunk` metadata so RAG answers know planned vs implemented.
- **Agent flows**: accepting an item creates an `AgentTask` (`trigger_type="queue_item"`) for the project agent (roster default → po_agent → tenant lead) with full doc/section/resource context; the agent links sections, updates the doc via `write_doc` (Govern-gated), and finishes with an impact summary. Verification (`trigger_type="queue_item_verify"`) checks docs against reality before `done`. `Project.autonomous_mode` auto-accepts conversation-born items and starts analysis immediately.
- **Agent tools** (category `projects`, `apps/api/app/tools/project_work.py`): `list_projects`, `create_queue_item` (renders an inline queue-proposal decision card under `ask` policy), `list_queue_items`, `update_queue_item_status`, `link_queue_item_to_doc`, `set_doc_section_status`, `list_project_docs`, `list_project_resources`, `propose_project_resource`. Posture: assisted=ask, autonomous=allow.
- **Project scope in runs**: `ToolContext.project_id` comes from the run (`AgentRun.project_id`); `write_doc` writes project docs by default in project-scoped runs, `search_repo` scopes to the project repo, and project tools resolve the project from explicit input → thread signal → run scope.
- **Interpretation**: inbound triage detects `intent` (e.g. `implementation_request`, `bug_report`) and `sentiment`; matching intents on project threads add an `add_to_queue` suggested-action chip and the action-suggestion decision card offers "Add to project queue". The inbound prompt includes a project snapshot (thread project + active items, or the tenant project list).
- **API** (all under `/workforce/projects/{id}`): `GET/POST /queue`, `GET/PATCH /queue/{item_id}`, `POST /queue/{item_id}/analyze|verify`, `GET/POST /docs`, `PATCH /docs/{doc_id}/sections/{section_id}`, `POST .../sections/{section_id}/links`, `GET/POST/PATCH/DELETE /resources`.
- **UI**: `ProjectDetail` is a three-tab surface (**Queue · Documentation · Settings**). Queue tab groups items by status with kind/priority badges, origin chip linking to the source thread, linked-section chips, and Analyze/Verify actions. Documentation tab lists project docs with a colored status rail per section and per-section linked items. Settings holds orchestration, repository + resources, budget, about, and the **Autonomous mode** toggle. Project cards show open-queue and doc-health badges; the thread detail panel shows an open-queue counter for the linked project.
...
#### Inbox operator walk (2026-08, less-techy pass)
- **Cockpit tiles** spell out what a number means (`awaitingDecisionHint`, `autonomyHint`). Partial-load slice labels are human words (`how much agents can do`, `pending reviews`). Empty CSAT goes to `/settings/widget`. When the platform check-in is off, the banner title is **Hourly inbox scan is off**.
- **Govern Undo** opens a confirm dialog that shows the change summary. `applied_yolo` reads **Applied without review**. Hardcoded Cancel is i18n `history.cancel`.
- **Messages sidebar:** queue rows have tooltips (All / Mine / Open / Unassigned / More). **Team chat** is the internal channel. An empty mailbox list shows **Connect email**. Settings leaf is **AI reply settings**. **Agent runs** has a hint.
- **Composer:** **Templates** is a visible label. Note vs Reply reminders (**The customer sees this** / **Only your team sees this**) sit on the tab tooltips. **Send and close** and **Send and snooze** live under the chevron next to Send. New conversation has **Back to Messages** and a To-field hint.
- **Contacts:** **Group contacts by company** is the backfill action. Status chips have hints. A thread without a person offers **Add to contacts** via `newContactPath(address)`. Dirty contact/company edits warn on tab close. Create form seeds `?address=`.
- **Activity:** Headlines vs Full log have titles. **Jump to newest** / **Clear list (does not delete data)**. Empty primary CTA is Open Messages.
- **Onboarding:** demo-thread failure toasts. Watching step primary CTA is Setup; secondary is ask the assistant.
- **Help center (public):** Retry on load error. Empty catalog stays on the public page (settings stay in Help centers for operators).
- **Agents library:** Lead badge also shows **Handles mail when nothing else is assigned**.
- **Members:** invite copy explains they will see Messages; selected role shows a one-line hint.
- **AI reply settings:** leaving with unsaved changes warns. Page title matches the Messages sidebar label.

#### Inbox operator walk (2026-08, SMB-owner pass)
- **Leftover suggestion cards** collapse to one line (`Earlier draft — dismissed` / `Eerder concept — terzijde gelegd`) once resolved or deferred. Feedback thumbs stay off those leftover cards. Approving one Suggested reply defers sibling Suggested reply cards (`human_approved_sibling`). `_defer_open_reply_suggestions` only matches `title == "Suggested reply"` so other awaiting decisions stay open.
- **Draft cleaning:** `split_suggestion` strips echoed invoke/draft instructions (`A teammate asked you…`) and `> Note for the reviewer` lines. `POST /signals/{id}/draft` and invoke-agent `reply_suggestion` run the same cleaner. The dashboard `stripAiScaffolding` / `translateMockAgentBody` hides leftover leaked prompts in already-stored cards.
- **Needs reply must not swap the inbox for setup.** `isInboxEmpty` is first-run only: no search, quick filter All, and no open thread. Search empty states offer **Clear search**. Mailbox load errors and missing-org screens have Retry / Sign out. Server `needs_reply=1` ignores mock/placeholder `agent_message` bodies (`I received your message about:` / `[mock]`), matching list previews — a stub reply does not count as answered.
- **Decision buttons** for less-technical operators: **Not now** / **I'll handle it myself** (NL **Niet nu** / **Ik doe het zelf**) on the canonical single escalate option (`id=escalate`). Agent-authored multi-choice cards keep each option's own label even when several share `action_type=escalate` — the UI must not collapse them all to "I'll handle it myself". Prefer `send_reply` + `payload.body_text` for choices that should mail the customer (e.g. ask for clarification). Policy gate options `always_auto` translate to **Always allow** / **Altijd toestaan**. In production, `call_mcp_tool` against `mock://` MCP servers returns unavailable to the agent instead of creating a dead-end approval card. Customer-facing email rewrites relative `/docs` / `/learn` markdown links to absolute `public_app_url` links (`format_customer_email_body`) and expands mistaken `/docs/{slug}` (missing section) via `get_article`. `search_product_help` returns `path`, `docs_path`, and `public_url` so agents cite `/docs/{section}/{slug}` exactly. When suggest-mode ends with bare `Done.` (no decision card) or a meta draft about Govern/concepts, the worker raises a **Needs your attention** card instead of leaving the thread empty. Defer toast is **Saved for later**.
- **Empty states:** Contacts filtered-empty has **Clear filters** and always shows the Pending chip hint. Activity filter-no-matches is distinct from “no activity yet”. Agent library empty search clears the query. Agent-runs Decisions chip empty CTA includes **Back to Open**. Members without invite permission see **Ask an admin**. Assistant missing config is a primary button to `/settings/assistant`. Delete confirm uses i18n.
- **Cockpit:** at most one promo banner besides onboarding (guide and hourly-scan hide while setup is incomplete). Autonomy % always shows “Share of replies sent without you clicking Send.” Tokens tile is **AI usage this month**. Review CTA is **workspace changes to approve**. Open-decisions empty copy points at customer threads.
- **New conversation** treats **Email a customer** and **Chat with assistant** as equal chips. Reply composer CC/BCC placeholders are i18n. The thread-list shortcut footer was later removed (shortcuts stay behind **?**). Agent-runs chip row has a muted subtitle. Govern Agent access tab has a one-line hint. Agent library model chips keep the pretty label and put the raw id in the tooltip.

#### Inbox operator walk (2026-08, power-user pass)
- **Keyboard is shared:** `useInboxListShortcuts` drives Communication and Assistant/agent chats. J/K move and scroll the active row into view. Esc leaves the thread (back to the list). H snoozes 1 hour (undo reopens). X toggles bulk select. R refuses closed/spam. Shift+A opens the assignee picker. N starts a new chat. L copies the canonical link. `#` copies the thread id. 1–5 switch All / Needs reply / Unread / Pinned / Needs decision. `/` works even if compose is open, unless focus is inside the dialog. New conversation Esc returns to the last inbox queue.
- **AND list flags:** `GET /api/signals` accepts `unread=1`, `needs_reply=1`, `needs_decision=1`, `pinned=1` on top of the current `view`/`folder`. `needs_reply` is open + (unread or last inbound `user_message` newer than last outbound reply). `needs_decision` is threads with an open `DecisionRequest`. Serialized threads include `has_open_decision` for the list badge. Human outbound replies are `user_message`/`outbound`, so they must count as answered. The dashboard sends the flags from the Unread / Needs reply / Needs decision / Pinned chips so they compose with Open/Mine/etc. Live gateway upserts honor the same flags in `threadMatchesFilters`.

#### Modules in the rail (2026-08-28)
- **Modules** is a top-level rail item under the Settings group (`TAB_GROUPS`), path `/modules` (not under Settings layout), Lucide `Boxes` icon. Active highlighting uses `tabFromPath` matching `/modules`. Legacy `/settings/modules*` redirects to `/modules*`. Settings sidebar no longer lists Modules; Connected / Marketplace / MCP stay under Integrations.
- A temporary **New** / **Nieuw** text badge (`isNewTab`, `tabs.modules.newBadge`) sits next to the label (dot when the rail is collapsed). Count badges for inbox/agents are unchanged.
- Module home tabs: Overview (packages + verbs), Connections (multi-registration + defaults), Sources (`ModuleSource`), Setup (company-assistant deep-link).

#### Uniform folder system (channels, tags, agents) (2026-08-28)
- **One repeatable folder model**: every channel, tag, personal assistant, and company agent in the Communication sidebar is a `SidebarFolder` with the same sub-queues — **Open, Mine, Unassigned, Closed** (`SUB_QUEUES`). Clicking a folder expands it (and opens the default sub-view); clicking again collapses it. Sub-items stay hidden until then.
- **All communication** (2026-08): the fixed top inbox is the same folder pattern — one `SidebarFolder` labelled **All communication** / **Alle communicatie** (`support.inbox.allCommunication`) instead of flat All / Mine / Open / Unassigned / More. Scope key `inbox`; `HubLeaf` inbox queue is optional. Expand opens the configured default (Open or Mine from `inbox_folders`), with **Snoozed** and **Spam** as extra rows under the standard four. Closed lives in the standard sub-list (no separate More for it).
- **Accordion**: only one folder's sub-list is open at a time (`expandedLeaves` holds a single scope key) to keep the rail scannable.
- **Agent paths** carry the same queues: `/communication/assistant/open`, `/communication/agent/{id}/mine` (+ `/t/:threadId`). `DirectCommunication` maps the queue via `SUB_QUEUE_TO_VIEW` onto `folder=assistant` + `agentId`.
- **Nav column is resizable** (`MessagesHub` SplitPane, max width 380px). Folder default sub-view still roams via `/me/preferences` `inbox_folders`.

#### Uniform mappensysteem per kanaal + tags (2026-08-28)
- **One repeatable sub-view set** (Front-model): every channel folder and tag folder in the Communication sidebar nests the same queues — **Open, Mine (Van mij), Unassigned (Niet toegewezen), Closed (Gesloten)** (`SUB_QUEUES` in `messages-paths.ts`). URLs: `/communication/channel/email/12/mine`, `/communication/channel/webchat/open`, `/communication/tag/billing/open` (+ `/t/:threadId`). `HubLeaf` channel leaves carry `queue?`; a new `tag` leaf carries `tag` + `queue?`. `configForLeaf` maps the sub-queue to server views (`all_open`/`mine`/`unassigned`/`closed`); a folder without queue keeps view `all`.
- **Sidebar folders** (`SidebarFolder`/`QueueSublist` in `QueueSublist.tsx`): clicking a folder opens its default sub-view and expands; the chevron expands/collapses without navigating. Expansion state persists in `communication-sidebar-prefs` (`expandedLeaves`, keyed by `folderScopeKey`). A folder auto-expands once when it becomes active.
- **Tags section** is a movable/hideable sidebar section (registered next to agents/channels in `SidebarCustomizeDialog`). It lists the tenant tag catalog from `GET /api/signals/tags` (`[{tag,total,open}]`, customer threads only, sorted by usage) with the open count as badge, plus a **Manage tags** link to `/settings/channels#tags`.
- **Default sub-view is configurable and roams**: `/me/preferences` stores `inbox_folders` (`default_queue` + `channel_defaults` keyed by folder scope key like `channel:email:12`). Global default is Open. UI: Settings → Email & messages → **Folders and tags** (`FoldersAndTagsManager`), which also renames/deletes tags in bulk (`PATCH/DELETE /api/signals/tags/{tag}`, admin-only, one audit event per bulk change).
- **AI tagging is curated**: triage (`triage_signal`) may propose `tags` but only from the tenant's existing catalog (top 30 by usage in the prompt); `apply_triage` merges as a union — operator tags are never removed. Agents get the same constraint via the `set_thread_tags` tool (messaging category); unknown tags are rejected with the catalog echoed back. New tags enter the catalog only through manual tagging or routing-rule labels.
- **Channel leaf filter integrity (2026-08):** Inbox-only UI `channelFilter` must not overwrite leaf-scoped `channel`. The old merge `channel: channelFilter ?? undefined` wiped Websitechat (`webchat` → API `widget`), WhatsApp, and Slack so those folders listed all open threads (mostly email). `mergeHubThreadFilters` in `lib/hub-list-filters.ts` applies the chip only on inbox leaves; channel leaves keep `channel=widget|whatsapp|slack`. Email mailbox leaves also send `channel=email` with `connectionId`. Wrong-scope deep links on channel/tag leaves redirect via `threadFitsChannelLeaf` / `threadFitsTagLeaf`. Live client filter set aligns with API `EXTERNAL_CHANNELS` (includes `whatsapp`, `api`).
- **Cohesion pass (2026-08):** Communication list filter **Needs decision** empty state links to Agent-runs **Decisions** (not only “show all”). Agent library cards add a **Runs** quick link (`?agent=`). Modules cards (when on) link to Agents and Govern policy. Tags empty state offers “Open a conversation to add a tag” plus Manage tags. Cockpit/notifications copy uses **Communication** (not Messages) so it matches the rail.

#### Person avatars (2026-08-28)
- **One avatar convention for external people** (`PersonAvatar` in `components/ui/PersonAvatar.tsx`): a known name renders colored initials (first + last word, color seeded by address), an email-only contact renders initials from the local part (`john.doe@…` → JD, via `getEmailInitials` in `lib/avatar.ts`), and an unknown person (website visitor, placeholder address like `visitor@web` / `cust_…`) renders a muted circle with a question mark — never a domain favicon or globe. Used in the thread list, message timeline, contact panel, Contacts page and Cockpit recent contacts.
- **Domain favicons are for companies only** (`DomainFavicon`, `host` prop on company cards). Google S2 serves a generic 16px globe with HTTP 200 for unknown domains; `DomainFavicon` detects that via `naturalWidth <= 16` on load and falls back to initials.
- **Channel glyphs are SVG icons** (`ChannelGlyph`): email renders the lucide Mail icon and widget/webchat renders MessageCircleMore. Only true third-party brands (Slack, WhatsApp) show a logo image via `CHANNEL_BRAND`; the smtp and bokito brand logos are no longer used as channel icons.

#### Contact context panel + data router (2026-08-28)
- **Contact context panel is open by default** in Communication and agent chats; closing it (X or the header PanelRight toggle) lasts only for the current browser session (`sessionStorage` key `inbox.contactPanel.open`), so it returns on the next visit.
- **SplitRow only accepts direct `SplitPane` children** — anything else (including a wrapping fragment) is silently dropped. The Communication context pane must be a direct conditional child; the mobile slide-over lives outside the `SplitRow`.
- **The dashboard runs on a data router**: `main.tsx` uses `createBrowserRouter` with a single `path: '*'` route wrapping the provider tree + `App` (which keeps its descendant `<Routes>`). This is required for `useBlocker` in `useUnsavedChangesGuard` (contact notes, workspace/messenger/project settings forms); a plain `BrowserRouter` makes those pages throw.

#### Inbox operator walk (2026-08-28, filter-and-daily polish)
- **Chip race:** clicking All must not snap back to Needs decision. URL sync only reads `?filter=` (it no longer writes the last chip back when state updates first). A stale list fetch is ignored; the list shows a skeleton until the matching page is ready.
- **Esc vs menus:** Escape closes an open menu first. It does not leave the conversation while a menu or dialog is open.
- **Saved searches** prompt for a short name (`suggestSavedSearchName` strips `from:` / `to:` and shortens).
- **New email** has the same **Templates** library as the thread composer.
- **Mailbox last sync** sits next to Channel settings on the list (`Mail 8m ago`).
- **Contact notes** use the shared unsaved-changes guard on in-app navigation, not only `beforeunload`.
- **Cockpit Needs attention** can snooze a row until tomorrow 9:00 without opening the thread.

#### Inbox operator walk (2026-08-28, daily-workflow pass)
- **Needs decision in the customer inbox:** Open/Mine/All can filter `?filter=needsDecision` (aliases `needs_decision`, `awaiting_decision`). Rows show a **Needs decision** badge. Cockpit empty “awaiting decision” opens that filter, not Agent-runs.
- **Defer parks the thread:** Decision Later / Defer resolves the card and snoozes the conversation until tomorrow 9:00 so it leaves Open. Bulk **Snooze until tomorrow 9:00** and the list indicator menu do the same (`POST /api/signals/bulk` action `snooze`).
- **Agenda clicks open customer threads:** Agenda search no longer forces `folder=internal`. A matching customer thread opens in Communication; internal matches still go to Agent-runs.
- **Auto-open does not mark read:** Opening `/communication/inbox/open` still selects the first thread for display, but skip-mark-read until the operator clicks it.
- **Contact panel:** Shows approved / pending / blocked, Approve/Unblock, company link to `/contacts/companies/{id}` when `companyId` exists, follow-up presets (1 hour / tomorrow / next Monday), and an unsaved-notes warning with `beforeunload`.
- **Create task** is always in the thread ⋮ menu on customer conversations. Closed/spam composer **Reopen** is a filled button. Email Send shows **Ctrl+Enter**.
- **Search debounce:** the search box stays instant; list fetches use `listSearch` (300ms). Clearing search fetches immediately.
- **Remembered inbox:** `bokito.inbox.lastQueue` and `bokito.inbox.quickFilter` persist. `/communication` and `/` resume the last queue. Compact density is `bokito.inbox.density`.
- **Copy / undo:** conversation link is canonical (`threadHubPath` + origin). More menu also copies the thread id. Close and spam toasts can undo (reopen). Activity **Clear list** keeps a snapshot for undo. Audit activity links use `threadHubPath` (Open, not All).
- **Bulk:** Assign to me, Pin, Unpin, and Snooze until tomorrow sit on the selection bar. Command palette adds Closed, Spam, Agent runs, Assistant, Needs reply, Needs decision, and searches numeric/UUID ids from the first character.
- **Govern / Setup / Agents:** Govern "Open in Communication" uses Open (the hub redirects internal threads to Agent runs). Setup Communication pillar has **AI reply settings**. Agent history empty-state Communication CTA opens Agent runs.

#### Inbox operator walk (2026-08, inspectability pass)
- **Range select and unread nav:** Shift-click a list checkbox selects the inclusive range from the last anchor (`toggleOrRangeSelect`). Cmd/Ctrl+A selects loaded rows when the list is focused (not the composer). **]** / **[** and Shift+J/K jump only unread threads. Shift+U marks the current thread read. **Mark loaded as read** bulk-reads the current list page, not the whole server queue.
- **Shareable chips:** `?filter=needsReply|needsDecision|unread|pinned` (aliases `needs_reply`, `needs_decision`, `awaiting_decision`) syncs with the quick-filter chips. Search and label filters persist when switching queues. List scroll position is restored per leaf in `sessionStorage`. The timeline no longer snaps to the bottom on every new message; scrolled-up readers see an **N new** jump pill.
- **Inspect IDs:** message bubbles copy message ID and plain text via hover-only icon buttons; live decision cards copy the decision ID via an icon (resolved one-liners dropped copy-ID); the thread menu copies `graphConversationId` when set. Activity rows copy run / thread / agent IDs and show an absolute timestamp tooltip. Event pills expose payload JSON on hover. Govern audit rows expand to JSON + copy. Agent traces have **Copy trace**.
- **Composer drafts** persist as JSON `{body,cc,bcc}` (legacy plain text still restores as the body). A **Draft restored** chip can discard. Scheduled outbound bubbles have persistent **Cancel send**. Failed sends offer **Copy text**.
- **Extra filters:** assignee and channel go to `GET /api/signals`; priority is client-side on the loaded page. Partial chip counts show a “loaded only” tooltip. Infinite scroll loads the next page via an IntersectionObserver sentinel.
- **Palette:** thread search returns 12 hits plus **View all matches in Communication**. A UUID/numeric query offers **Open conversation by ID**; two IDs open an agent run. Saved searches live in `bokito.inbox.savedSearches`.
- **Chrome:** ConnectionStatus is i18n. Cockpit attention rows show a channel glyph and the thread id in the title. Contact previous-conversation rows use the channel glyph.

#### Inbox operator walk (2026-08, workforce/setup pass)
- **Agents:** Members can read an agent page (no redirect to inbox). Library search is always on; pills All / Working / Paused / Lead filter the grid. New agent defaults to Communication, shows role hints, templates, optional project link, and a model retry. Detail keeps Chat / Agenda / threads / Pause / Lead primary; Knowledge, Inbox AI, Setup, Duplicate and Archive sit in **More actions**. Archive is an in-page confirm that offers Pause first. Autonomy has a one-line consequence plus a Govern posture link. Tools use humanized labels. Model cost shows Low / Medium / High (cents stay in the tooltip).
- **Projects:** Members can browse projects read-only. Create is name-first (slug under Advanced). Cards hide the slug. After create, the operator lands on the project. Repo without GitHub points to Integrations. Lead can be changed. Remaining budget shows token units and a cap tooltip. Empty flows offer **Add the first step on Agenda**.
- **Knowledge:** New docs take a title and generate `docs/{slug}.md`. Search has a button plus 300ms debounce. Empty state is a PDF/Word drop zone. Kind groups have one-line intros. AI-maintained docs show a banner. Publish asks before going public.
- **Contacts:** Create has a channel picker. Pending lists have Approve all / Block all. Row actions stay visible. Dirty contact/company forms use a router blocker. Empty companies auto-run company backfill once per session.
- **Agenda / Setup / Govern:** Empty week days offer Schedule. Automations empty state has Create automation. `kind` persists in the URL. Setup has a sixth Projects pillar and an Intelligence action **Organize work by project**. Govern remembers `bokito.govern.lastTab` and defaults to Policy when there is no last tab and no pending drafts. Posture cards list what changes. Members show a role matrix. Inbox AI save sticks when dirty. Widget copy offers Check installation (preview). Cockpit empty metrics link to Setup.

#### Auth/Knowledge operator walk (2026-08, less-techy pass)
- **Sign-in / verify / reset / invite:** Login remembers the last email on this device and can switch EN/NL. Verify-email pending shows a spinner, prefills `?email=`, and can copy the local dev link. The signed-in verify banner allows another resend after 60 seconds. A reset link without a token shows an error card instead of bouncing to login; after a successful reset the page counts down and **Sign in now** skips the wait. Accept-invite keeps a full-width **Back to sign in** and submits the welcome name on Enter.
- **New conversation / direct chat:** The last recipient is remembered. `?agent=` preselects an agent and `?to=` opens compose. Load failures offer **Try again**. When more than eight people match, **Open Contacts** is shown. Direct chat no longer swallows target-load errors; empty and error states explain the next step.
- **Knowledge / Projects / Branding:** Knowledge warns on unsaved edits, saves with Ctrl/Cmd+S, copies the document path, shows last refresh, and filters by kind. Projects search can be cleared, create submits on Enter, and `?new=1` (command palette **New project**) opens the dialog. Project detail guards About edits, formats token numbers, and refreshes with a timestamp. Branding Save stays disabled until something changed; the workspace URL can be copied and public help previewed.
- **Help / widget:** Help centers create a collection on Enter, confirm collection delete, search documents, and show last refresh. Public help can clear search, copy an article URL, and explains an empty index. Widget appearance Save also tracks Voice fields and availability settings; Ctrl/Cmd+S saves.

#### Inbox density cleanup (2026-08)
- **Composer hints live on hover:** the Enter / Ctrl+Enter send hints sit in the Send button tooltip; Reply/Note tab reminders sit on the tab tooltips; the signature note is a tooltip on the recipient row. No always-on hint sentences under the composer (ReplyComposer, DirectChatPanel, NewConversationPage).
- **Split Send button:** the customer-reply Send button has a chevron with **Send and close** and **Send and snooze** (snooze presets in a submenu). Notes keep a single button. The NEXT label before the **Create task** chip is gone; the chip stands alone.
- **Quiet thread list:** the shortcut footer is removed and **All N conversations loaded** is a tooltip on the list.
- **Quiet timeline:** copy message ID / copy text are icon buttons that appear on message hover (`group/msg`); failed sends keep a visible **Copy text**. `decision_created` / `suggestion_created` events are filtered out of the timeline because the cards already represent them. Resolved decision one-liners have no copy-ID; active cards use an icon-only copy-ID. **Correct this** on decision cards is an icon with a tooltip, matching the message feedback controls.

#### Cockpit/profile operator walk (2026-08, less-techy pass)
- **Cockpit Overview:** Subtitle greets by first name and shows today's date. A display name that starts with the product or organisation name (`Bokito Admin`) is skipped, so the greeting is just **Good morning** / **Goedemorgen**. Refresh shows last successful load time. **Agent freedom** shows **You decide** / **Ask first** / **Act first**. Empty AI-usage hides `US$ 0.00`. Placeholder website-visitor rows stay off Recent contacts. **Your numbers** empty state is a compact row. Numbers use the in-app locale. Empty time-saved and handled-without-you cards explain when values appear. Customer-rating empty state opens website widget install. Partial-load failures name agenda / contacts / inbox in plain language. **Needs attention** offers **Open first**. A dismissed setup card can be restored from the subtitle.
- **Activity:** Source, detail, search, and jump-to-newest persist in the URL. The log groups **Today** / **Yesterday** / **Earlier**. Refresh shows last load time.
- **Notifications:** Pause all in-app alerts, restore recommended defaults, and preview a sample toast. Slack column header links to connect Slack. Fetch uses `APP_API_BASE`. The bell menu groups Today / Earlier, can mark one group read, and opens Communication when items exist.
- **Profile / My assistant:** Edit pencils stay visible. Avatar shows file-type hint. Password Enter submits; errors are i18n. Language notes it stays on this device. Assistant has templates, character count, unsaved guard, sticky save, Ctrl/Cmd+S, and a Talk CTA.
- **Members / palette / docs:** Invite validates email and submits on Enter. Promoting to owner confirms. Command palette adds invite, profile, notifications, talk-to-assistant, and My assistant. Learn and public Docs remember the last article. Public help search stays in `?q=`. Setup hide can be undone. Live connection says **Live updates paused** with Reload.

#### Settings operator walk (2026-08, more-techy pass)
- **Providers & models:** Delete provider and Enable presets ask for confirmation. Provider types are humanized. API key can be shown/hidden; Enter saves the add-provider form. Test results are green/red. Tenant models can be filtered; copy model id; chat models show Low/Medium/High cost. Managed usage links to `/cockpit/usage`.
- **Developers:** New tokens warn when scopes are empty (full access), offer All API / All tools, a curl example for `GET /api/public/v1/signals`, and a dismiss after copy. Revoked tokens are hidden unless shown. Last-used is relative. Webhook URLs must be https (localhost http allowed). Copying a signing secret toasts.
- **Channels / Help center:** Channel routes can target one account, set priority, and pause without delete. Help center collections persist in `?collection=`, can be deleted, validate file URLs, and expose **Copy public link** (`/help/{slug}`).
- **Integrations / Usage / forms:** Marketplace `q` and `status` stay in the URL. Connected remembers last kind and can search. MCP table filters, copies endpoints, and confirms disconnect. Usage breakdowns accept `?days=7|30|90` and export CSV; clearing both caps confirms. Workspace, Branding, widget Look, and Inbox AI use the unsaved-changes guard.

#### Inbox operator walk (2026-08, techier pass)
- **List preview fidelity:** `GET /api/signals` already returned `last_message_preview` + `last_message_direction`. The Communication list goes through `listSignalThreads` → `normalizeSignalThread` in `signals-api.ts` (not `inbox-api` mapping). That normalizer must keep both fields or rows fall back to the subject. `_latest_message_previews` returns `(snippet, direction)` and skips placeholder / `[mock]` agent lines.
- **Needs reply:** client filter `threadNeedsReply` = open + (unread or last inbound). Chip **Needs reply** / **Jij aan zet** sits next to Unread/Pinned. Outbound previews prefix **You:** / **Jij:**.
- **Keyboard (Communication inbox):** `/` search, `J`/`K` next/prev, `E` close, `U` unread, `A` assign to me, `P` pin, `R` focus `#inbox-reply-composer`, `C` compose, `?` shortcut overlay, `Esc` clears/blurs search. Shortcuts are skipped while the compose modal is open or while typing in an input.
- **Assign to me:** `/auth/me` top-level `id` is already `user_numeric_id`. Nested `user` in the payload used to be a UUID string; it now also includes `numeric_id`. Dashboard `normalizeAuthUser` prefers `numeric_id` so Mine / assign / “your message” compare against the same numeric id as `GET /api/signals/members`. `AssigneeSelector` also matches the current user by email as a fallback.
- **Parked threads drop leftover drafts:** `PATCH` to `pending`/`closed`/`spam` defers open reply-suggestion decisions (`human_snoozed` / `human_closed` / `human_spam`), same as a human reply (`human_replied`).
- **Contacts:** profile header **Open conversation** jumps to the latest thread (primary CTA for widget visitors who cannot write email).
- **Browser tab:** AppShell sets `document.title` to the page name and prefixes `(N)` with `inboxUnread`.
- **New email compose:** From is always visible; Ctrl/Cmd+Enter sends; focus lands on the body when To is prefilled.

#### Inbox operator-tooling (2026-08, iteratie 1 completeness sprint)
- **Labels & prioriteit**: onder de thread-header staat een compacte meta-rij (`ThreadMetaRow` in `ThreadDetail.tsx`) met een prioriteit-chip (Normal/High/Urgent, dropdown met kleurstip) en labelchips met verwijder-x plus een "+ Label" inline-input. Alles gaat via `PATCH /api/signals/{id}` (`tags`, `priority`). Alleen zichtbaar op externe (klant)threads, niet op interne agent-threads.
- **Labelfilter**: klikken op een labelchip in de threadlijst zet een server-side filter (`GET /api/signals?tag=`). Boven de lijst verschijnt een filter-pill met x om te wissen; het filter reset bij het wisselen van hub-leaf.
- **Notities bewerken/verwijderen**: interne notities (`kind="internal_note"`) tonen potlood/prullenbak-iconen in de header van de note-bubble. Bewerken gebeurt inline (textarea + Save/Cancel) via `PATCH /api/signals/{id}/notes/{message_id}`; verwijderen via DELETE met een confirm. Backend `update_note` synchroniseert ook `body_html` (notities worden met een `<p>`-mirror opgeslagen; zonder die sync bleef de timeline de oude HTML tonen).
- **Mark as unread vanuit thread detail**: envelop-icoon in de headertoolbar zet de open thread terug op ongelezen (`useThreadDetail.markUnread`), met toast en badge-refresh.
- **Bulkacties uitgebreid**: de bulkbar heeft naast Read/Close/Assign een "..."-menu met **Mark unread**, **Reopen** en **Mark as spam** (backend `POST /api/signals/bulk` ondersteunde deze acties al).
- **Sync status-paneel** (historisch, verwijderd aug 2026): `SyncStatusPanel` toonde per mailbox status, laatste sync, fouten en per-map berichtaantallen. Vervangen door de checks op de kanaalrij (zie kanaalregister).
- **Mailbox-tabel (`/settings/channels`, 2026-08)** (historisch): Connected inboxes was een eigen tabel met kolommen Inbox/Sync/History/Status/Actions. Sinds het kanaalregister is dit één kanalenlijst met alle kanaaltypen als gelijke rijen.
- **Opgeruimd**: dode componenten `AutonomousProposalCard`, `RunStatusIndicator`, `AgentActivityLog` en `lib/messages-api.ts` zijn verwijderd (nergens geïmporteerd; het canonieke decision-pad is `DecisionRequestMessage` + `POST /signals/.../resolve`).

#### UI-polish en feedback (2026-08, iteratie 2 completeness sprint)
- **Humanized labels**: `lib/labels.ts` levert `humanizeLabel()` (snake_case → sentence case met overrides zoals `needs_auth` → "Needs sign-in") en `memberRoleLabel()`. Toegepast op alle plekken die rauwe API-enums toonden: SyncStatusPanel, WorkLogsTable, ContactsPage-statusbadges, AgendaPage-occurrencestatussen, Cockpit/Activity event kinds, MemberManagement/AcceptInvite/NewConversationPage-rollen, ProjectsSettings (`repo_index_status`, `autonomous_scope`), HelpCentersSettings (`index_status`), AgentContextPanel (agentstatus, threadstatussen).
- **AgentContextPanel**: "Open in Activity" heet nu "Open run" (de link ging altijd al naar runs); recente-conversatielinks gebruiken `inboxPath`/`agentRunsPath` i.p.v. legacy `/communication/agents|customers`-paden; en als een van de parallelle context-fetches faalt verschijnt een warning-banner met Retry i.p.v. stilzwijgend lege secties.
- **Onboarding company-stap**: de CTA linkt naar `/knowledge` (voltooiing wordt gemeten aan `company.md`-inhoud, niet aan branding); copy heet nu "Open company doc" / "Open bedrijfsdocument".
- **Personal agents**: `AgentChatAccessCard` wordt verborgen op personal-agentdetail (de chat-access API is company-only en gaf 404). `PATCH /workforce/agents/{id}/model` staat nu ook toe dat een gebruiker het model van de **eigen personal assistant** wijzigt (My Assistant-pagina); company agents blijven owner/admin-only.
- **Notificatie-instellingen**: de tabel toont naast In-app ook een **Email**-kolom (de backend ondersteunde e-mailnotificaties per categorie al via `notification_mail.py`); de ontbrekende "decisions"-rij is toegevoegd (werd voorheen door het frontend-filter weggegooid) en een korte "Saved."-bevestiging verschijnt na opslaan.
- **Members**: de Date-kolom toont de join-datum (backend `list_members` levert nu `joined_at` uit `Membership.created_at`); invite versturen geeft een success-toast.
- **Stille fouten opgelost**: profielveld-edits tonen een error-toast en blijven in editmodus bij een mislukte save; avatar-uploadfouten tonen een toast; taalvoorkeur-save meldt als de PATCH faalt; Knowledge-doc laden en zoeken tonen een toast bij fouten; contact aanmaken toont een success-toast.

#### Realtime inbox append (2026-08)
- Gateway `message`/`thread` events dragen de canonieke REST-payloads (`serialize_thread` / `serialize_message` uit `signal_threads.py`), gesplitst per topic: `threads` krijgt de volledige thread-row + een lichte message-preview (geen bodies), `signal:{id}` krijgt het volledige bericht (html, attachments, decision-payload). `publish_signal_message` accepteert optioneel `decision=` zodat decision-cards zonder refetch renderen.
- Elke envelope heeft `data.audience: "operator" | "all"`; `GatewayConnection.matches` laat operator-frames nooit bij widget-verbindingen aankomen. Interne notities, system events, `threads`-feed en thread-triage-updates zijn operator-only; visitor-zichtbare berichten blijven `all` (de widget leest `data.message.body_text` ongewijzigd).
- Dashboard: `useThreads` upsert de meegeleverde row direct in de lijst (`lib/thread-live.ts`: `threadMatchesFilters` spiegelt de server-view-predicaten; niet client-evalueerbare filters — search, awaiting_decision/pinned/updates/results/outbound, assistant-folder — vallen terug op de gedebouncede refetch). `useThreadDetail` appendt het volledige bericht met id-dedupe en markeert de open thread als gelezen; decision-/thread-events blijven quiet-refetchen. Een gateway-reconnect triggert één verzoenende `fetchThreads()`; de 90s-poll blijft het vangnet.
- Frontend unit tests draaien via vitest (`npm test` in `apps/dashboard`, nieuw); backend payload/audience-tests staan in `tests/test_gateway.py`.

#### Zelflerende inbox-regels (2026-08)
- **Concept**: het platform leert per afzender wat er met automatische mail moet gebeuren uit herhaalde operatorkeuzes op de "No reply needed"-kaart. Model: `InboxRule` (`models/learning.py`, tabel `inbox_rules`) met `match_type` (`sender` | `domain` | `list_id`), genormaliseerde `match_value`, `action` (`auto_close` | `auto_task` | `mute_ai`), `status` (`suggested` | `active` | `paused`), `source` (`learned` | `manual`), plus tellers `observations` (leerteller) en `hit_count`/`last_hit_at` (toepassingen). Service: `services/inbox_rules.py`.
- **Leren**: bij het approven van optie `close` of `create_task` op een action-suggestion-decision registreert `resolve_message_decision` (`signal_threads.py`) een `Feedback`-rij (`comment="no_reply_action:<optie>"`) en telt `inbox_rules.record_outcome` de keuze op een `suggested` regel voor de afzender. Drempel: 3 consistente keuzes (`PROMOTION_THRESHOLD`). Een afwijkende keuze reset de teller naar de nieuwe actie; `keep_open` leert niets; gepauzeerde regels worden niet opnieuw gesuggereerd. De resolve-response bevat `rule_suggestion` (serialized rule + `ready_to_activate` + `auto_promoted`).
- **Bevestigen**: bij `ready_to_activate` toont `DecisionRequestMessage` inline een "Always do this"-prompt (i18n `decisionCard.rulePrompt.*`); bevestigen doet `PATCH /api/signals/rules/{id} {status:"active"}`. Onder autonomy posture `autonomous` promoveert de regel zichzelf bij de drempel (toast meldt dit); `manual`/`assisted` vragen altijd bevestiging.
- **Toepassen**: `process_inbound_signal` (workers/tasks.py) checkt vóór de automated-mail-classificatie en vóór elke agent-run `inbox_rules.find_matching_rule` (prioriteit: sender > list_id > domain, alleen `active`). Match → `apply_rule_to_signal`: `auto_close` sluit de thread (unread weg, gateway thread-update), `auto_task` maakt een `AgentTask` (`trigger_type="inbox_rule"`, auto_start uit, thread blijft open — pariteit met de handmatige keuze), `mute_ai` slaat AI over en laat de thread ongelezen voor het team. Elke toepassing schrijft een `rule_applied` SignalEvent (timeline-pill "Auto-closed by rule (afzender)" e.d., AI-accentstijl) + `AuditEvent` (`inbox_rule:apply:<actie>`) en verhoogt `hit_count`.
- **Beheer**: CRUD op `/api/signals/rules` (list/create/patch/delete, routes in `app.routes.ts`); Inbox Settings heeft een "Automation rules"-kaart (`AutomationRulesManager.tsx`) met status (Active / Paused / Learning n/3), hit-teller, pauzeren/activeren, verwijderen en handmatig aanmaken (sender/domein/list-id). Handmatige regels zijn direct actief.
- **Hygiëne (2026-08)**: mailbox-sync stempelt `ai_live_since` in `settings_json` bij de eerste sync (`ensure_ai_live_since`); backfill-mail ouder dan die cutoff (`is_backfill_message`, 5 min grace) wordt wel opgeslagen maar triggert géén agent-run. Geautomatiseerde afzenders (no-reply/nieuwsbrief/bounce via `classify_automated_email`) krijgen geen CRM-`Contact`-rij meer bij ingest (`channels/base.py`); bestaande contacten blijven matchen zodat blokkeren blijft werken.
- Tests: `apps/api/tests/test_inbox_rules.py` (matching, leren/promotie, CRUD, pipeline-acties, backfill, contact-onderdrukking) en `apps/dashboard/src/lib/inbox-rules.test.ts` (normalizers).
...
- **Aanpasbare secties** (`ALL_SECTIONS` = `agents | channels | settings`; volgorde/verbergen/inklappen in localStorage `communication-sidebar-prefs`; legacy `assistant` in opgeslagen prefs wordt genegeerd bij normalize):
  - **Agents** — company agents (`/communication/agent/:agentId`), plus één **Activity**-leaf (`/communication/runs/all`). Updates / Results / Decisions zijn quick-filter chips op de Activity-lijstpagina (oude URLs `/communication/runs/:queue` blijven werken en preselecteren de chip). Geen persoonlijke-assistent-pin meer.
...
#### Communication-hub + company agent chat (juni 2026; personal assistants removed aug 2026)
The Messages hub was renamed to **Communication**. **Personal assistants (`kind=personal`) are removed:** startup deactivates legacy personal rows; no provisioning on signup/invite; no `/settings/assistant` or `GET/PATCH /api/me/assistant`. `GET /api/chat/targets` lists permitted company agents only (`default_agent_id` may be null). `POST /api/chat/conversations` requires `agent_id` (400 "Choose which agent to talk to"); 409 "No agents available for user" when the member has no chat-permitted company agents. New chat UI shows the same empty state. Bring-in-agent candidates are channel → project → company only.

- **Agent kinds (current):** chat targets are company agents the user may message. Legacy `kind=personal` rows may still appear in history display filters; workforce nav continues to hide them.
- **Company agent chat access:** `Agent.chat_access` = `everyone` | `selected` | `nobody`; allowlist in `agent_chat_users`. Manage on agent detail → `GET/PATCH /api/workforce/agents/:id/chat-access`.
- **Chat targeting:** `GET /api/chat/targets` returns permitted **company** agents only; `default_agent_id` may be null. `POST /api/chat/conversations` requires choosing a company agent; with none available the API returns 409 "No agents available for user". Optional user default: `/api/me/chat-default`.
- **New conversation:** `/communication/new` is composer-first with a To picker (company agents + people/email). User must choose an agent; `default_agent_id` may preselect when present in the list. Empty targets show **No agents available** / **Geen agents beschikbaar**. Send navigates to `/communication/agent/:agentId/t/:id`.
- **Hub nav:** Agents section lists company agents only (no personal pin). Legacy `/communication/assistant*` and `/settings/assistant` redirect to `/communication/new`.
...
- **Attachments / file hosting (aug 2026):** `app/services/storage.py` has `local` and `s3` (Cloudflare R2-compatible). Canonical attachment URLs always stay on auth-gated `GET /api/uploads/files/{tenant_id}/{filename}` — the bucket is private; API and worker `fetch()` from the same backend. Serve accepts Bearer access tokens, widget `session_token` (query or Bearer), or the refresh cookie. Widget image tags append `?session_token=` for cross-origin loads. Caps: 10MB (`POST /api/uploads`, livechat images-only, knowledge originals). Email inbound/sync also uses this store. Avatars stay as base64 on `User.avatar_url`. `boto3` is a main API dependency. Deploy upserts `STORAGE_S3_*` from GitHub secrets into `.env.staging`/`.env.prod`. Local fallback: Docker volume `bokito_uploads` mounted on both api and worker at `/app/data/uploads` so deploys no longer wipe files and the worker can see API writes. R2 (EU jurisdiction bucket `bokito-uploads`) is the production target; the existing Cloudflare DNS token cannot create R2 resources — enable with `STORAGE_BACKEND=s3` plus `STORAGE_S3_BUCKET/ENDPOINT/ACCESS_KEY/SECRET_KEY`.
- **Email:** outbound ondersteunt HTML body, CC/BCC, handtekening uit account settings, en `In-Reply-To`/`References` threading via laatste inbound `external_id`. Inbound sync bewaart `body_html` en attachment-metadata (Gmail MIME + Graph). **Signed body in timeline (aug 2026):** `deliver_outbound` / `send_via_provider` return the final HTML after signature append (`OutboundDelivery.body_html`); all store paths (`reply_to_thread`, compose `/email/send`, `send_reply` tool, auto-reply, scheduled flush) persist that HTML on `SignalMessage.body_html` so the thread bubble matches what was sent. Older messages stored only the unsigned draft remain without signature until resent.
- **Reply identity + clean drafts (aug 2026):** voorgestelde antwoorden zijn puur klantgericht. `services/suggestion_format.split_suggestion()` splitst rauwe LLM-output in draft-body en interne notitie: `INTERNAL_NOTE:`-sentinel, legacy `**Interne notitie:**`-blokken, research-preambles vóór de eerste groet (met of zonder `---`), en model-geschreven afsluitingen worden gestript — het handtekeningsysteem voegt server-side precies één handtekening toe. De interne notitie leeft alleen op de send-option payload (`internal_note`); de decision card toont hem volledig **onder** draft+handtekening (muted linkerlijn, geen geel, niet ingeklapt). Geen tweede timeline-`SignalMessage` meer voor suggestion notes. Timeline/composer notes gebruiken dezelfde muted left-rule chrome (geen gele wolken); `TimelineItem` toont `bodyText` vóór `bodyPreview` zodat notities niet afkappen. **Send-as + handtekening:** in dezelfde elevated draft-kaart, direct onder de body (niet onder de notitie); selector Jij/Agent + preview. Zonder custom HTML composeert `signatures.compose_default_signature_html` dynamisch uit naam/email/functie/bedrijf + workspace-taal (niet gepersisteerd); UI toont **Handtekening instellen** → `/settings/profile` of `/agents/{id}`. Keten: user custom → agent custom → dynamische default → mailbox fallback. Keuze onthouden via `reply-send-as.ts`; tenant-default via `reply_send_as` in AI modes (`/settings/communication`). De violet **Wacht op beslissing**-snelkoppeling onder Nieuwe chat is verwijderd — operators gebruiken Agents → Agent-runs / Cockpit.
...
- **CRM:** `Contact`-model heeft `company`, `title`, `phone`, `notes`. Contactpaneel in thread-context als tab naast Orchestration (`AgentThreadPanel`): contactkaart, bewerkbare notities, approve/block, eerdere conversaties — geen projectkoppeling of snooze-presets (die blijven in thread-toolbar / agent-context). Contacts-pagina op `/contacts` (zoeken, lijst met kanaal/bedrijf/laatst gezien/#threads) en contactdetail op `/contacts/:contactId`. De lijst vult `last_seen` en thread counts vanuit Communication wanneer de CRM-velden leeg zijn (`enrichContactsFromThreads`: e-mail/id, unieke echte namen — geen generieke websitebezoekers). Avatars gebruiken het domein-favicon (`DomainFavicon` via Google S2, initials als fallback) op de contactenlijst, company-lijst, contactdetail, ContactPanel en threadlijst; de kanaalkolom toont een Lucide-icoon (`ChannelLabel`: e-mail, widget, Slack, WhatsApp, intern). Endpoints: `GET/PATCH /api/channels/contacts/:id`, `GET /api/channels/contacts/:id/threads`; signal thread-payload bevat `contact_id`.
- **Agenda = alleen planning (2026-08):** `GET /api/agenda` toont uitsluitend geplande trigger-expansies, afgeronde one-shots (`once`/`event` via `last_run_at`) en run-geschiedenis van echte triggers (`AgentRun.trigger_id` moet naar een `Trigger` wijzen). On-demand runs (e-mailsuggesties, chat, widget — `trigger_id` wijst naar een Signal) horen op de activity-tijdlijn, nooit op de agenda. `AgentRun`-records worden overal gefinaliseerd: de inbound-worker (`handle_inbound_signal`) en chat-endpoints zetten `completed`/`failed` + `completed_at` + tokens; de startup-data-repair `_close_stale_agent_runs` sluit runs die >6 uur op `running` staan (agenda/cockpit/workforce lezen die status). Cockpit Activity (`/cockpit/activity`) heeft een horizontale tijdlijnstrip (`TimelineStrip.tsx`): een lijn met dots per minuut, uur-ticks, dagmarkering, accent "Now"-marker; uitgevoerd werk links (accent-dots, rood bij failed, groen live), geplande agenda-items komende 24u rechts als holle accent-dots. Naar links scrollen laadt oudere geschiedenis via `GET /api/cockpit/activity?before=` (cursor-paging, events krijgen nu een `id`).
...
- **Project workstreams (backend):** Er is één workstream-model: de runnable orchestration-`Workstream` (`workstreams` tabel) heeft een nullable `project_id` (migratie `011_unify_workstreams`; het oude list-only `project_workstreams` model is verwijderd). API: `GET/POST/PATCH /workforce/projects/:id/workstreams` (velden `name`, `description`, `enabled`, `steps_count`); runnen kan via `POST /orchestration/workstreams/:id/run`. Project-detail toont workstreams met run-knop en inline aanmaken; stappen beheer je op de Agenda-pagina. Bij projectverwijdering worden workstreams ontkoppeld (project_id → null), niet verwijderd. Response bevat ook gekoppelde `po_agent` (agent entity met `role=orchestrator`, via `projects.po_agent_id`). De dormante `ProjectOrchestration` (wake-cadence) en `ProjectNotificationPreference` modellen/endpoints zijn verwijderd (migratie `012_drop_dormant_project_tables`).
...
- Chat-widget accentkleur: tenant `appearance.main_color` (preview: `data-preview-overrides`) schrijft `--bk-brand` / `--bk-primary` op de host. Light/dark theme-CSS zet geen `--bk-primary` meer (dat overschreef eerder de gekozen accentkleur met neon-groen `#00FF99`). Surfaces (bg/text/border) wisselen per thema; header-atmosfeer, user-bubbles, invoer-focus en send-knop volgen de brand via `color-mix`. `--bk-on-primary` is contrast-afhankelijk (donkere tekst op lichte accents zoals `#00FF99`, witte tekst op donkere accents zoals `#415E78`). De “online”-statusdot blijft semantisch groen (`#4ADE80`).
- Chat-widget iconen: UI-chrome (header, tabs, composer, account) gebruikt een licht Lucide-achtig stroke-set (24px, stroke 1.5, `currentColor`) in `apps/chat-widget/src/icons.ts`. De launcher houdt de geanimeerde mascotte; de header-avatar is een statische `currentColor`-markering zonder morph-keyframes.
...
- Meertaligheid: de dashboardshell ondersteunt runtime taalwisseling met `i18next` (`en`/`nl`). Platformstandaard is Nederlands via `PLATFORM_DEFAULT_LANGUAGE` (API, runtime) en `VITE_PLATFORM_DEFAULT_LANGUAGE` (dashboard, bake-at-build). Ongeldige/lege waarden vallen terug op `nl`. Persistente keuze: `localStorage` key `bokito-language` plus persoonlijke `GET/PATCH /api/me/preferences` `ui_language`. Operators wisselen taal in **Profiel** en in het gebruikersmenu (avatar). `applyUiLanguageLocally` zet `i18n`, `document.documentElement.lang` en localStorage in één stap voordat de server-persist loopt, zodat de UI niet flikkert. Onboarding (`/signup`, `/login`, `/accept-invite`, wachtwoord/verify) volgt `?lang=nl` of `?lang=en`; ontbrekende of lege parameter gebruikt de platformstandaard.
...
**Design tokens (canonical, na Unified Premium Styling Pass aug 2026)**
- Achtergronden volgen een vier-laags hiërarchie: `bg` (root) → `bg-sidebar` → `bg-surface` → `bg-elevated`. Hover-achtergronden zijn `bg-bg-hover`. Inputs gebruiken `bg-bg-input` of `bg-bg-elevated`.
- **Kaartrecept (het enige)**: `bg-bg-surface` (solid) + `border border-border/60` + `rounded-xl` + `shadow-card`. In CSS beschikbaar als `.panel`; in React als `Card` (met optionele `interactive`-prop voor hover-lift). Surface-transparantie (`/80`, `/85`) is alleen toegestaan direct naast `backdrop-blur` (sticky headers, overlays, pills).
- **Drie border-niveaus**: `border-border/40` (zachte scheidingslijnen, card-header dividers, tabelrijen), `border-border/60` (kaarten, dropdowns, dialogs — default), `border-border` (inputs/strong). Andere opacities zijn weggesweept.
- **Radius-nestregel**: shell-paneel `rounded-2xl` (16) → kaarten `rounded-xl` (12) → binnenvlakken/inputs `rounded-lg` (8) → chips `rounded-md` (6) → pills/bubbles `rounded-full`/`rounded-2xl`. Kind altijd kleiner dan parent. Dialogs zijn top-level overlays en houden `rounded-2xl`.
- **Elevation-tokens**: `shadow-card`, `shadow-card-hover`, `shadow-overlay` (Tailwind-aliases naar per-theme CSS-variabelen `--shadow-card` etc. in `index.css`). Kaarten krijgen één zachte diffuse schaduw + subtiele inset-highlight; overlays (dialogs, dropdowns, selects, auth-cards) gebruiken `shadow-overlay`.
- **Motionsysteem (CSS-only)**: `animate-page-enter` (fade + 8px rise, 240ms — automatisch op `PageContent` en auth-cards), `.hover-lift` (translateY(-1px) + schaduwverdieping, 150ms — voor klikbare kaarten zoals integrations/workspaces), `animate-pop-in` (dropdowns/selects, 150ms), `animate-dialog-in` (dialogs, behoudt centrering-transform), `animate-fade-in` (overlays/tooltips). Buttons hebben `active:scale-[0.98]`. Alles gedempt achter `prefers-reduced-motion`. Let op: `tailwindcss-animate` is NIET geïnstalleerd — gebruik deze eigen keyframes, geen `animate-in`/`zoom-in-95` classes.
- **Lightmode**: near-white basis `#F7F9FC` (hue 225 behouden), witte kaarten met gelaagde zachte schaduwen i.p.v. grijze fills; borders koeler en lichter (`228 233 242`).
- **AI-tekst standaard**: alle AI-gegenereerde chat-content rendert via [`ChatMarkdown`](apps/dashboard/src/components/inbox/ChatMarkdown.tsx) (hand-rolled, geen dependency: koppen, bold/italic, inline/fenced code, links, ul/ol, tabellen, blockquotes, hr — alleen text nodes, nooit raw HTML). Gebruikt in `DirectChatPanel`, `AgentSessionCard` en de streamende agent-bubbel. **In-app paden** (`/settings/...`, `/learn/...`) renderen als inline nav-pills via React Router `Link` (geen `target=_blank`); externe `https`/`mailto` blijven onderstreepte links. Thread-bodies via [`MessageMarkdown`](apps/dashboard/src/components/inbox/MessageMarkdown.tsx) gebruiken dezelfde pill-styling + SPA-navigate op klik. Agents krijgen via `RESPONSE_STYLE` de instructie om markdown-links met in-app paden te gebruiken i.p.v. breadcrumb-proza ("Inbox > Automation rules"). Voorbeeld deep-link: `/settings/channels#automation-rules` (anchor op `AutomationRulesManager`). Tool `suggest_inbox_rule` levert `confirm_path` + note met dat pad. Streaming wordt gesmootht via `useSmoothStreamText` (rAF-reveal, ~8% backlog per frame, reduced-motion-aware) met een `.stream-caret` knipperende cursor. Think-stappen heten backend-intern "Loop N" maar tonen in de UI altijd "Thinking..." (`stepHeadline` in `lib/agentSteps.ts`). De AgentLoop-systemprompt bevat een vaste "Response style"-sectie (schone markdown, geen emoji, taal van de gebruiker volgen, in-app links).
- **Live agent-status vs bubbel (2026-08):** terwijl een agent werkt toont Communication geen thinking-wolk. [`AgentLiveStatus`](apps/dashboard/src/components/inbox/ThinkingTrace.tsx) stapelt losse paarse regels (`text-ai-ink`, `.thinking-shimmer-text`, `.agent-live-dot` + glow op de huidige regel; eerdere stappen dimmen). Een bubbel verschijnt pas bij streamtekst, eindantwoord of DecisionRequest. Na de beurt verdwijnt de live-regel; optionele `ReasoningDisclosure` blijft onder de bubbel. De widget gebruikt dezelfde class-namen (`.agent-live-status`, `.agent-live-line`, `.agent-live-dot`). `prefers-reduced-motion` houdt de tekst, stopt animatie.
- **Load-skeletons (2026-08):** pagina-load shimmert in de bestaande layout (`InboxListSkeleton`, `InboxThreadSkeleton`, `InboxSplitSkeleton`, `ChatTranscriptSkeleton`, `CardGridSkeleton`, `TableRowsSkeleton` in [`skeleton.tsx`](apps/dashboard/src/components/ui/skeleton.tsx)). Messages houdt chrome (split, filters, composer-outline). Knop-spinners (Send/Save/Retry) blijven. RouteFallback gebruikt een card-grid skeleton i.p.v. een midden-spinner.
- **No-emoji standaard (backend, aug 2026)**: [`app/services/agent/style.py`](apps/api/app/services/agent/style.py) is de single source of truth — `RESPONSE_STYLE` (volledige prompts, gebruikt door de AgentLoop), `PLAIN_STYLE` (korte gestructureerde outputs: triage in `interpretation.py`, compaction in `assistant_threads.py`) en `strip_emoji()` als vangnet op korte AI-gegenereerde strings (decision-titels/samenvattingen, taak-titels en delegatie-titels in `tools/builtin.py`, triage-samenvattingen). `strip_emoji` draait nooit op klant- of operatorcontent. De frontend-broncode zelf is emoji-vrij; alle iconen gaan via Lucide (workspace-rule `no-emoticons-ui-copy`).
- **AI-identiteit (violet, 2026-08):** alles wat de agent doet deelt `--color-ai` / `--color-ai-ink` (niet de workspace-brand). Primitives in [`components/ai/AiMark.tsx`](apps/dashboard/src/components/ai/AiMark.tsx): `AiMark` (sparkles), `AiChip`, `AiIconBox`, `AI_PILL_CLASS` / `AI_CARD_CLASS` (plus `.ai-surface` glow). Gebruikt op beslissingskaarten, timeline-pills ("AI asked for a decision", "AI reviewed this message" — `.ai-glow`), live agent-status, AI-bubbles, triage-chip, Draft with AI, agent-sessions, assistant-nav. Knowledge gebruikt dezelfde tokens (brein i.p.v. sparkles). Light-mode knoppen gebruiken `.btn-elevated` (`--shadow-btn`) zodat secondary/outline clickable aanvoelen; primaire AI-acties (o.a. Goedkeuren) gebruiken `Button variant="ai"`.
- **Kennis-identiteit (violet brein, aug 2026)**: kennis en auto-learning zijn overal herkenbaar aan een violet Brain-icoon (zelfde `--color-ai` als de AI-identiteit). Primitives in [`components/knowledge/KnowledgeMark.tsx`](apps/dashboard/src/components/knowledge/KnowledgeMark.tsx): `KnowledgeMark` (inline icoon), `KnowledgeTile` (afgerond violet tegeltje, md/lg), `LearnedChip` ("AI-maintained"/"Self-learning" chip, optionele `.knowledge-glow` pulse — reduced-motion-safe). Terugkerend op: de rail (Kennis-tab heeft Brain-icoon + violet actieve staat in `ShellSidebar`), de Kennis-pagina (header-tile, AI-onderhouden kinds `memory`/`heartbeat`/`daily_log` krijgen mini-brains en een "AI-maintained" chip, violet actieve doc-selectie), agent-steps (kennis-tools `search_index`/`list_docs`/`read_doc`/`write_doc` tonen "Consulting/Updating knowledge..." met brein via `isKnowledgeTool` in `lib/agentSteps.ts`; `ReasoningDisclosure`-samenvatting toont een brein wanneer kennis is gebruikt), learned inbox-rules (`AutomationRulesManager` header) en de onboarding-kennisstap.
- **Kennis-pagina (`/knowledge`, aug 2026)**: full-bleed route (in `AppShell.isFullBleed`) met `featurebase-shell-panel` twee-paneel layout. Sidebar: brain-header + zoek + docs per kind met published-dot (emerald). Documentweergave: titel + chips-rij (kind, mono pad, "AI-maintained", "Published"), frontmatter als chips (zonder `published`), leidende `# Titel` gelijk aan de doc-titel wordt niet dubbel gerenderd. Lege staat: brain-hero met auto-learning uitleg. [`MarkdownView`](apps/dashboard/src/components/docs/MarkdownView.tsx) (gedeeld met het publieke helpcenter) ondersteunt nu ook tabellen, `---` dividers, `_em_`/`__strong__` underscores, links, blockquotes en geordende lijsten, op platformtokens (document-typografie; ChatMarkdown blijft de chat-schaal variant).
- CSS-variabelen leven centraal in [`apps/dashboard/src/index.css`](apps/dashboard/src/index.css); Tailwind-aliases in [`apps/dashboard/tailwind.config.ts`](apps/dashboard/tailwind.config.ts) (incl. `bg-root`, `bg-canvas`, `shadow-card`, keyframes).
...
- **Page guide banners (2026-08, product-help 2026-08):** main rail pages plus Channels, Integrations, Models, Widget and Govern show a dismissible intro banner (`PageGuideBanner`). Communication uses the inbox banner or the `runs` variant on Agent runs. "Read the full explanation" opens `/learn/:slug`. Banner copy stays in `nav.json` under `pageGuides`; article bodies live in `docs/product-help/{en,nl}/*.md`. Dismiss is stored in `localStorage` as `bokito-page-guide-dismissed:{slug}`.
...
- Settings-pagina's (`/settings/*`, `/ai/*`) zijn full-bleed in `AppShell`, gelijk aan Communicatie. `SettingsLayout` rendert een tweede rail flush tegen het hoofdmenu (`bg-bg-sidebar/50`, `border-r`), met de paginainhoud ernaast. Op smalle viewports wordt de settings-nav een compacte balk boven de inhoud.
...
- Terminologie (aug 2026): in Nederlandse copy heet een thread altijd **gesprek** (meervoud **gesprekken**) — in de dashboard-locales, de NL product-help-docs en mobile. "Thread" blijft alleen bestaan in code-identifiers, i18n-keys (bv. `threadCreated`) en Engelse copy.
...
- **King-login vs API-credentials:** een King-gebruikersnaam + wachtwoord werkt **niet** in het Zweedse BLA-formulier en ook niet voor KING Accountancy. BLA (Zweden) gebruikt OAuth2 client-credentials via [developer.bjornlunden.se/get-started](https://developer.bjornlunden.se/get-started/). Nederlandse accountancykantoren op **KING Accountancy / KING Finance** gebruiken Cloudswitch (zie hieronder). On-premise **King 5 / KING ERP Webservices** (ACCESS-TOKEN / ODBC) wordt door geen van beide native adapters ondersteund.
...
### KING Accountancy — Cloudswitch (Nederland, 2026-08)

- **Product:** KING Accountancy / KING Finance (cloud, voorheen iMUIS Online). Eén kantoor-tenant in Bokito, veel klantadministraties. Dit is **niet** de Zweedse BLA REST API.
- **Implementatie:** native FastAPI-adapter `apps/api/app/services/king_finance.py`, sentinel-URL **`native://king-accountancy`**, provider slug `king_accountancy`. SOAP naar `https://api.kingfinance.nl/v1/ws1_xml.asmx` (override: `KING_FINANCE_BASE_URL`).
- **Auth:** platform-env **`KING_FINANCE_PARTNER_KEY`** (Bokito als softwarepartner; aanvragen via `partners@muis.nl` na partnerovereenkomst `partners.nl@bjornlunden.com`). Per administratie een **omgevingscode** op de connectie (`auth.administraties[]`: `id`, `name`, `omgevingscode`). King-gebruikersnaam/wachtwoord worden niet opgeslagen. Productie-calls zijn IP-geallowlist op de partnerkey (VPS outbound IP registreren bij BL).
- **Onboarding (kantoor):** registreer als private/maatwerk-partner op [apps.kingsoftware.nl/developer](https://apps.kingsoftware.nl/developer); sandbox admin **2000**, test partnerkey `abcdefghijklmnopqrstuvwxyz`, test omgevingscode `00000` op [kingfinance.nl/demo](https://kingfinance.nl/demo). De accountant activeert de koppeling per administratie en plakt de omgevingscode in Bokito. Eerste kantoor: Bourgondienadvies (contact Harold). Conceptmail: `docs/king-cloudswitch-partneraanvraag.md`.
- **V1 tools (read-only):** `list_companies`, `get_company_details` (GetAdmInfo), `search_customers` / `get_customer` (DEB), `list_suppliers` (CRED), `list_accounts` / `get_account_balance` (GRB), `list_recent_bookings` (GetDatumLaatsteBoekingen + GetLaatsteBoekingPeriode). Elke company-scoped call vereist `company_id` (interne id, niet de omgevingscode). Writes (`CreateJournaalpost`, …) blijven buiten V1.
- **UI:** marketplace-kaart KING Accountancy; `McpConnectionForm` voegt administraties toe (naam + omgevingscode). Partnerkey staat niet in het formulier.
- **Gedrag zonder credentials:** install + discovery werken; `note` is `credentials_pending` / `partnerkey_pending` / `administraties_pending`. `list_companies` leest altijd de opgeslagen administraties. Overige tools: dev zonder partnerkey+admins → sandbox-mock; prod → duidelijke fout.
- **Tests:** `apps/api/tests/test_king_finance_native.py`.
...
- **Unified AI-event styling in de thread-timeline (2026-08):** timeline-events renderen niet meer als full-width scheidingslijnen met rauwe `snake_case`-labels, maar als compacte gecentreerde pills. AI-flow events (`agent_processed`, `suggestion_created`, `decision_created`, `agent_replied`, `triaged`, `escalated`, `ai_paused/resumed`, `decision_*`) delen de vaste violet AI-identiteit (`--color-ai` / `AiMark`, zelfde familie als Knowledge) met Sparkles-icoon (Check/X voor approved/dismissed); overige systeemevents zijn gedempt (`text-text-muted`). Brand-accent blijft voor workspace-chrome (nav, primary CTAs), niet voor AI-kaarten. Opeenvolgende events worden geclusterd tot één wrap-rij (`EventClusterTimelineItem` + `clusterEntries` in `ThreadDetail.tsx`) i.p.v. gestapelde regels. Alle event-types hebben vriendelijke labels in `EVENT_LABELS`; onbekende types worden gehumaniseerd (underscores → spaties, hoofdletter). AI-kaarten (`DecisionRequestMessage`) dragen dezelfde signatuur: Sparkles/BellOff-icoon en label in accentkleur zolang onopgelost; de "No reply needed"-kaart zit in dezelfde accent-familie (`border-accent/20 bg-accent/5`).
- **Dark-mode e-mailrendering (`EmailHtmlFrame` in `TimelineItem.tsx`, 2026-08):** e-mail-HTML rendert in een sandboxed srcdoc-iframe. In dark mode worden kleuren **direct in de DOM herschreven** (Gmail-aanpak), **niet via CSS `invert`-filters** — filters schakelen subpixel-antialiasing (ClearType) uit waardoor tekst dun en wazig oogt. Detectie: de effectieve achtergrond van de mail wordt gemeten via computed styles op body/tables/divs (legacy `bgcolor` telt mee; geen expliciete achtergrond = licht-ontworpen). Lichte mails, per element (cap 4000): near-witte achtergronden (luminantie > 0.88) worden exact `--color-bg-surface` (naadloos in de kaart, geen zwart blok); lichtere achtergronden (0.45–0.88) krijgen een **HSL-lightness-flip met behoud van hue/saturatie** (`flipLightness`); donkere/gekleurde achtergronden (knoppen, banners) blijven hun merkkleur houden. Donkere tekst (HSL L < 0.55) wordt licht geflipt (clamp 0.66–0.94); lichte borders worden gedimd zodat ze geen felle lijnen vormen. Secties met een echte computed `background-image: url(...)` worden volledig overgeslagen (tekst is op de foto ontworpen). Afbeeldingen blijven per definitie onaangeroerd. Donker-ontworpen mails blenden transparant in de kaart met lichte default-tekst. `isSimpleMessageHtml` routeert HTML met inline `color:`/`<font color>` ook door het iframe. Themawissel zonder reload: inline overrides worden teruggedraaid via `data-bokito-prev-bg` / `data-bokito-prev-color` / `data-bokito-prev-border`. Valkuilen (eerder ondervonden): filters op `html`/`body` worden niet op de canvas-achtergrond toegepast (root-achtergrond propageert eruit), en `[style*="background-image"]`-attribuutselectors matchen onbedoeld na `style.setProperty` (herserialisatie zet letterlijk `background-image: initial` in het attribuut).
...
- **Resizable Communication columns (2026-08):** the Messages hub uses `SplitRow` / `SplitPane` (Gmail/Linear-style). Operators drag the vertical dividers between the communication sidebar, conversation list, thread, and optional contact/agent panel. Widths persist in `localStorage` (`bokito.split.messagesHub`, `bokito.split.inbox`), clamp to min/max, and shrink so the thread pane keeps a minimum. Double-click or Enter on a focused divider resets that pane. Mobile keeps the existing list-or-thread stack (handles from `md` / `lg`).
...
- Workspace-branding gebruikt appgroep endpoints:
  ...
  - **Dashboard accent tokens (2026-08):** workspace `brand_color` is a seed, not raw paint. `applyBrandColor` in `tenant-branding.ts` converts the hex to OKLCH, keeps the hue, and clamps lightness/chroma into a product-safe range (Linear/Radix-style) so neon or muddy picks still look modern on buttons and nav. Derived tokens: `--color-accent` (refined solid), `--color-accent-hover` / `--color-accent-dark`, `--color-accent-fg` (near-black or white on the solid), `--color-accent-ink` (same hue walked to 4.5:1 on the page), `--color-border-focus`, and `--body-glow-*` for a soft shell atmosphere (`.app-atmosphere` on `AppShell`). Recomputed when the hex or theme changes. Tailwind `bg-accent` / `border-accent` use the refined solid; `text-accent` maps to ink. The color picker still stores the exact hex. **Platformstandaard is teal `#0D9488`** (dezelfde tint als de BA-avatar in het default palette). Lege of legacy neon seeds (`#00FF99` / `#00D986`) worden via `resolveBrandSeed` / `resolve_brand_color` naar die teal gemapt. CSS-fallbacks in `index.css` en de widget `DEFAULT_BRAND` volgen dezelfde hex. AI-violet (`--color-ai`) blijft een aparte identiteit.
...
- **Contact pairing:** `ChannelAccount.settings_json.require_pairing` zet onbekende afzenders op `pending`; hun threads worden niet door agents verwerkt tot een operator de contact goedkeurt (`PATCH /api/channels/contacts/{id}`). `blocked` contacts worden al bij de webhook geweigerd. Workspace-leden (login-e-mail) krijgen geen Contact-rij en triggeren geen inbound agent.
...
**App (`apps/mobile`):** Expo + expo-router (npm workspace `bokito-mobile`; monorepo-aware `metro.config.js`). Light/Dark/System thema via `ThemeProvider` (`bokito_ui_theme`), tokens gespiegeld aan de dashboard-palette (`#f7f9fc` licht, `#101319` donker, accent `#0d9488`). Backend-URL via `BOKITO_API_URL` in `app.config.ts` (dev: LAN-IP; prod/EAS preview: `https://app.bokito.ai`).
...
- **Mobile cohesion pass (aug 2026):** The Expo app now mirrors dashboard tokens (teal `#0d9488`, cool gray surfaces) instead of the old purple palette. Tabs are Home, Messages, Assistant, Decisions, Settings. Home is a Cockpit-lite: greeting, open-message and decision counts, “how Bokito fits together”, attention threads, pending decisions, and an Ask assistant jump. Messages use channel glyphs, relative time, AI summary preview, collapsed filters, and debounced search. Thread headers show the contact, tap opens a contact sheet, suggested-reply Edit prefills the composer, Stop dismisses a live stream, and takeover shows a banner. Decisions render real option labels and live-refresh via the gateway. Settings expose live connection, EN/NL copy, the same notification categories as the web app (desktop channel drives mobile push), and a link to the full workspace. Login matches product copy, remembers the last email, supports 2FA, and lands on Home. Gateway status is the same `connected|connecting|disconnected` model as the dashboard.
- **Mobile cohesion pass 2 (aug 2026):** A second iteration closed operator gaps so the phone app is the same loop as the dashboard, just slimmer. Home now has a notification inbox jump with unread count. Messages gained Closed + Slack filters, urgency/category chips, a compose jump to the assistant, and an error/retry empty state. Threads show human channel labels, a triage strip, Assign to me, Ask assistant (creates a chat grounded with `context_signal_id`), takeover on email/widget/WhatsApp/Slack, and AI vs customer vs you bubbles. Settings can switch workspaces (`POST /api/auth/switch-workspace`), fall back to default notification prefs, and open the notification inbox. Login can request a password reset and, in local Metro builds, fill the seed test account. Assistant Stop also clears the gateway stream and accepts a `conversationId` deep link. Decisions reject actions use secondary contrast. In-app notifications use `GET /api/notifications` and the same routing as push (`mention`/`assignment`/`handoff` → Messages).
- **Mobile cohesion pass 3 (aug 2026):** The phone app now follows the same human-label and Open-inbox rules as the dashboard. Messages defaults to Customers (`folder=external`); Team and All remain explicit chips. Home open-count is customer-only. Compose is labeled as a new assistant chat. Known agent subjects (`Reply to customer message`, `Daily platform scan`, `Assist:`, suggested reply) and widget placeholders (`Website visitor`, `visitor@web`, `cust_…`) are humanized in Home, Messages, thread header/contact sheet, decisions, notifications, and bubbles. Widget channel copy is Website chat / Websitechat. Roles map to Owner/Admin (Eigenaar/Beheerder). Relative hours in NL use `u`. Appearance is Light / Dark / System with the same tokens as the workspace; HTML mail and send buttons use theme ink. Agent thinking copy matches the dashboard (`Looking something up` / `Aan het opzoeken`) and the shimmer is teal. Empty customer inbox points at workspace channels.
- **Mobile cohesion pass 4 (aug 2026):** A second mobile walk closed operator gaps that still made the phone feel thinner than the workspace. Thread detail now has load-error and not-found states instead of an empty composer. Inbox Decisions no longer applies the Customers folder, so internal run decisions stay visible. Notifications route Govern/agent/contact/agenda/channel alerts to the workspace in the browser and keep thread/decision taps in-app. The composer is channel-aware (Email / Chat / Slack / WhatsApp / Agent, recipient row, no send-and-close on assistant). Reply drafts persist per thread. Failed sends surface an alert. Home and Decisions show retry empty states when the API fails. Decision option labels, agent roles (`orchestrator` → Lead), urgency, mock assistant bodies, and knowledge-tool steps (`search_index` → consulting knowledge) are humanized. AI bubbles use teal, not leftover purple. Settings shows push registration status and a retry. Thread menu can unassign; the contact sheet links to the workspace contact. EN/NL copy keys stay in parity.
- **Mobile cohesion pass 5 (aug 2026):** A third extra mobile walk aligned remaining operator queues and silent failures with the workspace. Messages views now follow the folder: Customers get Open / Mine / Unassigned / Decisions / Snoozed / Closed / Spam; Team gets Open / Updates / Results / Decisions / Closed (same `view=` API as the dashboard runs queues). Compose offers a new customer conversation on `/communication/new` in the browser, plus an assistant chat. Thread menus surface errors, can snooze until tomorrow, mark spam, draft a reply (`POST /api/signals/{id}/draft`), run the thread agent (`POST /api/signals/{id}/invoke-agent`), and pick saved replies (`GET /api/signals/saved-replies`). A live-updates banner appears on Home, Messages, threads, Assistant, and Decisions when the gateway is not connected. Expired API tokens sign the user out; the tab shell redirects to login. Assistant chats confirm delete, report load/rename failures, and start a new conversation when the agent chip changes. Contact sheets show notes and sibling threads. The Messages tab badge stays all-unread; Open/Mine counts are scoped to the active folder or the current list total. Internal channel copy is Team, matching the folder chip. Workspace slugs are hidden in Settings.
- **Mobile cohesion pass 6 (aug 2026):** A follow-up walk fixed operator bugs that still made the phone thinner than the workspace. Suggested-reply decisions now send the draft body and ask Send as you / Send as agent. Urgency is numeric (0–100): ≥80 Urgent, ≥50 High; Home “Needs you” and inbox badges use that, not string compare. Assign uses `GET /api/signals/members` with a teammate picker and a real “assign failed” alert. Outbound bubbles show failed/scheduled send status and can cancel a scheduled send. Agent replies take helpful / not-helpful feedback (`POST /api/messages/{id}/feedback`). Internal notes can be edited or deleted. Team/agent threads use a note-only composer. Teammate outbound sits on the left, not as “you”. Customers have a Pinned view. Contact notes are editable (`PATCH /api/channels/contacts/{id}`); sibling threads come from `GET /api/channels/contacts/{id}/threads`.
...
- **Platform LLM keys:** tenants zonder eigen Anthropic-key gebruiken de Bokito platform key (`platform_secrets` of env). Chat-API exposeert `llm_key_source` (`tenant` | `platform` | `mock`).
- **Bokito AI (managed provider, aug 2026):** Settings → AI → "AI providers" leidt met een **Bokito AI**-kaart: het platform-managed default (platform catalog + platform keys, Bokito kiest modellen — nu Claude). `GET /api/settings/models` bevat een `managed`-object: `{name: "Bokito AI", status: active|standby|unconfigured, chat: {slug, display_name, provider, key_source, ready}, embedding: {...}}` (berekend via `_managed_ai_status` in `routers/models.py`). `status=standby` wanneer de tenant self-managed modellen (BYOK) heeft — die gaan altijd voor in `resolve_model_call`. BYOK is her-geframed als optionele "Your own providers"-sectie; de oude amber "legacy platform catalog"-banner is weg. Frontend types: `ManagedAiStatus` in `lib/models-api.ts`.
- **Token-metering per user:** `usage_ledger.user_id` wordt gevuld door user-geïnitieerde chats (AgentLoop krijgt `auth.user.id`; autonome runs/system taken → NULL). `GET /api/cockpit/usage` bevat naast `by_model`/`by_agent` nu `by_user` (join op `users.display_name`/`email`; NULL → "Agents / system"). UsagePage toont een "By user"-kaart. Hiermee is resale-metering per tenant én per user compleet, onafhankelijk van de onderliggende provider.
- **API-foutshape frontend:** FastAPI's handler wrapt fouten als `{error: {code, message}}`; `parseApiErrorBody` in `lib/api.ts` leest naast `message`/`detail` ook `error.message`. Daarvoor toonden settings-calls elke fout als "HTTP 403 Unknown error".
- **Uitloggen-UX (aug 2026):** uitloggen is bereikbaar op drie plekken: (1) tenant-topbar avatar-dropdown (bestond al), (2) portal-sidebar naast de gebruikerskaart (`WorkspaceHubNav.tsx`, LogOut-icoonknop), (3) accountpagina in een eigen neutrale "Session"-sectie. "Delete account" staat apart onderaan in een rood omkaderde "Danger zone"-sectie zodat Sign out en Delete niet naast elkaar staan.
- **Settings-menu IA (aug 2026, UX-pass):** groepen in `SettingsLayout.tsx`: **Personal** (Profile & security, My assistant, Notifications), **Workspace** (Setup guide eerst, dan General, Branding, Members), **Communication** (Email & messages, Chat widget, Inbox AI, Help center), **Integrations** (Integrations), **Govern** (Govern, voorheen Autonomy & approvals), **Advanced** (Developers, Providers & models). Als onboarding incompleet is, wijst de Settings-subtitle naar de Setup guide. De losse menu-items Setup/Connected/Marketplace/Connected tools blijven één "Integrations"-entry met inner-tabbar (`components/shell/IntegrationsTabs.tsx`: Connected | Marketplace | Connected tools). Routes/deep-links (`/settings/marketplace?connect=...`) ongewijzigd. Help center (settings) is bewust anders dan rail **Knowledge**. Chat widget blijft `/ai/assistant`. Replay tour zit in het user-menu.
- **Platformtaal (2026-08 UX-pass):** **Communication** is de hubnaam overal (nav, command palette, CTAs). **Inbox** alleen voor queues (Open, Mine, Unassigned). Communication-side **Activity** heet **Agent runs**; Cockpit **Activity** blijft het event-log. Settings **Communication agent** → **Inbox AI**. **Autonomy & approvals** → **Govern**. Leftover "Messages" / "AI OS canvas" is uit hub-copy gehaald.
- **UX-pass iteratie 2 (2026-08):** `/settings` (rail + command palette) landt op de Setup guide zolang onboarding incompleet is, anders General. Settings-nav is i18n (EN/NL). Agent-runs-lijst, Communication-sidebar zonder agents, Cockpit-contacten, Govern-audit en Knowledge-sidebar hebben actie-CTAs. Channels-pagina spreekt over mailboxes, niet "Inbox". Agent-runs-chips zijn vertaald.
- **UX-pass iteratie 3–4 (2026-08):** Project/agent/agenda-empties linken naar Agenda, Agents of Communication. Help center-collections wijzen naar Knowledge (intern vs klantartikelen). Integraties- en Cockpit-innertabs zijn i18n. NL-filter "Communication" is "Communicatie". Fallback-titels "Support Inbox" zijn Communication. Lege company-conversations, notificaties, usage-by-user, saved replies, routing/automation-rules, API-tokens en webhooks hebben actie-CTAs. Thread zonder e-mailberichten wijst naar Email & messages. Cockpit-innertab Activity blijft het event-log (niet hernoemd naar Agent runs).
- **UX-pass iteratie 5 (2026-08):** Paginaheaders voor Contacts, Agenda, Cockpit (overzicht/activiteit/verbruik), Setup guide en Email & messages zijn i18n. Communication-chip "Agent runs" is vertaald. Inbox AI-copy spreekt over mailboxen, niet "per inbox". Composer zonder templates opent Email & messages (`#saved-replies`). Channel bindings, Agenda-automations (profiles/workstreams) en Cockpit-metrics hebben actie-CTAs of een duidelijke next step.
- **UX-pass iteratie 6 (2026-08):** Agenda-tabs/filters/empty, Contacts-filters/tabellen/empties, Setup guide-pijlers (inclusief NL assistant-prompt) en Cockpit-Activity-filters zijn i18n. Agenda-automations-innertabs (Triggers / Workstreams / Runtime / Runs) ook.
- **UX-pass iteratie 7 (2026-08):** Agenda-aanmaakdialog, Contacts-formulier, Automations-formulieren/acties en Cockpit-widgets (stats, Needs attention, events, agenda, contacten, metrics) zijn i18n.
- **UX-pass iteratie 8 (2026-08):** Cockpit-metricdialog (aanmaken/bewerken, eenheden, agents-update zonder `record_metric`-jargon), Usage-pagina (budget, plafonds, breakdowns), notificatie-dropdown en Email & messages-tabelchrome (Bokito-adres, mailboxen, row-acties) zijn i18n (EN/NL).
- **UX-pass iteratie 9 (2026-08, local walk):** Na login landt `/` op Communication als er al een current workspace is; de workspace-picker blijft `/workspaces` (user-menu). Lokale workspace-kaarten verbergen identieke localhost-URL's. Seed-admin (`admin@bokito.ai`) wordt altijd `email_verified`. **Projects** is geen rail-item meer (blijft `/projects`, link vanaf Agents). Agentlijst heeft Chat (Communication) en Schedule (Agenda `?agent=`). Knowledge-empty wijst naar Setup guide en assistant-chat. Verify-banner is i18n.
- **UX-pass iteratie 10 (2026-08):** Communication-inbox toont All / Mine / Open / Unassigned; Closed / Spam zitten achter **More**. Decision-cards en agent-feedback zeggen "That's not right" i.p.v. "Correct … interpretation". Cockpit Activity-tijdlijn toont max. 12 uur verleden (compact als er geen recente events zijn) i.p.v. een lege uren-grid over dagen. Communication-settings linken naar Email & messages en Inbox AI. Contactpaneel verbergt lege telefoon/bedrijf-velden. Workspace-kaarten tonen "subdomain required" alleen zonder slug.
- **UX-pass iteratie 11 (2026-08):** Login (inclusief 2FA, workspace-setup, Microsoft-SSO-fouten) is i18n (EN/NL). Routingregels, Slack-koppelkaart, syncstatus en notificatievoorkeuren gebruiken de bestaande `nav.json`-keys. Notificatieregels vertalen op row-id; opgeslagen labels blijven Engels.
- **UX-pass iteratie 12 (2026-08):** Signup, wachtwoord vergeten, wachtwoord resetten en e-mailverificatie zijn i18n (EN/NL), inclusief fouten, success-states en lokale dev-links. Shared copy (rechten, terug naar inloggen, e-mail) hergebruikt `loginPage`.
- **UX-pass iteratie 13 (2026-08):** Communication-sidebar **Inbox AI** opent `/settings/communication` (niet My assistant). Nieuw gesprek, thread-acties (Close / Assign to me / Create task), contactpaneel, directe chat en lijstlabels zijn i18n (EN/NL). Lege thread wijst naar Email & messages. Agent-types in de UI zijn **Lead** / **Agent**. Agenda-soorten zijn Recurring / Repeating / Check-in / Incoming. Agent-detail zonder runs wijst naar chat + Agenda (`agendaPage.openAgenda`).
- **UX-pass iteratie 14 (2026-08):** Cockpit-metrics: **Handled without you**, **How much agents can do**, **Customer rating 30d** (geen CSAT/Posture-jargon). Agenda-automatiseringen: Workstreams → **Flows**, runtime-rollen → Does the work / Lead / Reviews. Projects-lijst en -detail spreken over lead en flows; lege projecten linken naar Agents en Setup guide. Help center is klantartikelen; interne docs blijven Knowledge. Rail-subtitles voor Agents en Projects zijn in gewone taal.
- **UX-pass iteratie 15 (2026-08):** `/settings/govern` is de canonieke Govern-URL; `/settings/autonomy` en `/govern` redirecten ernaartoe. Project-aanmaak/verwijder-dialogs en projectdetail (about, runs, lead) zijn i18n; orchestrator-copy is **Lead agent**. Agenda-automatiseringstoasts zeggen flow/schema i.p.v. workstream. Direct-chat beslissingskaart is i18n. Channel bindings heten **Channel routing** en wijzen naar Agents. Govern history/passports-copy is in gewone taal (Undo i.p.v. Rollback).
- **UX-pass iteratie 16 (2026-08):** Knowledge-soorten en bekende paden zijn in gewone taal: Voice / Memory / Skills / Check-ins / Docs; `persona.md` → How we sound, `heartbeat.md` → Daily check-in. Nieuwe-agent-dialog is i18n; Orchestra-rol is **Lead**; system prompt heet **Instructions**. Agent archiveren zegt agentlijst, niet workforce. Developers **App tokens** (geen API/MCP-jargon in de titel); lege tokens wijzen naar Integraties. Govern-tokenkaart wijst naar Developers.
- **UX-pass iteratie 17 (2026-08):** Email & messages-toasts (sync, signature, routing, copy) zijn i18n. Communication-composer, compose-modal, project-picker, thread mentions/taken en notitie-edit zijn i18n. Developers-webhooks en Workspace API-copy zijn i18n. Assistant-settings-tabs: Look / Voice & hours / Install; persona-sectie heet **Voice**.
- **UX-pass iteratie 18 (2026-08):** Knowledge verbergt dubbele/legacy headings (`# Persona`, `# Heartbeat checklist`, `# Company`). Nieuwe workspaces bootstrappen How we sound / What we remember / About the company / Daily check-in. Assistant-look/voice/install-velden zijn i18n. Publiek Helpcentrum is i18n. Thread-projectpicker zegt **Lead** (geen orchestrator) en linkt naar het project. Agenda-triggers tonen Recurring/Check-in i.p.v. ruwe kind. Govern-diffs zeggen Flow / Instructions / Voice review.
- **UX-pass iteratie 19 (2026-08):** Integraties-copy spreekt over tools/apps i.p.v. MCP in titels en empties (Marketplace, Verbonden, Gekoppelde tools, Cursor-tab). Helpcentrum-404 is leesbaar, geen HTTP-status. Project-toasts en Agenda-fouten zijn i18n. Command-palette fallback voor Projects is "Shared goals…", niet orchestration. Workstream/orchestrator-restcopy in locales is Lead/Flow. Agenda-schemafouten en “plan op deze dag” zijn i18n. Nieuwe flow-runs zeggen flow, geen workstream.
- **UX-pass iteratie 20 (2026-08):** Communication-toasts (Inbox AI, verwijderen, bulk, ongelezen) zijn i18n. Zijpaneel toont Lead i.p.v. Orchestrator. Agentdetail linkt naar het gekoppelde project. Email-en-berichten-fouten (mappen, mailbox, OAuth) zijn i18n. Integratie-capabilities noemen geen MCP meer in de gebruikerstitel. Gekoppelde tools-hint zegt niet meer “MCP URL”.
- **UX-pass iteratie 21 (2026-08):** Contacts (lijst, persoon, bedrijf) is i18n inclusief toasts en lege staten die naar Communication linken. Cockpit/Govern/Agenda-automatisering-fouten zijn i18n. Marketplace-kaarten zeggen tools/apps i.p.v. MCP. Pin/Inbox AI/correctie/concept-antwoord-fouten zijn i18n. Setupchecklist-fout is leesbaar.
- **UX-pass iteratie 22 (2026-08):** Agentdetail-kaarten (model, instructies, chattoegang) zijn i18n en linken naar Providers & models. Members-pagina (rollen, tabel, uitnodigen) is i18n; mail-niet-geconfigureerd noemt geen env-vars. Project-repo en Agenda incoming-webhook-toasts/chrome zijn i18n. Profile-verificatiemailsuccess is i18n. Leftover project.po/flow-copy zegt Lead/Flow, geen orchestrator/workstream.
- **UX-pass iteratie 23 (2026-08, cohesion pass):** Communication-hub settings-rail linkt naar Chat widget, Integraties en Setup guide (naast Email & messages en Inbox AI). Channels ↔ Inbox AI hebben cross-links; threads en agentcontext wijzen naar Inbox AI. Page guide voor Communication + Learn-pagina linkt naar setup guide i.p.v. video-placeholder. Setup guide dekt ook first_decision/team; KPI-pijler opent Cockpit met `?addMetric=1`. Command palette: settings-pagina's, inbox-wachtrijen, mailbox koppelen. Projects terug in de rail. Govern-empties gebruiken EmptyState met CTAs. User-menu: Resume setup guide zolang onboarding incompleet is. Tour "Explore" opent setup guide.
- **UX-pass iteratie 24 (2026-08, inbox i18n + empty cross-links):** Thread-detail chrome is volledig i18n (EN/NL): toolbar (toewijzen, sluiten, spam, verwijderen, AI-overname, pinnen, details), composer-tabbladen (E-mail/Notitie), opgeslagen antwoorden/bijlagen, agentcontext en thread-lijstindicatoren. Mobiele threadlijst toont altijd een kanaalinstellingen-link (`ThreadListQuickFilters`). Cockpit `timeAgo`, gedeeltelijke load-fouten en thread-fallback zijn i18n. Lege Cockpit-events, Activity, Agenda en Contacts linken naar Communication, Agents, Agenda en/of Setup guide.
- **UX-pass iteratie 25 (2026-08, bulk actions + thread meta i18n):** Bulk-selectiebalk (`BulkActionsBar`) is i18n. Thread-prioriteit, daglabels (Vandaag/Gisteren), triage-chip en label-tags zijn i18n. `workforce.agents.title`/`newAgent` keys toegevoegd (NL-knop "Nieuwe agent"). Lege lead-sectie op Agents heeft een create-CTA. Knowledge-empty linkt naar setup guide, Agents en assistant-chat. Workspace language-sectie is i18n. Usage-empty linkt naar setup guide. Direct-chat copy/delete bevestiging is i18n.
- **UX-pass iteratie 26 (2026-08, branding + timeline + cross-links):** Branding-pagina (`/settings/branding`) volledig i18n (logo, favicon, merkkleur, subdomein, upload/delete, chatwidget-link). Timeline-events in threads (`TimelineItem`) zijn i18n — ~40 eventtypes (toewijzing, status, AI-flow, regels). Agent-contextpaneel: "Wat deze agent mag doen" en tooldefaults i18n. Command palette aria-labels i18n. Contactdetail lege gesprekken: hint + CTA naar kanaalinstellingen. Ledenpagina lege staat: hint + setupgids-link. Projectdetail: stapcount i18n. Learn-artikelen (`/learn/:slug`) tonen "Gerelateerde vervolgstappen" met links naar kanalen, agents, govern, enz.
- **UX-pass iteratie 27 (2026-08, agent telemetry + decision i18n):** Beslissingskaarten vertalen bekende backend-titels ("Reply to customer message" → NL). Agent-denkstappen (`agentSteps.ts`) volledig i18n in ThinkingTrace en ReasoningDisclosure. Cockpit/Activiteit-feed vertaalt "Loop N", tool-aanroepen en eventtypes (Nadenken, Tool-aanroep, Kennis doorzoeken). Chatwidget-instellingen: placeholders i18n. Bedrijfsdetail in Contacten: lege contacten/gesprekken met kanaal-CTA's. Agent-sessiekaart: actieteller i18n.
- **UX-pass iteratie 28–29 (2026-08, status labels + empty-state cross-links):** Gecentraliseerde statuslabels (`status-labels.ts`) voor threads, contacten, mailboxen, agenda, agent-runtime, flows, index en worklogs — gebruikt op Contacten, Agenda, Projecten, Helpcentrum, Sync-status en agentdetail. Mock-agent antwoorden worden in NL getoond (`translateMockAgentBody`) in timeline, direct chat en agent-sessies. Models-pagina: custom-model sectie en enabled/disabled/remove i18n. Slack/WhatsApp-connect placeholders i18n. Lege thread-selectie toont titel, hint en links naar nieuwe chat + setupgids. Lege klant-inbox linkt naar setup, kanalen en nieuwe chat. Agentdetail: agenda-leegstaat met link naar Agenda; agentdetail-agendakind i18n. Worklog-tabel status i18n. Nieuwe-gesprek-picker: agentrol i18n + setupgids-link bij geen agents. Activiteit-feed: audit-acties zoals "Mailbox losgekoppeld" in NL.
- **UX-pass iteratie 30–39 (2026-08, models/govern/onboarding polish):** Models-pagina (`/settings/models`) volledig i18n inclusief Bokito AI-kaart, provider-formulier en staff-sectie. Govern toolcategorieën (Berichten, Workspace, Agents, …) i18n via `toolCategoryLabel` op Govern en agent-tools-picker. Agent-sessie samenvattingen vertalen mock-tekst. Beslissingskaart concepttekst vertaalt mock-antwoorden. Accept-invite welkomststap i18n (naam, notificaties, knoppen). Inbox AI lege mailbox: extra link naar setupgids. Usage lege modelverbruik: links naar setup en models/providers.
- **UX-pass iteratie 40–49 (2026-08, govern i18n + auth onboarding):** Govern posture-kaarten, toestemmingsniveaus (Weigeren/Eerst vragen/Toestaan), tool-override labels en wijzigingsmetadata (resource/kind/status + diff-preview) zijn volledig i18n via `govern.json` — API-presetlabels worden niet meer getoond. Accept-invite formulier + welkomstheader volledig i18n (rol, wachtwoord, knoppen). Login/signup/wachtwoord-vergeten/e-mailverificatie placeholders i18n. Mailbox-koppel fout "geen autorisatie-URL" i18n. Agentdetail zonder runs: extra link naar Communication naast chat en Agenda.
- **UX-pass iteratie 50–60 (2026-08, agent permissions cohesion):** Agentdetail "Tools en toestemmingen" volledig i18n (autonomieniveau, toegestane tools, wake/pauze/archief). `agentAutonomyLevelLabel()` vertaalt manual/approval/auto op agentdetail en Govern agenttoegang-tab. Tools-picker linkt naar Govern-beleid. Ledenpagina lege tabel: primaire CTA "Lid uitnodigen" springt naar uitnodigingsformulier. Workspace-aanmaak: subdomein-placeholder i18n. Govern draft-bevestigingsdialog titels/knoppen i18n.
- **UX-pass iteratie 61–72 (2026-08, product loop cohesion):** Contacts, Cockpit, thread-sidepanel and New conversation can start an outbound email via `/communication/inbox/all?compose=1&to=`. Command palette keeps recent pages (`bokito.recent.pages`), shows create actions (email/contact/agent) first, and only lists settings after you type. Empty palette search offers suggested next steps. Settings nav has one-line hints; the topbar breadcrumb shows the settings subsection; incomplete setup shows a Setup chip; user menu links to Email & messages and Inbox AI. Cockpit has a dismissible “How Bokito fits together” card (Messages → Agents → Approvals). Deep-links `/contacts?new=1` and `/agents?new=1` open the create flows. Thread list shows a channel glyph and language-neutral relative time (`h` instead of `u`). Composer leftover English (“Send and close”, “Save current text as template”) is i18n.
- **UX-pass iteratie 73–83 (2026-08, first-run cohesion):** Topbar crumbs no longer repeat the workspace name when it matches “Bokito”, and extra segments (`Beslissingen`, `Persoon`, …) use `nav.crumbs.*` instead of raw keys. Communication search asks to “search conversations” (Ctrl+K stays on the command palette). Composer placeholders, AI feedback (Goed / Niet behulpzaam / Corrigeer), agent standby (`Pauze`), and model ids (`Claude Sonnet 4 (Bokito)`) are humanized. Cockpit “Conversations 7d” explains a zero week and still links into the inbox; recent contacts show Widget instead of `customer_widget`; agent-run events open the related run. Inbox Unread/Pinned stay clickable with an empty-state way back to All. Contact names are real links. Opening a contact falls back to the list when `GET /channels/contacts/{id}` 404s (mixed UUID text in SQLite). First login and the Communication rail land on **Open** (`/communication/inbox/open`) so newcomers see work that needs them, not the mixed All list.
- **UX-pass iteratie 84–93 (2026-08, contact–inbox–agent loop):** Contact profiles load conversations via inbox search when the CRM thread route 404s (same mixed UUID text), show last-seen from the latest thread, and open those threads in Communication Open. Opening `/communication/inbox/open` auto-selects a customer thread instead of internal agent work. The Open list marks internal rows with an Intern/Internal chip. Agents is titled “Agents” (not library), shows `Claude Sonnet 4 (Bokito)` and Lead/Agent roles, and explains the rail badge with a “review decisions” banner. Notification titles/bodies reuse `translateDecisionText` (NL: Antwoord op klantbericht); the dropdown has Settings / Open decisions and lands customer items on Open. Settings → General starts with Setup guide, Email & messages, and Inbox AI before the delete-workspace zone. Agenda week/list cards resolve a missing `agent_name` from a completed sibling occurrence, the trigger, or the agent role (planned Daily scans use `agent_role=orchestrator` with no `agent_id`) so future items still show who will run.
- **UX-pass iteratie 94–103 (2026-08, contact panel + inbox cohesion):** The thread side panel resolves a person by email/name when `contactId` is missing or 404s, lists their other conversations, and shows last-seen from those threads instead of “this conversation has no contact.” Open inbox lists customer threads above internal agent work. Cockpit hides “How Bokito fits together” while the page-guide banner is visible, translates known decision subjects, and deep-links to Open. Communication’s settings rail is Email & messages, Inbox AI, and All settings (no duplicate widget/integrations/setup). Knowledge auto-opens the first Voice/Memory doc. Widget ids like `cust_…` display as Website visitor. Learn-page related links use `/communication/inbox/open`.
- **UX-pass iteratie 104–116 (2026-08, inbox snooze + cockpit/agenda cohesion):** Snooze is back in the product: More → Uitgesteld/Snoozed, a toolbar clock with presets (1h / 4h / tomorrow / next Monday / until reply), and composer **Send and wait** on customer threads. Send-and-close stays hidden on internal agent-run threads. The snoozed queue includes wait-until-reply (backend `view=snoozed` is all `pending`). List rows show a Later chip and a wake time. Resuming a parked thread returns to Open instead of an empty snoozed pane. Cockpit Recent events hide thinking/search noise and keep settings, decisions and completed work; stale decisions say “Waiting N days — open to decide.” Mailbox titles collapse `Bokito (Bokito)` to Bokito. Agent-runs get their own page-guide copy. Agenda completed items open the matching Agent-runs conversation (Results) instead of the raw work-log dump; the leftover run page now uses human labels and links back to Communication and Agenda. Command palette includes Open and Snoozed. Notifications older than a week say they are still open.
- **UX-pass iteratie 117–128 (2026-08, contacts–inbox–project–knowledge loop):** The Contacts table and Cockpit recent-contacts list enrich last-seen and thread counts from Communication (email/id match; unique real names only — generic website visitors are not collapsed together). Clicking a thread count opens the inbox conversation. Duplicate notification cards for the same waiting decision collapse into one “N similar — still waiting” row; opening it marks the group read. Customer thread chrome keeps assign / close / snooze / take over / details; spam, delete, unread, pin and forward sit under More actions. NEXT chips no longer repeat Close/Assign — only Create task stays. Cockpit consecutive identical events collapse (“2 similar”), and the Awaiting-decision card opens the first waiting item in Inbox or Agent-runs. Project cards show Conversations / Lead / Knowledge links; project recent activity opens the matching Agent-runs conversation. Knowledge docs say agents read them and link to Agents and Agent-runs. Command palette searches workspace knowledge. New conversation has Write to a person and lists CRM emails that open compose.
- **UX-pass iteratie 129–140 (2026-08, agent–inbox–settings loop):** Agent run history and project activity open the matching Agent-runs conversation instead of the raw work-log dump. “This agent's threads” lands on Agent-runs with an `?agent=` scope chip (not customer Inbox/All). Inbox shows a clearable “Conversations for {agent/project}” banner when scoped. The Agents attention banner opens the first waiting conversation (Inbox or Agent-runs). New email compose labels (From/To/Subject/Attach) and dialog Close follow the workspace language. Activity collapses consecutive identical events like Cockpit. Command palette knowledge hits drop duplicated path prefixes. Agent detail upcoming agenda includes future wakes even when status is not exactly `planned`. Govern Policy explains per-agent overrides and links to Agents and Inbox AI. Inbox AI links to Agents, Govern, and Communication. Setup KPIs link to Usage. Cockpit groups decisions waiting 14+ days. Contacts with zero threads offer Write email instead of a bare 0.
- **UX-pass iteratie 141–153 (2026-08, settings–activity–widget loop):** Settings → Chatwidget and `/settings/widget` land on website install (not the internal team widget). Branding, Inbox AI, Email & messages, and Communication/Contacts learn-pages also point at the website widget. Activity defaults to Headlines (hides thinking/search), translates leftover English (“Executed approved action approve”, “Run started”), and opens agent work in Agent-runs — including run rows that previously dumped into the raw work-log (audit rows stay in Inbox). Members show a Jij/You badge. My assistant “new chat” opens the personal assistant thread, not the customer Open queue. Companies empty state can jump back to People. Widget breadcrumbs show Install / Voice / Appearance.
- **UX-pass iteratie 154–163 (2026-08, agenda–marketplace–profile loop):** Agenda Automations translate trigger names (Dagelijkse platformscan, Check-in) and schedules (elke 30 min, elke dag) with a real separator so name and cadence no longer jam together. Heartbeat maps to Check-in for display and Agent-runs matching. Marketplace cards and search use Dutch host copy (Gmail-mailboxen…, Eigen tool, WhatsApp). Connected empty-inbox CTA says Communicatie, not Communication. Profile signature links to mailbox signatures, My assistant, and Notifications. Contacts without a company explain domain backfill and jump to Companies. Usage shows a next step when conversations this week are 0. Inbox “Ask agent” and bare “Label” are now Ask assistant / Add label so first-time users see one name for help.
- **UX-pass iteratie 164–175 (2026-08, integrations–automations–assistant loop):** `/settings/integrations/mcp` and `/settings/integrations/marketplace` no longer dump into the Open inbox — they redirect to Connected tools and Marketplace. MCP empty state explains the next step and links Marketplace / Agents / Govern; Docker reachability copy stays in the connect dialog. Marketplace kind/filter says Tools / Tools voor agents so it is not confused with the Gekoppelde tools tab. Inbox AI routing and the Messages rail both say Websitechat. Setup, `/automations`, and project “add steps” open Agenda Automations. My assistant links Knowledge, Inbox AI, and Notifications; Profile “Mijn assistent” opens `/settings/assistant`. Command palette finds Marketplace and Gekoppelde tools. Help center points at website-widget install. Widget preview chrome follows the workspace language (Nieuw gesprek starten, Instellingen, Berichten).
- **UX-pass iteratie 308 (2026-08, local product walk):** Page-guide banners put a real period + space before “Read the full explanation” so title and link no longer run together. Modules rail “New” badge clears after the first visit (`bokito.nav.newTab.seen.modules`). Composer distinguishes *no channel* vs *channel needs setup* (Bokito address can show enabled + Setup required when platform Resend webhook/API is missing — `can_send` false); CTA becomes Finish channel setup. AI handling banner softens when send is blocked. Contact panel empty previous-threads explains first conversation + profile link. Channels with setup_required / action_required / error auto-expand their checks and show a page-level setup alert. Knowledge sidebar subtitle wraps (`line-clamp-2`) instead of truncating mid-sentence.
- **UX-pass iteratie 309 (2026-08, decisions cohesion):** Cockpit `open_decisions` counts the same open decision threads as Agents attention (including internal agent-run decisions, not only external channels). Cockpit KPI uses `max(summary, attention list)` so the card cannot show 0 while Needs attention lists a waiting item. Communication sidebar shows **Awaiting decision** / **Wacht op beslissing** under New chat when `agentsAttention > 0`, linking to `/communication/runs/awaiting-decision`. Empty KPI hint and product-help Communication articles mention that path.
- **UX-pass iteratie 310 (2026-08, empty-state cohesion):** Calm Cockpit “Needs attention” links Communication, Agenda, and Govern drafts. Empty Agent-runs lists link Agents, Agenda, and Start chat. Empty project queues offer Add + Open Communication; empty project agent rosters link Browse agents. Command palette actions include Open Agenda and Review Govern drafts (also in no-results suggestions). Contacts people empty links Open Communication. Agents list empty links Open Agenda (copy already mentioned scheduling). Automations triggers empty links Open Agents beside Back to Agenda. Customer list chips (Needs reply / Unread / …) no longer stick when opening Agent-runs or other non-customer leaves — they were hiding runs and showing the wrong empty copy.
- **UX-pass iteratie 311 (2026-08, channels–knowledge–projects loop):** Settings → Channels empty offers Setup / Communication / Widget beside Add channel; page cross-links add Communication and Setup. Project docs empty links Knowledge + Agents; project resources empty links Modules + Integrations. Knowledge doc header adds Talk to your assistant (try-in-chat). ContactPanel “no previous conversations” offers Write email + Open Communication + Profile. Modules that are on link Communication beside Agents and Govern.
- **UX-pass iteratie 312 (2026-08, marketplace–cockpit–chat–projects loop):** Marketplace filtered/empty lists offer Clear filters + Modules + Connected integrations. Cockpit “Time saved” at 0 deep-links to Agents (not Usage). Direct-chat empty pane links Agents / Knowledge / Setup beside New chat. Thread project picker with no projects offers Open Projects. Learn related links: Cockpit→Govern, Knowledge→Communication, Govern→Communication.
- **UX-pass iteratie 313 (2026-08, channels–usage–activity–copy loop):** WhatsApp and Slack post-connect screens link Communication + Inbox AI (Slack also Govern). Usage API failure shows recovery links (chat / Agents / Setup / Models). Activity headlines empty adds Open Communication beside Show full log. Inbox automation-rules empty links Agenda automations. Unknown module setup offers Modules + Marketplace. Invite and onboarding copy say Communication (not Messages/Berichten).
- **UX-pass iteratie 188–227 (2026-08, settings/tech operator walk):** Providers & models confirm destructive actions, humanize provider types, filter models, copy model ids, and show cost bands. Developers warn on full-access tokens, copy a curl example, hide revoked tokens, and reject non-https webhooks (localhost http allowed). `PATCH /api/channels/bindings/{id}` toggles enabled/priority; routes can target one channel account. `DELETE /api/kb/collections/{id}` removes a help-center collection and its documents. Help center copies `/help/{slug}`. Marketplace search/status persist in the URL. Connected remembers last kind. MCP disconnect confirms. Usage `?days=` plus CSV export. Unsaved guards on Workspace, Branding, widget Look, and Inbox AI.
- **UX-pass iteratie 228–267 (2026-08, cockpit/profile office-manager walk):** Cockpit greets by name, shows last refresh, locale numbers, Open first, and restore-setup. Activity filters persist in the URL and group by day. Notifications can pause in-app / restore defaults / preview. Profile edit affordances stay visible. My assistant has templates, sticky save, and unsaved guard. Members validate email and confirm owner transfer. Palette adds invite/profile/talk. Learn/Docs continue last article. Public help search stays in `?q=`.
- **UX-pass iteratie 268–307 (2026-08, leftover office-manager walk):** Login remembers last email and language. Verify/reset/invite show a spinner, copyable dev links, a missing-token reset card, a 60s resend cooldown, and a prominent back-to-sign-in. New conversation remembers the last recipient, honors `?agent=` / `?to=`, retries load failures, and opens Contacts when more than eight people match. Knowledge has unsaved guard, Ctrl/Cmd+S, copy path, kind chips, and last refresh. Projects clear an empty search, submit create on Enter, open from `?new=1`, and format token numbers. Branding Save stays off until dirty; workspace URL copies; public help previews. Help centers confirm collection delete and search documents. Public help clears search and copies article URLs. Widget Save includes Voice and availability. Command palette adds New project. Agenda automations search triggers, confirm delete, and add a runtime profile on Enter.
- **UX-pass iteratie 176–187 (2026-08, crash copy + agenda future links + locale dates):** A crashed page shows Dutch/English copy instead of raw `errorBoundary.title` keys. Agent upcoming rows use the workspace language (`wo 26 aug, 14:57`) and future wakes open Agenda (`?trigger=`) instead of a past same-subject Agent-runs thread; Agenda and Automatiseringen sit next to each other. Empty agent signatures link to mailbox signatures. Usage 0% autonomy and missing feedback point to Govern and website-chat install. NL no longer says Inbox-AI. Settings General hint is company name and language (no timezone). `/settings/website-widget` lands on website-chat install. Inbox list, thread times, snooze, Cockpit timeline, members, help articles, and mailbox last-sync follow the workspace locale (`24 jul`). WhatsApp/Slack empty states explain the outcome first. Command palette “markt” finds Marketplace. Notification budget copy no longer leaves a space before the period.
- **Inbox after close (2026-08):** closing a thread (toolbar, send-and-close, or the no-reply **Gesprek sluiten** card) stays on the current inbox (Open, Mine, All, …). The hub opens the first remaining conversation in that box, or the empty inbox. It no longer hops to All while still showing the closed thread that just left the list.
- **Tenant branding in the rail + widget (2026-08):** the left rail shows the workspace name and pictogram (favicon, else logo) instead of a hardcoded Bokito mark; without a pictogram the Bokito logo stays as fallback. The website widget launcher and header use the same branding pictogram and hide the Bokito monkey so the two marks no longer overlap. Widget settings can still upload an override.
- **Settings nav (2026-08):** the settings sidebar lists only the page name. The old one-line hints (Logo en kleuren, Collega's uitnodigen, …) move to a hover tooltip that hangs directly under the button (`side="bottom" align="start"`, small offset) so it stays visually attached to the nav column.
- **Notification deep links (2026-08):** clicking a bell notification opens the thread on Inbox All (or the matching Agent-runs box) using the payload `signal_id`. Closed, snoozed, and spam threads retarget to their dedicated inbox box instead of jumping to another conversation.
...
- **SPA cache-strategie (aug 2026):** de container-Caddyfile serveert `index.html`/`sw.js`/root-assets met `Cache-Control: no-cache` (altijd revalideren) en gehashte `/assets/*` met `public, max-age=31536000, immutable`; chat-widget bundle ook `no-cache`. Zonder deze headers cachten browsers `index.html` heuristisch, waardoor gebruikers na een deploy op een oude bundle bleven en nieuwe routes (zoals `/projects`) via de catch-all op Communication landden. Daarnaast reload't `main.tsx` eenmalig bij `vite:preloadError` (verdwenen oude chunk na deploy), met 30s-guard tegen reload-loops.
...
- **AI-taalbeleid (2026-08):** twee onafhankelijke assen in `app/services/language.py`. (1) **Antwoordtaal** (`reply_language`): taal van conceptantwoorden aan de klant; default `auto` = spiegel de taal van het klantbericht (Engelse mail → Engels concept, ook op een NL-workspace); vast te pinnen per mailbox (`ai_config.reply_language`) of tenant-breed (`settings.ai_reply_language`); precedentie mailbox → tenant → auto. (2) **Teamtaal** (`ai_workspace_language`, fallback `PLATFORM_DEFAULT_LANGUAGE`, standaard `nl`): taal van AI-tekst gericht aan het team, zoals de `NO_REPLY_NEEDED:`-samenvatting. Nieuwe workspaces slaan die fallback op in `default_tenant_settings()`. Beide worden als "Language rules" in de suggest- en auto-prompts geïnjecteerd. Instelbaar op `/settings/communication` (kaart "Taal" + per-mailbox override); API: `GET/PUT /api/settings/ai-modes` (velden `reply_language`, `workspace_language`) en per-mailbox `ai_config.reply_language` (lege string wist de override). Ondersteunde talen: nl, en, de, fr, es. Statische kaart-copy (titels, knoplabels als Send/Edit/Escalate/Close thread) komt niet van de backend maar wordt client-side vertaald via i18n (`communication.json` → `decisionCard.*`, mapping op optie-id/action_type in `DecisionRequestMessage.tsx`), zodat die de platformtaal van de gebruiker volgt.
...
- **Composer-pills (aug 2026):** de thread-composer (`ReplyComposer`) toont mentions als gekleurde pills in plaats van rauwe markup. De textarea bevat de weergavetekst (`@Name`); `lib/mention-editor.ts` mapt weergave- naar markup-offsets en past edits atomisch toe (elke edit die een mention raakt verwijdert de hele pill, zoals Slack/Notion). `ComposerCard` heeft een `highlighter`-prop: een mirror-laag achter een transparant-getinte textarea die de pills tekent (`.composer-mention-pill` in `index.css`, layout-neutraal — alleen achtergrond/kleur/radius zodat metrics gelijk blijven aan de textarea). Verzonden berichten blijven markup gebruiken en renderen via de bestaande `.mention-chip`.
...
- UI-taal is persoonlijk (`GET/PATCH /api/me/preferences` → `User.settings_json.ui_language`), niet workspace-Save. Ontbrekende/ongeldige voorkeur valt terug op `PLATFORM_DEFAULT_LANGUAGE` (`nl` tenzij `en`).
...
- Channel settings: **Sync now** op de sectieheader (alle sync-kanalen) + per-rij Sync via `POST /api/channels/accounts/{id}/sync`.
...
- `GET /api/signals/sync-status` reported per-mailbox sync state for the inbox Sync status panel. **Removed in aug 2026** (see the channel registry entry in section 31): sync facts are checks on the channel row from `GET /api/channels`.
...
## 30. Live UX-audit Bokito-tenant (augustus 2026)

Bevindingen uit een audit van de live tenant op app.bokito.ai (echte Outlook-mail, AI-suggesties, agent sessions). Status: open issues, nog niet gefixt.

- **Suggestiekaart bevat interne scaffolding**: de suggest-mode agent stopt soms `**Internal note:**`, `**Proposed reply:**`, `---`-scheiders en `> Note for the reviewer` (incl. emoji) in de body van de "Suggested reply". Eén klik op Versturen zou dit alles naar de klant mailen. De suggestie-payload moet gescheiden worden in verstuurbare reply-body vs interne toelichting/reviewer-notitie.
- **AI krijgt afgekapte e-mailinhoud**: de agent claimde dat een DJI-mail "mid-sentence cut off" was terwijl de volledige mail in de thread staat — de loop-input bevat kennelijk een getrunceerde body/snippet in plaats van de volledige tekst.
- **No-reply-classificatie inconsistent**: marketing/orderbevestigingen krijgen correct de "Geen antwoord nodig"-kaart (Gesprek sluiten / Taak aanmaken / Open laten), maar een geautomatiseerde afsluitmail van DJI kreeg toch een volledige suggested reply + decision.
- **Decision-flood bij backfill**: elke gesynchroniseerde (ook oude/notificatie-)mail triggert een suggestie/decision → 79 openstaande decisions in Cockpit. Suggesties zouden backfilled mail en notificatie-afzenders (GitHub, LinkedIn, PayPal, e.d.) moeten overslaan.
- **Markdown niet gerenderd**: zowel de suggestiekaart als agent-session-berichten tonen letterlijke `**`/`##`-tekens.
- **Taalmix NL/EN**: rail/nav in NL, pagina-inhoud (Settings, Cockpit, thread-acties, AI-chips) in EN; binnen één kaart NL-kop ("GEEN ANTWOORD NODIG") met EN-uitleg. Eerste render is EN en flipt daarna naar NL.
- **Tijdweergave inconsistent**: 12-uurs AM/PM in een NL-omgeving; threadlijst toont sync-tijd-gebaseerde relatieve tijden ("1u") terwijl het contactpaneel andere waarden toont ("9u ago", "Last seen 3h ago"); "Conversation started"-divider staat onder de eerste mail i.p.v. erboven.
- **Statuskleur fout**: Sync status-paneel toont "Connected" in rood/waarschuwingskleur.
- **Agent-naamgeving**: "Bokito's assistant" en "Bokito Assistant" bestaan naast elkaar en zijn in het Vraag-agent-menu niet te onderscheiden; Agents-pagina toont rauwe modelstring (`claude-sonnet-4-20250514`).
- **Interne namen lekken in Cockpit**: Recent events toont "Loop 2 · Agent run · Think", `search_index`, `create_decision_request`; Usage-KPI toont een getal zonder eenheid; "Awaiting decision 79" vs badge "6" bij Needs attention is inconsistent; badge "79" op het Agents-navitem verwijst nergens zichtbaar naar.
- **Notificatiemail vervuilt inbox en CRM**: 13+ identieke "GitHub Deployment review"-threads elk als losse unread thread met eigen decision; geautomatiseerde afzenders (GitHub, LinkedIn, GoPro) worden CRM-contacten in "Recent contacts".
- **Time saved 0m** ondanks tientallen AI-verwerkte threads — learning/metrics-hook registreert geen uitkomsten.
- **Wat goed werkt**: inline agent session (contextueel antwoord, "Gebruik als antwoord", think/token-disclosure), "Geen antwoord nodig"-kaart, mailbox History-instelling (sync window), automations-toggledesign, correcte ontvangsttijd voor nieuw gesynce mail.

## 31. Member lifecycle & transactional mail (augustus 2026)

- **Mail-delivery**: `transactional_mail.py` levert via Resend HTTP API (`RESEND_API_KEY`) met SMTP-fallback (`SMTP_HOST`) en dev-log als laatste optie; `MAIL_FROM` overschrijft `SMTP_FROM`. Verzending is fire-and-forget met 3 pogingen (0s/5s/25s backoff) zodat een mail-storing invites/resets nooit blokkeert. `send_mail` retourneert True zodra een provider geconfigureerd is en de levering is ingepland; `mail_sent: false` in API-responses betekent "geen provider geconfigureerd". Alle mails (invite, password reset, verificatie, welkom, password-changed) hebben één gedeelde HTML-layout (inline CSS) + plain-text alternatief. Productie zonder provider logt een luide startup-warning (`production_config_warnings`). Ops: bokito.ai-domein in Resend verifiëren (SPF/DKIM) vóór go-live.
- **Invites**: opnieuw uitnodigen van een pending e-mail vervangt de bestaande invite (nieuw token + 7 dagen expiry) i.p.v. rijen te stapelen; een bestaand lid uitnodigen geeft 400. `POST /api/app/workspaces/{id}/invites/{invite_id}/resend` roteert het token, reset de expiry en mailt opnieuw (max 3 resends/uur per invite, daarna 429). Members-UI heeft een resend-knop op pending rijen; bij `mail_sent: false` toont de UI een warning-toast en kopieert de invite-link automatisch.
- **Accept-invite = geverifieerd**: het klikken van de invite-link bewijst mailbox-bezit, dus `email_verified` wordt True gezet. Na acceptatie volgt een licht, overslaanbaar welkomstscherm (display name, avatar, e-mailnotificatievoorkeuren) dat `onboarded_at` in `User.settings_json` zet via `PATCH /api/auth/profile { onboarded: true }`, plus een welkomstmail.
- **Sessie-hygiëne**: logout en `/api/auth/revoke` verwijderen de server-side `Session`-rij (niet alleen de cookie). Password reset trekt álle refresh-sessies in; password change trekt alles in en geeft het huidige device een nieuwe sessie. Beide sturen een "password changed"-notificatiemail. Member-verwijdering zet threads van die user terug in de unassigned queue (met `SignalEvent` `unassigned`-audit) en trekt refresh-sessies in wanneer dit het laatste workspace-lidmaatschap was.
- **Soft verification gate**: signup stuurt direct een verificatiemail (24u token). Onverifieerde users zien een slanke banner in de shell met resend-knop; server-side blokkeert `require_verified_email` alleen outbound acties: `/api/signals/{id}/reply`, `/api/email/send` en mailbox-connect (`/api/email/*/oauth/start`). Invite-accepted en SSO-users zijn al geverifieerd en raken de gate nooit.
- **No-workspace login state (ClickUp-model)**: een user-account blijft bestaan wanneer een admin het laatste workspace-lidmaatschap verwijdert; alleen de toegang tot dat tenant vervalt. Login met correct wachtwoord retourneert dan geen 403 meer maar `{ requires_workspace: true, setup_token, email, pending_invites }`. Het `setup_token` (JWT type `workspace_setup`, 30 min) geeft uitsluitend toegang tot `POST /api/auth/workspace-setup/accept-invite` (pending invite op het eigen e-mailadres accepteren zonder invite-token/wachtwoord-herhaling) en `POST /api/auth/workspace-setup/create` (nieuwe workspace als owner via `create_workspace`). De Login-pagina toont deze staat als aparte stap: openstaande invites met Join-knop plus een "Create workspace"-formulier. SSO-login maakte al automatisch een workspace aan voor membership-loze accounts; dit gedrag is ongewijzigd. `/auth/2fa/verify` kan dezelfde workspace-setup-respons geven.
- **Mail-status endpoint**: `GET /api/app/mail-status` retourneert `{ configured }` op basis van `mail_configured()`. Member Management toont admins een permanente warning-banner wanneer geen mailprovider is geconfigureerd (invitemails worden dan niet bezorgd; de kopieerbare invite-link blijft de fallback).
- **Prod mail-diagnose (aug 2026)**: de Hostinger-VPS blokkeert ALLE uitgaande SMTP-poorten (25/465/587/2525 timeouten; 443 werkt) — SMTP-delivery vanaf productie is dus onmogelijk zonder Hostinger-supportticket. bokito.ai-mail (MX `mx1/mx2.spacemail.com`, SPF `spf.spacemail.com`) draait bij Spacemail/Spaceship, niet bij Hostinger (geen mail-order voor bokito.ai). Enige werkende route: **Resend HTTP API** (`RESEND_API_KEY` + `MAIL_FROM` in `/opt/bokito/.env.prod`, domein verifiëren met de DKIM/SPF-records die Resend geeft in de Spaceship-DNS). De mailer ondersteunt nu ook impliciete SSL (SMTPS): `smtp_ssl` setting of automatisch bij `smtp_port=465` (`transactional_mail._deliver_smtp_sync`), relevant zodra SMTP ooit ontgrendeld wordt.
- **Resend live (aug 2026)**: transactionele mail is in productie operationeel via Resend. Domein `bokito.ai` is aangemaakt en geverifieerd in Resend (regio eu-west-1, alleen sending; DKIM-TXT `resend._domainkey`, Return-Path-MX + SPF-TXT op `send`). De DNS van bokito.ai staat bij **Cloudflare** (niet Hostinger/Spaceship; nameservers irena/scott.ns.cloudflare.com); records worden beheerd via de Cloudflare-API met `CLOUDFLARE_API_TOKEN` in de lokale `.env`. De API-key (`bokito-production`) staat als GitHub-secret `RESEND_API_KEY`; beide deploy-jobs upserten hem automatisch in `.env.staging`/`.env.prod` op de VPS, dus key-rotatie = secret updaten + redeploy. Afzender is de code-default `Bokito <no-reply@bokito.ai>` (geen `MAIL_FROM` gezet). End-to-end geverifieerd: signup op productie levert de verificatiemail af via Resend.
- **Bokito relay-adressen (aug 2026, vervangt het auto-adres)**: een tenant krijgt géén e-mailadres meer bij bootstrap. Operators maken **maximaal 3** relay-adressen aan via `POST /api/channels/email/relays` (`services/email_relay.py`): prefix van 3-24 tekens `[a-z0-9-]`, niet uit `RESERVED_PREFIXES` (`postmaster`, `abuse`, `admin`, `noreply`, `no-reply`, `mailer-daemon`), adres = `{prefix}-{workspace-slug}@in.bokito.ai` (domein via `BOKITO_INBOUND_DOMAIN`). Globale uniciteit op het genormaliseerde adres → 409 met `suggestion`. `GET /api/channels/email/relays` geeft opties (domein, slug, limiet, gebruikte prefixes). Provider-string blijft `"bokito"`, dus de Resend in/uit-paden zijn ongewijzigd; `find_relay_by_address` doet de inbound lookup. `bootstrap_tenant` maakt alleen een `channel="widget"`-rij aan (`ensure_widget_channel`). Migratie `017_channel_registry` verwijdert ongebruikte auto-relays (geen signals/routing rules/bindings) en backfilt één widget-rij per tenant. Tests: `apps/api/tests/test_email_relay.py`. **Inbound**: Resend Receiving op domein `in.bokito.ai` (eu-west-1, sending+receiving, DKIM/SPF/MX via Cloudflare) → Svix-signed `email.received`-webhook naar `POST /api/inbound/resend` (`routers/inbound.py`; secret `RESEND_WEBHOOK_SECRET`, GitHub-secret + deploy-upsert in `.env.staging`/`.env.prod`). De webhook is metadata-only: body/headers worden opgehaald via `GET https://api.resend.com/emails/receiving/{email_id}`, attachments via `.../attachments` (gedownload en opgeslagen via de storage-backend). Threading via RFC-headers: `thread_external_id` = eerste Message-ID uit References (of eigen Message-ID); `rfc_message_id`/`references` in message-metadata zodat `_reply_context` replies correct threadt. Mail aan onbekende adressen op het catch-all-domein wordt gedropt. **Outbound**: `send_via_provider` heeft een `bokito`-branch die via de Resend Send API verstuurt (platform-`RESEND_API_KEY`, From = tenant-adres, In-Reply-To/References-headers, attachments base64). **UI**: een relay is een gewone rij in de kanalenlijst op `/settings/channels` (state-badge, capabilities `receive`+`send`, checks `inbound_webhook`/`outbound_key`/`forwarding`, copy-adres, Signature/Routing/Primary, verwijderbaar). Aanmaken gaat via "Kanaal toevoegen" → "Bokito-adres" met live preview en teller "x van 3".
- **Kanaalregister (aug 2026)**: `ChannelAccount` is de enige kanaalentiteit; `services/channel_registry.py` leidt per rij één uniform beeld af, met één resolver per kind (`email_mailbox`, `email_relay`, `widget`, `whatsapp`, `slack` — nieuw kanaaltype = één resolver bijschrijven).
  - **state**: `setup_required` | `connecting` | `active` | `degraded` | `action_required` | `paused` | `error`. Afgeleid, niet opgeslagen: `is_enabled=false` → `paused`; een `fail`-check → `action_required` of `setup_required` (afhankelijk van of de operator credentials moet leveren); `warn` → `degraded`; alleen `pending` → `connecting`.
  - **capabilities**: `receive`, `send`, `sync`. Relays en widget/WhatsApp/Slack hebben geen `sync`, dus tonen geen syncdetails. `can_send(row)` (capability `send` + state niet `paused`/`error`/`action_required`) bepaalt sendability; `GET /api/email/accounts` geeft `state`, `capabilities` en `can_send` mee zodat de frontend niet meer op `provider === 'bokito'` uitzondert. `channels/outbound._workspace_email_account` kiest primair → echte mailbox → relay, allemaal via `can_send`.
  - **checks**: granulaire regels (`id`, `state` ok/warn/fail/pending/na, `detail`, optionele `action`). Mailbox: `credentials`, `folders`, `last_sync`, `sync_errors` (fail vanaf 3 opeenvolgende fouten; `last_sync` warn na 24u stilte). Relay: `inbound_webhook` (`resend_webhook_secret`), `outbound_key` (`resend_api_key`), `forwarding` (pending tot eerste mail). Widget: `installed`, `office_hours`. WhatsApp: `credentials`, `phone_number`, `webhook`. Slack: `bot_token`, `signing_secret`, `events`.
  - **Endpoints**: `GET /api/channels` (alle configureerbare rijen; `internal` en `mock` uitgefilterd), `GET/PATCH /api/channels/accounts/{id}` (label, `is_enabled`, `is_primary`, `sync_window_days` 0-3650 als backfill-venster), `POST /api/channels/accounts/{id}/sync` (400 zonder capability `sync`), `DELETE` (widget geblokkeerd, ontkoppelt eerst signals/routing rules/bindings). `GET /api/signals/sync-status` en de `SyncStatusPanel` zijn verwijderd: folderdetails zitten nu in de checks van de e-mailrij. Frontend: `lib/channels-api.ts` + `ChannelList`/`ChannelStateBadge`/`AddChannelDialog`, statuslabels centraal via `channelStateLabel` (`status.channel.*`). `AddChannelDialog` groepeert e-mail in één menu-item ("E-mail", met de Outlook-, Gmail- en SMTP/IMAP-logo's); de tweede stap kiest tussen Gmail, Outlook, SMTP/IMAP (disabled, nog geen backend) en Bokito-adres, waarna Gmail/Outlook direct de OAuth-stap voor die provider opent. Op `/settings/channels` staan naast de titel **Kanalen** de merken van Outlook, Gmail, WhatsApp, Slack en websitechat. De Communicatie-rail toont onder **Kanalen** alleen geconfigureerde accounts (mailbox/relay, enabled widget, WhatsApp, Slack) — geen vaste lege stubs zoals Team chat. Tests: `apps/api/tests/test_channel_registry.py`.
- **Inbound webhook-ops (aug 2026, geleerd tijdens live e2e-debug)**:
  - De Resend-webhook wijst naar `https://hooks.bokito.ai/api/inbound/resend`, NIET naar `app.bokito.ai`: Cloudflare (proxied records) blokkeert Svix-webhookdeliveries stilletjes (requests bereikten de origin nooit; een neutrale request-bin ontving ze direct). `hooks.bokito.ai` is een **DNS-only** A-record → VPS, met een eigen site block in de **host-Caddyfile** (`/etc/caddy/Caddyfile` op de VPS, niet de container-Caddyfile in de repo) dat alleen `/api/inbound/*` doorproxyt naar `localhost:8088` en al het andere 404't. Let's Encrypt-cert wordt automatisch door host-Caddy uitgegeven (kan omdat het record DNS-only is).
  - De `RESEND_API_KEY` moet **full access** zijn: een sending-only key krijgt 401 (`restricted_api_key`) op de Received Emails API, waardoor de inbound-handler 502't. Huidige key: `bokito-production-full`.
  - **PowerShell-valkuil**: `'value' | gh secret set NAME` voegt een UTF-8 BOM (`\ufeff`) toe aan het secret; die belandt via de deploy in `.env.prod` en laat elke Resend-call (ook transactionele mail) crashen met `UnicodeEncodeError` in httpx-headers. Altijd `gh secret set NAME --body 'value'` gebruiken. Zelfde geldt voor `echo script | ssh host bash -s` — gebruik `cmd /c "ssh ... < file"` met een BOM-loos bestand.
  - Uvicorn op prod logt geen access-regels; 4xx op de webhook is dus onzichtbaar in `docker logs`. Diagnose van webhookdelivery: endpoint tijdelijk naar een request-bin zetten via `update-webhook`, of handmatig een Svix-signed request afspelen (HMAC-SHA256 over `msg_id.timestamp.body` met het base64-gedecodeerde `whsec_`-secret).

## 32. Kennisbank document-ingestie (augustus 2026)

- **Document upload -> RAG**: `POST /api/workspace/docs/upload` (multipart, max 10MB, rate-limited) ingest PDF-, Word- (`.docx`), tekst-, markdown-, csv/tsv/rst/log/json-bestanden in de kennisbank. `app/services/doc_ingest.py` extraheert platte tekst (pypdf voor PDF, python-docx voor Word incl. tabellen; 415 bij niet-ondersteund type, 422 bij onleesbare/gescande bestanden zonder tekst). Het origineel wordt via de storage-backend bewaard; `source_file` en `source_url` staan in de frontmatter van de aangemaakte `WorkspaceDoc` (kind `doc`, pad `docs/uploads/<slug>.md`). Opnieuw uploaden van dezelfde bestandsnaam updatet het bestaande doc (geen duplicaten). Ingest loopt via `upsert_doc` -> `reindex_doc`, dus chunking + embeddings + hybrid search werken direct en agents kunnen de inhoud in drafts gebruiken (`search_index`/automatische RAG in `AgentLoop`).
- **UI**: `/knowledge` (WorkspaceDocs) heeft naast "nieuw markdown-doc" een upload-knop (Upload-icoon) die na ingest direct naar het geïndexeerde document navigeert.
- **Bestaande RAG-context**: workspace-RAG draait op `WorkspaceDoc`/`DocChunk` met embeddings als JSON (Python-cosine hybrid search, geen pgvector ANN); `BlueprintBlock` is gemigreerd naar `workspace_docs`. KB-collections (`/kb/*`) blijven een aparte metadata-overlay voor help centers zonder content-ingestie.

## 33. Widget CSAT (augustus 2026)

- **Trigger**: bij het sluiten van een widget-gesprek publiceert `publish_thread_update` naast het operator-only `thread`-event ook een visitor-veilig `conversation`-event (audience `all`, alleen `signal_id` + `status`) op `signal:{id}`. `append_thread_message` publiceert dit ook na `send_and_close`/`send_and_pending`. De widget toont bij `status: closed` eenmalig een inline 1-5 sterren-kaart (`.bk-csat`), met optioneel commentaarveld na het kiezen van een score; beoordeelde gesprekken worden in localStorage (`bokito_csat_rated`) onthouden.
- **Endpoint**: `POST /api/livechat/conversation/{id}/csat` (visitor-token, ownership-check via `_get_owned_conversation`, rate-limited). Slaat op als `Feedback(subject_type="signal", subject_id=<conversation>)`; opnieuw beoordelen updatet dezelfde rij (één rating per gesprek) en reset `processed` zodat de learning-batch hem opnieuw meeneemt.
- **Aggregatie**: `compute_eval_scores` schrijft een `EvalScore(metric="csat")` (7d-gemiddelde van signal-scoped Feedback, `sample_size` = aantal ratings). `cockpit_summary` levert `csat_score`/`csat_responses` (30d); Cockpit toont een "CSAT 30d"-StatCard en de operator ziet de rating als ster-badge in de thread-detailheader (`detail.csat` op `GET /api/signals/{id}`).

## 34. Publiek help center (augustus 2026)

- **Model**: geen aparte artikel-tabel — help-artikelen zijn gewone markdown-`WorkspaceDoc`s (kind `doc`) met frontmatter `published: true` + stabiele `slug`. Hetzelfde doc voedt agent-RAG en het publieke help center. Persona, memory, skills en logs kunnen niet gepubliceerd worden (400).
- **Publiceren**: `POST /api/workspace/docs/{id}/publish` (`{published: bool}`) zet/verwijdert de frontmatter-vlag, genereert een slug uit de titel (uniek per tenant, `-2`-suffix bij conflict) en schrijft een AuditEvent (`doc.publish`/`doc.unpublish`). In `/knowledge` staat een Publish/Published-knop (alleen kind `doc`) plus een link naar de publieke URL.
- **Publieke leeskant**: unauthenticated router `/api/help/{tenant_slug}` (index: tenantnaam + artikelen met slug/titel/description/updated_at) en `/api/help/{tenant_slug}/{slug}` (markdown-body), tenant-resolutie via `Tenant.slug` (zelfde patroon als livechat), rate-limited. SPA-routes `/help/:tenantSlug` en `/help/:tenantSlug/:articleSlug` staan buiten `ProtectedRoute` (`pages/HelpCenter.tsx`) met client-side zoeken; markdown-rendering via gedeeld `components/docs/MarkdownView.tsx` (geëxtraheerd uit WorkspaceDocs).
- **Frontmatter-behoud**: `upsert_doc` bewaart bestaande frontmatter wanneer opgeslagen content geen frontmatter-blok heeft (de editor round-tript alleen de body); bewerken van een gepubliceerd of geüpload doc wist `published`/`source_file` dus niet meer.
- **Widget Help-tab**: de chatwidget toont een Help-tab in de tabbar (tussen Messages en Tools) zodra de tenant gepubliceerde artikelen heeft. Na session start haalt de widget eenmalig `/api/help/{tenant_slug}` op (slug uit de session-payload); zonder artikelen blijft de tab verborgen. De tab bevat client-side zoeken over titel/description, een artikellijst en een inline artikelweergave (terug-knop) gerenderd met de widget-`MarkdownRenderer`, die nu ook `#`/`##`/`###`-koppen ondersteunt.
- **Deflection in widget-antwoorden**: als de assistent in een widget-gesprek RAG-hits gebruikt die naar gepubliceerde help-artikelen herleiden, voegt `livechat_stream` een "Related articles"-blok (max 2 markdown-links naar `/help/{tenant}/{slug}`) toe aan het definitieve antwoord — zowel gestreamd als gepersisteerd. Resolutie loopt via `app/services/help_articles.py` (`related_published_articles`); ongepubliceerde docs worden nooit gelinkt. Absolute URLs gebruiken de `public_app_url`-setting.

## 34b. Platform product-help / docs-platform (augustus 2026)

- **Bron**: `docs/product-help/{en,nl}/{sectie}/{slug}.md` — operator-artikelen in use-casevorm (korte stappen + optionele screenshots) in zes vaste secties, plus `surface-map.yaml` (routes/`code_globs` → slug). Nieuwe slugs t.o.v. de eerste ronde: `members`, `agent-runs`, `inbox-ai`, `help-centers`. Frontmatter vereist `title/intro/description/keywords/sort` (+ optioneel `related`). Sync (`python apps/api/scripts/dev/sync_product_help.py`) kopieert `en/`, `nl/` en `assets/` naar `apps/api/app/data/product_help/`. `tests/test_product_help.py` faalt op drift, pariteit, map-gaten en ontbrekende screenshots. Cursor-rule `.cursor/rules/product-help-docs.mdc` verplicht docs in dezelfde change als een significante feature; core-intent pre-flight stap 9 is Docs. User-facing how-to hoort hier, niet alleen in deze knowledge-file.
- **Publieke docs-site** `/docs`: landing met hero + zoek + sectiekaarten; artikelen op `/docs/{sectie}/{slug}` met sidebar-nav, ToC uit `##`-koppen (ankers via `lib/docs-toc.ts`, gedeeld met `MarkdownView`), vorige/volgende, "Copy as Markdown", EN/NL-switch (localStorage `bokito.docs.lang`), breadcrumb en SEO-meta (`lib/docs-seo.ts`). Platte `/docs/{slug}` redirect naar het sectiepad. `/docs/api` = Scalar API Reference (CDN-script) op het gefilterde schema. `html/body/#root` zijn `overflow: hidden` voor de dashboard-shell; publieke `/docs`, `/docs/api` en `/help/{tenant}` scrollen daarom in `DocsScrollShell` (`h-dvh` + inner `overflow-y-auto`). Screenshot-afbeeldingen in artikelen hebben een max-hoogte zodat lange pagina's bruikbaar blijven.
- **Operator-artikelen (aug 2026, tweede ronde):** Inbox AI documenteert de echte modi (`Suggest replies` / `Reply automatically` / `Off` per e-mail, websitechat, WhatsApp), antwoordtaal vs teamtaal, send-as, triage-zekerheid en per-mailbox override. Communication dekt Nieuwe chat / Nieuwe e-mail, Notitie vs reply, opgeslagen antwoorden, snooze. Channels dekt het ingebouwde Bokito-adres, sync, handtekening, routing. Developers dekt token-scopes (REST + MCP-groepen) en webhook-UI (test, deliveries, signing secret). Cockpit Usage dekt dag-/maandplafonds en BYOK. Agenda dekt wake-types (eenmalig, event, cron, interval, check-in, incoming). Knowledge dekt kinds (Voice/Memory/Skills/Docs/Check-ins/Daily notes). Persoonlijke schermen (Profiel, Notificaties, Mijn assistent) blijven `docs: none` in `surface-map.yaml`.
- **Operator-artikelen (aug 2026, derde ronde):** Settings-labels in docs volgen de zijbalk: **Email & messages** / **E-mail en berichten**, **Chat widget** / **Chatwidget**, **Setup guide** / **Setupgids**, **Providers & models** / **Providers en modellen**. Channels documenteert WhatsApp (Telefoonnummer-ID, Toegangstoken, webhook) en Slack (Bot-token, Signing secret, Events/Interactivity-URL, Goedkeuren/Weigeren). Communication dekt wachtrijen All/Mine/Unassigned, Labels en de Channels-sectie. Widget dekt Vooraf-formulier en de Help-module. Setupgids dekt de live checklist (inclusief Probeer de demo). MCP staat onder Integraties → Gekoppelde tools. Agent-runs-chips zijn All/Updates/Results/Decisions. Members dekt In afwachting, opnieuw versturen en link kopiëren. Webhooks documenteert ook `decision.resolved`, `agent.run_failed`, `platform_change.applied` en `spend.threshold_reached`.
- **Operator-artikelen (aug 2026, vierde ronde):** Agents documenteert **Nieuwe agent** (rollen Assistent/Communicatie/Bouwer/Lead), **Tools en toestemmingen**, **Maak lead agent**, Pauzeren/Wekken/Archiveren. Agenda documenteert **Nieuw schema**, Hook-URL + `X-Bokito-Secret`, Testping. Cockpit documenteert echte overzichtskaarten en **Cijfer toevoegen**. Widget documenteert twee snippets en **Behandelende agent**. Kennis documenteert Nieuw document/Upload/Zoeken/Publiceren-badge. Helpcentrum documenteert Collecties + Uploaden. Models documenteert de kaart **Bokito AI**, **Provider toevoegen** en **Voorgestelde modellen inschakelen**. Projecten documenteert Orchestratie (wake-frequentie, autonomiemodus, HITL). Channels documenteert routingregels (Afzenderdomein / Onderwerp bevat / Mailbox). MCP documenteert **Tool koppelen** + Test connection. Rate limits dekken Agenda-hooks (60/min) en widget-sessies (30/min). Signals API: tags worden Labels, kanaal is altijd `api`. Inbox AI-conceptkaarten gebruiken Versturen/Bewerken/Escaleren.
- **Operator-artikelen (aug 2026, vijfde ronde):** Communication documenteert snooze-presets (**1 hour** / **1 uur** … **Until the customer replies** / **Tot de klant antwoordt**) plus **Send and wait**. Contacts documenteert **Create contact** / **Contact aanmaken**, **Write email** / **E-mail schrijven**, Profielvelden en status **Approve/Block**. Cockpit Activiteit documenteert **Headlines**/**Full log** (**Kopregels**/**Volledig log**), **Filter events...**, **Auto-follow**. Cockpit Verbruik documenteert **Budget (platform keys)** / **Budget (platformsleutels)** en lege CSAT. Developers-authenticatie gebruikt de echte groepen **Workspace API** / **What agents and apps may do** (niet REST/MCP). Tour documenteert paletgroepen en lege-query-acties (**New chat**, **Connect mailbox**, **Open help**). Channels documenteert Outlook-**Folders** (**Select folders to sync**) en **Channel routing** / **Add route**. Agents documenteert **My assistant** (**Assistant name**, **Default chat**, **Open a new chat**). Projecten documenteert **I use GitHub** vs **Set it up for me** (NL **Doe het voor mij**). Setupgids documenteert **Your language** als persoonlijke voorkeur, brandingvelden en **Require 2FA for all members**.
- **Operator-artikelen (aug 2026, zesde ronde):** Inbox AI documenteert **The approving teammate** / **The AI agent**, composer **Send as:** **You**, **Save**, en triage **permissive**/**balanced**/**strict**. Widget documenteert **Welcome title**, **Accent**, **Widget icon**, **Tone**/**Do**/**Do not**, **From**/**Until**/**Offline message**. Integraties filtert met **All integrations** / **Tools for agents** en **Disconnect**. Members documenteert tabs **All**/**Active**/**Pending**, veld **Role**, en de mail-niet-geconfigureerd-hint. Govern documenteert de kaart **How much agents can do**, Accept/Reject-bevestigingen, en audit **Open thread**/**Open run**. Agenda documenteert **Name**/**When**/**Save**/**Edit schedule**. Communication documenteert de split **Customer**/**Agents**, **Outbound**, en geleerde regels **Always do this**. Decisions documenteert de echte opties (**Approve**/**Reject**/**Defer**/**Escalate**) plus **Looks right**/**Correct this**. Knowledge documenteert **Create first document** en **Delete document**. Agent-runs legt uit dat die onder **Agents** zitten en wat **Updates**/**Results** betekenen.
- **API** (`routers/product_help.py`, rate-limited, unauthenticated): `GET /api/docs` (nav-boom `sections[]` + platte `articles[]`), `GET /api/docs/search?q=&lang=` (keyword-only, geen embeddings publiek), `GET /api/docs/{slug}` (artikel), `GET /api/docs/{slug}.md` (raw markdown met frontmatter), `GET /api/docs/assets/{path}` (alleen `.png`/`.webp` onder `assets/`, geen path-escape), `GET /api/docs/sitemap.xml`, `GET /api/docs/openapi.json` (gefilterd publiek schema via `services/public_openapi.py`: tags public-api/mcp/help-center, alleen gerefereerde component-schema's, bearer-securityscheme). FastAPI-root serveert `/llms.txt` (index met absolute raw-md-URL's) en `/llms-full.txt` (alle en-artikelen geconcateneerd). `MarkdownView` rendert `![alt](/api/docs/assets/...)` plus italic bijschrift.
- **Public API-verrijking**: `routers/public_api.py` heeft Pydantic-responsemodellen (`SignalSummary`, `SignalListResponse`, `SignalDetail`, `SignalMessageOut`) plus summaries/docstrings zodat de Scalar-reference volledig is.
- **In-app `/learn`**: zelfde nav-data (secties), accepteert elke slug; artikelpagina toont API-`related`-artikelen plus de gecureerde app-quicklinks (`PAGE_GUIDE_RELATED`) voor de oorspronkelijke 13 banner-slugs.
- **Feedback-hook**: "Was dit nuttig?" duimen op `/learn`- en `/docs`-artikelen (`components/docs/ArticleFeedback.tsx`) → `POST /api/learning/feedback` met `subject_type=product_help`, `subject_id={slug}`; alleen zichtbaar met sessie (publiek anoniem ziet niets).
- **Assistant**: `search_product_help` tool (ook in suggest-mode) plus automatische top-hits in de system prompt bij interne/operator-turns; hybrid (embeddings + keywords) blijft alleen voor assistant-RAG. Citeer `/learn/{slug}` in de app of `/docs/{sectie}/{slug}` publiek.
- **Niet**: `docs/company/` of `BOKITO_KNOWLEDGE.md` publiceren; tenant-helpcenter (`/help/{tenant}`) blijft klantartikelen; `/api/kb` wordt niet uitgebreid. `docs.bokito.ai` wordt later een host-mapping naar dezelfde SPA-routes (geen aparte build).

## 35. Outbound webhooks, Public API en CRM-companies (augustus 2026)

- **Outbound webhooks**: `WebhookEndpoint` (URL, events-lijst of `*`, Fernet-versleuteld secret, actief-vlag) + `WebhookDelivery` (payload, status pending/success/failed, attempts, laatste fout) per tenant. `emit_webhook_event` matcht actieve endpoints op event, maakt deliveries aan en enqueued ARQ-taken (`deliver_webhook_job`, inline fallback zonder Redis). `perform_delivery` POST met JSON-payload en `X-Bokito-Signature: t=<ts>,v1=<hmac-sha256(secret, ts.body)>`; retries met backoff, deliveries worden per endpoint geprund (retentie 100).
- **Events**: `signal.created` (inbound ingest, nieuwe widget-conversatie, public API-create), `signal.closed` (thread-patch, bulk-close, send_and_close, close_thread-tool, inbox-rule auto_close), `decision.created`, `decision.resolved`. Beheer via `/api/webhooks` (CRUD, test-delivery, delivery-historie; admin-rol, audit-events) en de Settings-pagina `/settings/developers` (secret wordt eenmalig getoond bij aanmaken).
- **Public API v1**: `/api/public/v1/signals` (list/get/create) geauthenticeerd met tenant-`ApiToken` (Bearer, SHA-256-hash, zelfde tokens als MCP-toegang uit Govern), rate-limited. Create accepteert channel/subject/body/contact en emit `signal.created`. Documentatie + tokenbeheer-link staan op `/settings/developers`.
- **CRM companies**: `Company`-model (naam, domein, website, notes) met `Contact.company_id`-FK. Bij contactcreatie (handmatig, inbound ingest, signals-service) wordt het e-maildomein herleid: gratis providers (gmail/outlook/etc., lijst in `app/services/companies.py`) krijgen geen company; zakelijke domeinen krijgen get-or-create op domein met genormaliseerde naam. `POST /api/companies/backfill` linkt bestaande contacten retroactief. Endpoints: `/api/companies` (list+search+contact-counts), detail (incl. contacten + recente threads over alle contacten), patch, delete (contacten blijven, worden ontkoppeld). UI: Contacts-pagina heeft een People/Companies-toggle, companies-tabel met "Link contacts"-backfill-knop, en een company-detailpagina (`/contacts/companies/:id`) met bewerkbare velden, gekoppelde contacten en gesprekken; contact-detail linkt naar het company-record.
- **NL-localisatie**: alle acht i18n-namespaces (`common`, `nav`, `profile`, `workspace`, `communication`, `workspaces`, `aios`, `govern`) hebben volledige en/nl key-pariteit; `aios` en `govern` zijn in augustus 2026 volledig naar het Nederlands vertaald en geregistreerd in `src/i18n/index.ts`.
- **Dashboard code-splitting**: alle pagina's in `App.tsx` laden via `React.lazy` (één `Suspense`-fallback rond de router); shell-componenten (AppShell, MessagesHub, SettingsLayout) blijven eager. `vite.config.ts` splitst daarnaast vendor-chunks via `manualChunks`: `react-vendor`, `sentry`, `i18n-vendor` en `locales` (de statische vertaal-JSONs). Entry-chunk daardoor van 1.455 kB naar 400 kB minified; route-chunks laden on demand.

## 36. Dashboard UX polish — cross-feature links en i18n (augustus 2026)

Iteratieve UI/UX-ronde op het dashboard (NL-locale) richt zich op samenhang tussen features, minder dode einden en volledige vertaling:

- **Agent context paneel** (`AgentContextPanel`): loading/tools/integrations-labels i18n; `timeAgo` hergebruikt contactpanel-keys; Govern-link wijst naar `/settings/govern?tab=policy`; nieuwe link naar `/settings/integrations`.
- **Project budget** (`ProjectDetail`, `ProjectBudgetBar`): budget-titels en daglimiet-labels i18n (`projects.detail.*`).
- **Agenda automations** (`AutomationsPanel`): loading-toast en step-added meldingen i18n.
- **Signature editor** (`SignatureEditor`): volledige dialoog i18n (tabs, toolbar, templates, hints) via `communication.signatureEditor.*`.
- **Shell navigatie**: collapse/expand/close navigation aria-labels i18n (`topbar.collapseNavigation`, `topbar.closeNavigation`); route lazy-load spinner en workspace-loading tekst via `app.loading` / `app.loadingWorkspaces`.
- **Integraties verbonden** (`IntegrationsConnected`): gefilterde tabs (bijv. `?kind=mcp`) tonen nu sectie-specifieke lege staten i.p.v. de globale "geen integraties"-kaart; MCP-leegstaat heeft Marketplace + **Open Govern-beleid** links.
- **Developers** (`DeveloperSettings`): webhooks-leegstaat linkt naar Communicatie-instellingen; Workspace-API-sectie heeft knop **Open Govern-beleid**; MCP test-knop aria-label i18n (`integrations.mcp.servers.testConnection`).

### Ronde 2 (iteraties 72–83, augustus 2026)

- **DirectCommunication**: lege staat en delete-foutmelding i18n (`directChat.noAssistantConfigured`, `configureAssistant`, `deleteFailed`).
- **ReplyComposer**: laden opgeslagen antwoorden vertaald (`composer.loadingSavedReplies`).
- **ComposeEmailModal**: `(No subject)` via `compose.noSubject`.
- **AutomationsPanel**: stap-agent fallback via bestaande `agendaPage.agent` / `noAgentsOption`.
- **ShellSidebar**: merklabel "Control" → `topbar.controlBrand` (NL: Besturing).
- **LiveWorkLog** (`AiAgentDetail` run-stream): volledige i18n via `workforce.runLog.*`.
- **IntegrationsConnected**: disconnect-bevestigingen/toasts zonder Engelse `defaultValue`-fallbacks.
- **Cockpit agenda-leeg**: tweede link **Plan een geplande run** → `/agenda?view=list`.
- **Knowledge**: bij geselecteerde maar niet-geopende docs: cross-links naar setupgids en agents onder `selectHint`.
- **AgendaPage**: tab-weergave gesynchroniseerd met URL (`?view=week|list|automations`) voor deelbare links vanuit Cockpit.

### Ronde 3 (iteraties 84–95, augustus 2026)

- **ContactsPage**: tab Mensen/Bedrijven gesynchroniseerd met URL (`?view=people|companies`) — zelfde patroon als Agenda.
- **AiAgentDetail** / **ComposeEmailModal** / **Communication** / **GovernPage** / **AiAgents**: Engelse `defaultValue`-fallbacks verwijderd waar vertaalsleutels bestaan; Govern-titel gebruikt `govern.title`; tool-override toasts via `allowances.toolOverrideSaved` / `toolOverrideCleared`.
- **AgentToolsPicker**: link naar **Integraties** naast Govern-beleid op agentdetail.
- **HelpCentersSettings**: lege documentenlijst linkt naar **Kennis** (naast bestaande collectie-leegstaat).
- **ModelsSettings**: lege provider- en modellenlijsten krijgen cross-links naar setupgids, Agents, Verbruik en Govern.
- **NotificationSettings**: budgetwaarschuwingen gekoppeld aan **Verbruik** (`/cockpit/usage`); rij-labels zonder Engelse fallbacks.
- **MyAssistantSettings**: link naar **Providers en modellen** naast Communicatie openen.

### Ronde 4 (iteraties 96–107, augustus 2026)

- **ActivityPage (NL)**: filter-placeholder en event-telling vertaald (`Filter gebeurtenissen...`, `gebeurtenissen`).
- **Agentstatus (NL)**: `Standby` → `Stand-by` in de agentlijst.
- **Messages hub**: Engelse `defaultValue`-fallbacks verwijderd uit `MessagesHubNav`, `ThreadListQuickFilters`, `SidebarCustomizeDialog`, `MessagesHub`; `support.customize.anchored` toegevoegd (NL: Vastgezet).
- **Syncstatus**: leegestaat legde uit dat het Bokito-adres al mail ontvangt (paneel is in aug 2026 vervangen door de checks op de kanaalrij).
- **Saved replies / Automatiseringsregels**: lege staten linken naar Communicatie.
- **Projects**: lege staat koppelt ook naar Kennis en Communicatie.
- **Agents (leads)**: lege lead-sectie linkt naar Projecten.
- **ContactPanel**: lege “eerdere gesprekken” linkt naar Contacten; threadstatus via `threadStatusLabel`.
- **Chatwidget install**: cross-links naar Communicatie, Helpcentrum en Setupgids.

### Ronde 5 (iteraties 108–119, augustus 2026)

- **Agenda automations**: Engelse aria-labels (`Activate Heartbeat`) → NL (`Activeer Heartbeat`); “Open” run-link i18n; bugfix — `tasks.map((t) =>` schaduwde de i18n-functie `t`; run-status via `workLogStatusLabel`.
- **Agenda lijst-leeg**: cross-links naar Communicatie en Kennis (naast Agents/Setup).
- **Channel routing (Inbox AI)**: lege staat linkt altijd naar Agents + Communicatie.
- **Routing rules**: lege staat linkt naar Communicatie + Leden.
- **Profile 2FA**: Engelse `defaultValue`-fallbacks verwijderd (NL-keys bestonden al).
- **Branding**: onder chatwidget-kaart links naar Communicatie, Helpcentrum en Widget installeren.
- **Nieuwe chat**: bij geen agents ook link naar Kennis.
- **IntegrationsConnected**: load-error toont de echte fouttekst zonder Engelse fallback-hack.

### Ronde 6 — motion / feel (iteraties 120–141, augustus 2026)

Gedeelde motion-taal in `index.css` + primitives, zodat het dashboard tikkender en moderner aanvoelt zonder extra feature-pagina's:

- **Tokens**: `btn-sheen`, `stagger-in`, `count-pop`, `pulse-dot`, `row-interactive`, `skeleton-shimmer`, `link-draw`; `::selection` in accent; reduced-motion zet alle decoratieve animaties uit.
- **Primitives**: primary/AI-knoppen sheen + steviger press; inputs focus-glow in teal (was paars); switches spring + checked-halo; tabs actieve schaduw; skeleton sweep i.p.v. pulse; interactive cards accent-rand; dialogs meer blur; empty-state icoon met zachte ring.
- **Shell**: actieve rail met inset-streep; badge pop-in; theme-toggle schuift; connected-dot pulse; sidebar width 200ms; mobile drawer slide-in; command palette fade+pop; zoekchip en avatar reageren op hover.
- **Surfaces**: Cockpit-statkaarten stagger + number pop; attention-rijen pulse; threadrijen ink-bar + unread pulse; agents/notifications/agenda-triggers `row-interactive`; login gebruikt `app-atmosphere`.

### Ronde 7 — Open als klantinbox + locale-samenhang (iteraties 142–151, augustus 2026)

- **Open-inbox**: toont alleen klantgesprekken; interne agent-runs verdwijnen uit Open en openen onder Agent-runs als je een interne thread-URL opent.
- **Onderwerpen**: bekende agentonderwerpen (`Reply to customer message`, `Daily platform scan`) worden vertaald in de threadlijst, de kop en het agentpaneel.
- **Contactnamen**: `Website visitor` / `visitor@web` / `cust_…` worden Websitebezoeker; E-mail schrijven verdwijnt bij placeholder-adressen.
- **Samenhang**: nieuwe e-mail, Communicatie-CTA's, tour, onboarding, command palette en agent-recente-threads landen op Open (of Agent-runs voor intern werk).
- **Datums/tijden**: Agenda, Cockpit, Activiteit en project-runs volgen de in-app taal (NL: ma/di + 14:57), niet de browserlocale.
- **Govern**: tab "Openstaande concepten"; Cockpit-agenda toont Herhalend i.p.v. Interval; repo-status Ready → Klaar; "Agent passport update" → Agenttoegang bijgewerkt.

### Ronde 8 — Jargon eruit, schermen aan elkaar (iteraties 152–161, augustus 2026)

- **Verbruik**: model-IDs worden leesbare namen (Claude Sonnet 4); bedragen volgen de in-app taal; modelrijen linken naar Providers en modellen. BYOK heet Eigen sleutel (geen kosten).
- **Agents**: het modelkaartje toont geen ruwe slug meer; kosten staan in gewone taal (per miljoen tokens). Toestemmingen zoals `platform:read` worden "Platform bekijken" op agentdetail, in het agentpaneel en op Govern-toegang. Run-onderwerpen `PO wake` / `PO heartbeat` / `Daily platform scan` worden Lead-copy op runs, Cockpit-agenda en Agenda. Wekken i.p.v. Weken.
- **Gesprekken**: recente threads met prefix `Assist:` worden Hulp: plus het vertaalde onderwerp.
- **Helpcentrum**: zonder collectie wijst het documentenpaneel naar het formulier en naar Kennis, met een voorbeeldnaam.
- **Modellen / Setup / Cockpit**: "Open Agents" en presets zijn Nederlandse CTAs; KPI-jargon is Eigen cijfers / Cijfer toevoegen. Govern-audit zegt Gesprek openen i.p.v. Open thread.

## 26. Agent-toewijzing, lead agent en kanaalzichtbaarheid (augustus 2026)

Eén mentaal model voor "wie behandelt dit": **binding op het item wint, anders de lead agent.**

- **Lead agent (`Agent.is_lead`)**: elke tenant heeft precies één actieve lead agent (gewone company agent met een label, geen aparte rol). Bootstrap zet de seed-"Assistant" als lead; startup-backfill (`ensure_lead_agents` in `init_db`) promoveert de oudste actieve company assistant voor bestaande tenants. `PATCH /api/workforce/agents/{id}/lead` (owner/admin) verplaatst het label atomair (`set_lead_agent`). De lead kan niet gearchiveerd worden (409) totdat het label is overgedragen. UI: `/agents` is één kaartgrid (`filterLibraryAgents` + `sortAgentsForLibrary`); de lead krijgt alleen een **Lead**-badge op de kaart, geen aparte Leads-sectie. Lead wissel je op agentdetail via "Maak lead agent".
- **Agent library-filter:** `filterLibraryAgents` toont company-agents inclusief orchestrators/assistants; personal agents en role `communication` blijven buiten de bibliotheek. De oude split Leads vs Agents (orchestrator-rol vs workers) is verwijderd.
- **Agentstatus (aug 2026):** `runtime_status=standby` is idle/klaar (label **Klaar** / **Ready**), niet gepauzeerd. `active` + lopende run is **Bezig**. Operator-pauze is `sleeping` + `is_active=false` (**Pauze**). `PATCH /agents/{id}/status` zet `is_active` alleen uit bij `sleeping`/`error`. Inbox-badges tellen geen interne of assistant-threads. Cockpit "Gesprekken 7d" telt alleen externe klantthreads (geen demo). Operator-reply zet `has_unread=false`. Fixture-domeinen (`example.com`) worden geen bedrijf.
- **Inbound threading:** `create_inbound_signal` hergebruikt een open/pending thread voor dezelfde widget/chat-contact (e-mail: zelfde contact + subject). Een klant-follow-up start geen tweede gesprek. `folder=inbox` sluit `internal` en `assistant` uit (zelfde als Open in de UI).
- **Thread-preview:** lijst toont het laatste echte bericht; `[mock]`-agenttekst wordt overgeslagen. WhatsApp en `api` zitten in `EXTERNAL_CHANNELS` (Cockpit-volume, inboxfilters).
- **Human reply vs suggestie:** een operator-reply (`outbound`) zet open thread-beslissingen zonder `platform_change_id` op `deferred` (`human_replied`), zodat suggestiekaarten verdwijnen nadat je zelf hebt geantwoord. Een interne notitie laat `has_unread` staan. `POST /signals/{id}/invoke-agent` valt terug op de lead agent als `agent_id` ontbreekt. Cockpit `open_decisions` telt alleen externe, niet-gesloten klantthreads. `learning_autonomy_rate` volgt de live `autonomy_rate_pct`.
- **Gone thread:** een 404 op threaddetail (verwijderde demo-URL) stuurt Communication terug naar de actieve wachtrij.
- **Fallback-keten**: alle voormalige impliciete assistant-fallbacks (routing, widgetnaam, triggers, orchestration runner, execution, threadresolutie) lopen via `get_lead_agent`. Uitzondering: auto-reply op interne threads gebruikt `fallback_to_lead=False` zodat de lead niet elk kaal intern gesprek beantwoordt.
- **Agent per kanaal-item**: `ChannelBinding` (contact > channel_account > kanaal, dan prioriteit) is het enige beslispunt. `AgentBindingPicker` (dashboard) staat op mailboxrijen (Email & messages), WhatsApp/Slack-kaarten en de widget-instellingen; default-optie is "Lead agent (standaard)" (= binding verwijderen). Account-scoped bindings krijgen priority 20, kanaal-breed 10.
- **Agents per project (`project_agents`)**: roster-tabel (project_id, agent_id, `is_default`, uniek per paar; Alembic 015). API: `GET/POST /api/workforce/projects/{id}/agents`, `PATCH/DELETE /{agent_id}` (default-vlag / verwijderen); maximaal één default per project. Threadresolutie voor threads met `project_id`: project-default-agent → `po_agent_id` (projectlead) → lead agent. Projectenlijst levert `agents`-chips mee; projectdetail heeft een Agents-roster (toevoegen/verwijderen/default via ster-icoon). Workstream-runs erven `ws.project_id` in `create_agent_task` zodat runs hun project behouden.
- **Kanaalzichtbaarheid per user**: `ChannelAccount.settings_json.visibility = {"mode": "everyone"|"selected", "user_ids": [...]}` (default everyone; owners/admins/staff zien altijd alles). Service `channel_visibility.py` (`visible_channel_account_ids` → None voor admins, anders een set). Harde ACL voor members: threadlijst en badge-counts filteren op `Signal.channel_account_id` (threads zonder account blijven zichtbaar), threaddetail 404't, en `GET /api/channels/accounts` + `GET /api/email/accounts` tonen alleen zichtbare accounts. Instellen via `PATCH /api/channels/accounts/{id}/visibility` (owner/admin). UI: `ChannelVisibilityPicker` (Iedereen / Geselecteerde members + member-multiselect) als kolom op mailboxen en op WhatsApp/Slack-kaarten. `GET /api/signals/members` levert nu ook `uuid` per member.
- **Tests**: `apps/api/tests/test_agent_assignment.py` dekt lead-transfer + archive-guard, project-agents CRUD/default/threadrouting en de visibility-ACL (member vs owner).

## 37. Complexe communicatiescenario's (Pass 12, augustus 2026)

Scenario-pass over e-mail-, webchat- en beslisflows; belangrijkste gedragsregels die nu gelden:

- **Suggestiekaarten stapelen niet:** een nieuwe `create_decision_request` met dezelfde titel op hetzelfde signaal zet de oudere `awaiting_human`-kaart op `deferred` (`chosen_option_id="superseded"`) en markeert de gekoppelde belnotificatie als gelezen. Ook `_defer_open_reply_suggestions` (menselijke reply, sluiten/snoozen/spam, bulk-sluiten) ruimt de belnotificatie op en dekt zowel "Suggested reply" als "Reply to customer message".
- **Goedkeuren dat faalt, liegt niet:** als de tool-uitvoering achter een goedgekeurde beslissing een error teruggeeft, gooit `resolve_decision` een `DecisionActionError` (422), zet de kaart terug op `awaiting_human` en audit met `outcome="error"` — de operator kan opnieuw proberen.
- **Heropenen op klantantwoord:** `ingest_inbound` (webhook/sync-pad) matcht op provider-`thread_external_id` ongeacht status; een klantreply heropent een gesloten of gesnoozde thread (spam blijft geparkeerd, backfill van oudere mail heropent niet). Hetzelfde geldt voor het handmatige inbound-pad (`_open_thread_for_inbound`, external-id match negeert status behalve spam).
- **Cross-channel contacthistorie:** `GET /api/channels/contacts/{id}/threads` matcht naast `contact_id` en `contact_email` ook sibling-contactrijen met hetzelfde adres op andere kanalen (widgetbezoeker die ook mailt = één gecombineerde historie in het contactpaneel).
- **CC end-to-end:** Gmail- en Graph-syncparsers leveren het CC-veld mee (`cc` in message-metadata); de timeline toont de CC-regel op inbound mail en de reply-composer seedt het CC-veld met de klant-CC zodra de operator CC/BCC opent (reply-all, nooit automatisch).
- **Widget-takeover live:** het bezoekersveilige `conversation`-gateway-event draagt `ai_paused`; de widget toont/verbergt de "medewerker helpt je verder"-banner live bij overnemen/teruggeven en verbergt hem bij sluiten (naast de CSAT-prompt). Een al zichtbare "human requested"-banner behoudt zijn specifiekere tekst.
- **Mentions bij notitie-edit:** het bewerken van een interne notitie waarbij een `@[Naam](user:id)` wordt toegevoegd, notificeert alleen de nieuw toegevoegde mentions (geen duplicaten bij herformuleren).
- **Bezorgfouten benoemd:** een mislukte outbound toont naast "Niet afgeleverd" een vertaalde reden (mailboxkoppeling verlopen / geen mailbox / netwerkfout / geen ontvanger); de ruwe statuscode zit in de tooltip.
- **Nieuwe conversatie:** een getypt e-mailadres dat nog geen contact is dead-endt niet meer — de To-picker biedt "E-mail {adres}" dat naar de compose-flow springt. Nieuwe outbound compose koppelt de ontvanger direct als Contact (`contact_id` op het Signal); inbound replies backfillen `contact_id` op oudere threads zonder link.
- **Widgetstrings volledig gelokaliseerd:** alle resterende hardcoded Engelse widgetteksten (verwijderbevestiging, exportleeg, uploadfouten, microfoon, spraak) lopen via de `#chrome` EN/NL-tabel.
- **Tests:** `test_channels.py` (cross-channel historie, heropenen op reply), `test_mentions_notifications.py` (mention bij edit), `test_email_fidelity.py` (CC-parsers), `test_platform_tools.py` (supersede), `test_inbox_ops.py` (bulk close defer), `test_mailbox_integrity.py` (compose-contactlink).
