...
- **Communication reply fallback (2026-08):** Email threads without `channel_account_id` still send when the workspace has an enabled mailbox. `deliver_outbound` binds the first sendable account (Bokito, then Gmail/Outlook/mock). The composer blocks only when **no** sendable mailbox exists — not when the thread itself has a null `emailConnectionId`. Website chat (`widget` / `webchat` / `livechat`) stays on the chat surface even if the visitor left an email. Invoice/quote threads (`threadLooksFinancial`) show **Open bookkeeping**, a **Billing** label chip, a list Invoice badge, and a post-send Bookkeeping toast. Closed/spam composers keep notes and offer **Reopen**. **Quote** works on chat and email. **R** on a closed thread reopens then focuses the composer. The command palette has **Open bookkeeping**. Widget visitors with a placeholder address get an ask-for-email hint. Contacts search honors `?q=`. Empty inbox no longer requires a mailbox before New chat / widget setup.
...
- **Keyboard is shared:** `useInboxListShortcuts` drives Communication and Assistant/agent chats. J/K move and scroll the active row into view. Esc leaves the thread (back to the list). H snoozes 1 hour (undo reopens). X toggles bulk select. R refuses closed/spam. Shift+A opens the assignee picker. N starts a new chat. L copies the canonical link. `#` copies the thread id. 1–4 switch All / Needs reply / Unread / Pinned. `/` works even if compose is open, unless focus is inside the dialog. New conversation Esc returns to the last inbox queue.
- **AND list flags:** `GET /api/signals` accepts `unread=1`, `needs_reply=1`, `pinned=1` on top of the current `view`/`folder`. `needs_reply` is open + (unread or last inbound `user_message` newer than last outbound reply). Human outbound replies are `user_message`/`outbound`, so they must count as answered. The dashboard sends the flags from the Unread / Needs reply / Pinned chips so they compose with Open/Mine/etc. Live gateway upserts honor the same flags in `threadMatchesFilters`.
...
- **Bulk:** Assign to me, Pin, and Unpin sit on the selection bar. Command palette adds Closed, Spam, Agent runs, Assistant, Needs reply, and searches numeric/UUID ids from the first character.
...
- **Shareable chips:** `?filter=needsReply|unread|pinned` (alias `needs_reply`) syncs with the quick-filter chips. Search and label filters persist when switching queues. List scroll position is restored per leaf in `sessionStorage`. The timeline no longer snaps to the bottom on every new message; scrolled-up readers see an **N new** jump pill.
...
- **Sync status-paneel**: `SyncStatusPanel` is gemonteerd op de mailbox-instellingenpagina (onder de connected-inboxes tabel) en toont per mailbox status, laatste sync, fouten en per-map berichtaantallen, met een "Sync now"-knop.
- **Mailbox-tabel (`/settings/channels`, 2026-08):** Connected inboxes is één `SettingsSection` (geen los zwevende tabel in een tweede card). Kolommen: Inbox (providerlogo, naam, e-mail, Primary-badge), Sync, History, Status (+ laatste sync), Actions. Per-rij acties (sync, folders, signature, routing, make primary, remove) zitten in een knopgroep met overflow-menu; bij foutstatus staat Reconnect als zichtbare split-actie. "Sync now" en "Connect mailbox" staan in de sectieheader.
...
- **CRM:** `Contact`-model heeft `company`, `title`, `phone`, `notes`. Contactpaneel in thread-context als tab naast Orchestration (`AgentThreadPanel`): contactkaart, bewerkbare notities, approve/block, eerdere conversaties. Contacts-pagina op `/contacts` (zoeken, lijst met kanaal/bedrijf/laatst gezien/#threads) en contactdetail op `/contacts/:contactId`. De lijst vult `last_seen` en thread counts vanuit Communication wanneer de CRM-velden leeg zijn (`enrichContactsFromThreads`: e-mail/id, unieke echte namen — geen generieke websitebezoekers). Avatars gebruiken het domein-favicon (`DomainFavicon` via Google S2, initials als fallback) op de contactenlijst, company-lijst, contactdetail, ContactPanel en threadlijst; de kanaalkolom toont een Lucide-icoon (`ChannelLabel`: e-mail, widget, Slack, WhatsApp, intern). Endpoints: `GET/PATCH /api/channels/contacts/:id`, `GET /api/channels/contacts/:id/threads`; signal thread-payload bevat `contact_id`.
...
- **Built-in tenant e-mailadres (aug 2026)**: elke tenant krijgt automatisch een zero-setup mailbox `{slug}-{token}@in.bokito.ai` (ChannelAccount `provider="bokito"`, geen OAuth-credentials). Aangemaakt in `bootstrap_tenant`; voor bestaande tenants lazily via `GET /api/email/bokito-address` (`services/bokito_mailbox.py`, domein configureerbaar via `BOKITO_INBOUND_DOMAIN`). **Inbound**: Resend Receiving op domein `in.bokito.ai` (eu-west-1, sending+receiving, DKIM/SPF/MX via Cloudflare) → Svix-signed `email.received`-webhook naar `POST /api/inbound/resend` (`routers/inbound.py`; secret `RESEND_WEBHOOK_SECRET`, GitHub-secret + deploy-upsert in `.env.staging`/`.env.prod`). De webhook is metadata-only: body/headers worden opgehaald via `GET https://api.resend.com/emails/receiving/{email_id}`, attachments via `.../attachments` (gedownload en opgeslagen via de storage-backend). Threading via RFC-headers: `thread_external_id` = eerste Message-ID uit References (of eigen Message-ID); `rfc_message_id`/`references` in message-metadata zodat `_reply_context` replies correct threadt. Mail aan onbekende adressen op het catch-all-domein wordt gedropt. **Outbound**: `send_via_provider` heeft een `bokito`-branch die via de Resend Send API verstuurt (platform-`RESEND_API_KEY`, From = tenant-adres, In-Reply-To/References-headers, attachments base64). **UI**: `/settings/channels` toont een "Your Bokito address"-kaart (copy-knop), het adres staat als rij in de inbox-tabel (status altijd `connected`, "Instant"/geen backfill, geen Sync/Remove; wel Signature/Routing/Primary), de connect-dialog heeft een disabled "SMTP / IMAP — Coming soon"-tegel, en de Setup Hub Communication-pijler noemt het built-in adres (logo-slug `bokito` in brand-assets). Verwijderen van het built-in account is server-side geblokkeerd (pauzeren kan). Tests: `apps/api/tests/test_bokito_mailbox.py`.
...
- **Syncstatus**: leegestaat legt uit dat het Bokito-adres al mail ontvangt; OAuth-mailboxen zijn optioneel voor map-sync.
