...
- **Integratie-modules (2026-08)**: een **module** is een bedrijfscontract (schema, verbs, skill, setup); een **connector** is het pakket eronder (KING, Moneybird). Inbox/GitHub zijn geen modules. Registry is `ModuleSpec` in `apps/api/app/modules/catalog.py` (accounting `available`; banking, investing, documents `coming_soon` met schema-stubs, geen adapters). `GET /integrations/providers` levert een `modules`-array met verbs, `verb_labels`, `needs_when`, `setup_steps`, `capability_summary`, `setup_path`, `connected` en `tenant_status`. Module-home: `/settings/modules/:slug` (`ModuleSetupPage`) — capability in mensentaal, writes-als-beslissing, connectors + coming-soon, hergebruikt `ApplicationHubDialog`; `?connect=` opent een pakket. Marketplace-sectiekop en **Set up** gaan naar die pagina; coming-soon modules blijven zichtbaar als de kind-filter niet `all` is. Connected groepeert accounting onder **Boekhouding** (link naar module-home) met administratie-chips; overige MCP onder **Overige**. Setup Hub (Intelligence) heeft één regel + link “Modules die bij dit werk passen”. Frontend-fallback: `FALLBACK_MODULES` in `integration-modules.ts` (zelfde slugs, geen tweede catalogus).
- **Agent ziet modules ook zonder connectie**: tenant-snapshot heeft altijd een `Modules:`-blok (`accounting — not connected — use when … Setup: /settings/modules/accounting`; na connectie `use accounting_*`). Coming-soon in één regel “prepared, not connectable”. Tools: `list_modules` (ungated catalog + tenant-status) en `recommend_module` (DecisionRequest “Enable Accounting?” met Connect now → module-setup; coming_soon maakt geen connect-beslissing). Unconnected available modules krijgen een kort setup-playbook in het system prompt; de volle skill blijft connection-gated. `accounting_*` `no_connection` wijst naar `/settings/modules/accounting`. Heartbeat-seed + bestaande heartbeats krijgen de `recommend_module`-regel. Setup-chat (tour, onboarding-checklist, Setup Hub) opent `/communication/new?kind=company` naar de bedrijfs-Assistant. `ONBOARDING_SYSTEM_PROMPT`: na Communication, als het werk boekhouden/facturen/BTW/bank raakt → `list_modules` + `recommend_module`.
...
- Meertaligheid: de dashboardshell ondersteunt runtime taalwisseling met `i18next` (`en`/`nl`). Platformstandaard is Nederlands via `PLATFORM_DEFAULT_LANGUAGE` (API, runtime) en `VITE_PLATFORM_DEFAULT_LANGUAGE` (dashboard, bake-at-build). Ongeldige/lege waarden vallen terug op `nl`. Persistente keuze: `localStorage` key `bokito-language` plus persoonlijke `GET/PATCH /api/me/preferences` `ui_language`. Onboarding (`/signup`, `/login`, `/accept-invite`, wachtwoord/verify) volgt `?lang=nl` of `?lang=en`; ontbrekende of lege parameter gebruikt de platformstandaard.
