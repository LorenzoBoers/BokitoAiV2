...
- **Unified Agenda (bokito):** FastAPI `/api/agenda/*` with `agenda_calendars` and `agenda_events` (kinds: `user`, `orchestrator`, `implementation` projected from `orchestra_tasks`, `external`). System calendars per tenant: **My agenda** and **Orchestrator**. `GET /api/agenda/events` commits system calendars on first load (same as calendar list). Dashboard shares calendar state via `AgendaCalendarProvider` (sidebar + page). Orchestrator wakes are recurring agenda events with `prompt`, `enabled`, `next_run_at`; in-process scheduler (`AGENDA_SCHEDULER_ENABLED`, default on) runs due wakes via `AgentLoop` every 60s. External Google/Outlook: mock OAuth connect stub + `POST .../connect/{provider}/complete`. Dashboard: `/agenda/:view` (`month|week|day|list`), `?date=YYYY-MM-DD`, calendars sidebar (visibility in `localStorage`), `AGENDA_API_BASE=/api`. Seed includes standup + daily orchestrator wake.
...
- **Intelligence Stack (2026-06 restructure):** Product backbone mapped to Salim Ismail layers — visible in OS Graph pipeline UX (hybrid IA), not as separate rail items per layer.
  ...
  - **GOVERN & ASSURE:** `AuditEvent` log (`GET /api/govern/audit`), agent passports (`GET /api/govern/passports` with `permission_scopes_json` + role defaults), `PlatformChange` draft queue (`GET /api/govern/changes`, accept/reject, `POST .../rollback`), tenant apply modes (`GET/PUT /api/govern/apply-modes`: `draft|yolo|decision` per resource type), **Autonomy Posture** presets (`GET/PUT /api/govern/posture`: `manual|assisted|autonomous` — dials `ActionPolicy.mode` + default apply modes; default `assisted`). Dashboard **Govern** page at `/govern` (pending drafts, passports, audit tail, posture cards).
  - **Platform self-maintenance:** orchestrator/workstream agents propose structural edits via agent tools (`create_agent`, `update_agent`, `create_workstream`, `update_workstream`, `register_mcp_server`, `connect_integration`, `propose_integration`, `add_graph_node`, `connect_graph_nodes`, `write_blueprint`). Mutations route through `propose_platform_change()` unless yolo; scopes enforced via `platform_access.py` (`platform:agent:create`, `platform:agent:update`, `platform:graph:edit`, etc.). Update tools capture a `before` snapshot for diff/rollback, `propose_integration` always escalates to a human decision, and PO agents may only mutate workstreams in their own project (`agent_can_access_project`).
  ...
  - **Orchestration (2026-06):** Background agent tasks via `/api/orchestration/*`. Canonical entities: `RuntimeProfile` (model/tools preset per planner|executor|judge), `AgentTask` (durable work unit linked to internal `Signal` thread), `EvalCheckpoint` (rubric|tool_assert|llm_judge), `TaskArtifact` (step outputs), `AutomationTemplate` (global ops presets). `AgentRun` extended with `task_id`, `step_id`, `parent_run_id`, `segment_index`, `runtime_snapshot_json`, `checkpoint_json`. Workstream steps bind `agent_id` + optional `runtime_profile_id`; runner enqueues ARQ segment jobs (`run_agent_task_segment`) or runs inline when Redis unavailable / `BOKITO_MOCK_EXECUTION=true`. `POST /api/workforce/trigger-agent` and `POST /api/orchestra/tasks/{id}/run` invoke the dispatcher (no empty runs). Multi-agent handoff: each step runs a different agent/model; outputs pass via `handoff_template` + `context_json.step_outputs`. SSE: `GET /api/orchestration/runs/{id}/events/stream`. Bootstrap seeds planner/executor/judge profiles + demo **Ops Triage Pipeline** workstream per tenant.
...
- **Tests:** pytest (59+ tests, mock LLM); covers govern, platform changes, signals, learning, decisions, notification preferences, branding `appearance_json`, livechat SSE. CI: `api` (ruff + pytest), `dashboard` + `messenger` builds (`VITE_API_MODE=bokito`), `e2e` (Playwright: dashboard login/cockpit/inbox/integrations/database/orchestra/agenda + messenger login). Local: `npm run verify:bokito` and `npm run verify:bokito:e2e` (see `docs/AI-OS-DEV.md`).
- **Legacy removed (production track):** `apps/runtime`, `apps/mobile`, `packages/docker`, `xano-patches` — niet meer actief in de V1 FastAPI-lijn.
...
Tenant preset on Govern **Policy** tab: `manual` | `assisted` (default) | `autonomous`. Sets `ActionPolicy.mode` and `platform_apply_modes` via `GET/PUT /api/govern/posture`. **Autonomous** keeps `integration` at `decision` so external connects always need human approval.
...
- Relatieve paden per groep (bijv. `/workspaces`, `/inbox/threads`, `/agenda/events`, `/auth/refresh` op de auth-base) staan centraal in `apps/dashboard/src/api/routes/` (`auth.routes.ts`, `app.routes.ts`, `integrations.routes.ts`, `workforce.routes.ts`, `agenda.routes.ts`) en in `apps/dashboard/src/api/url.ts` voor query-strings. Featurecode en `lib/*-api.ts` importeren daaruit; `lib/xano.ts` blijft transport (fetch, headers, cookies).
...
De portal gebruikt een Featurebase-achtige 2-laagse shell. Zie ook `apps/dashboard/docs/NAVIGATION.md`.

- **Primary rail (admin, icon-only):** Home (`/home`) → **AI OS** (`/os`, Network-icoon; unified Projects + Workforce) → Inbox → Integraties → Data; **Instellingen** onderaan (`/settings/profile`). Bokito rail adds Orchestra and Agenda before AI OS.
- **AI OS sidebar:** Canvas (`/os`), Agent library (`/os/agents`), Decisions (`/os/communication`), Blueprint (`/os/docs`), plus platform agents. Workstreams sidebar block on `/project/:id/*` only. Canvas uses React Flow (`AiOsCanvas.tsx`); add nodes via toolbar palette; connect by drawing between handles. V1 is config-first (edges stored; runtime enforcement is a follow-up). Legacy `/projects`, `/workforce`, `/ai/agents` redirect to `/os` paths.
- **AI OS canvas interactions:** Click a node for `NodeDetailPanel` (connections list, drill-in links, remove from canvas). Orchestrator panel includes workspace-wide `DecisionsInline`. Draw connections: workstream→orchestrator (`routed_by`), workstream→repo (`uses_repo`), workstream→tool (`uses_tool`), orchestrator→blueprint (`reads_blueprint`). Multiple blueprints per tenant allowed (`BlueprintDoc.tenant_id` no longer unique).
...
- **Project hub (`/projects`):** context sidebar: **Overzicht**, **Communicatie** en **Blueprint** (enkele link naar `/projects/docs`); **Workstreams** (`ProjectHubBackgroundWorkersNav`) met compacte projectrijen: gekleurde dot + teller, status als gekleurde tekst onder de titel. Paginaboom staat in de content op `/projects/docs` (`PageTree` variant `minimal`), niet in de sidebar. Per-project canvas: `WorkerStatusStrip` boven `ProjectTabNav`; status afgeleid via `deriveWorkerStatus` uit berichten (`decision_request` = geblokkeerd, overige `awaiting_human` = aandacht), budget, orchestration, work logs en repo-index.
- **Centrale Blueprint:** `/projects/docs[/:slug]` is tenant-breed (workspace-only, label-first migratie bovenop compat-route). `WorkspaceDocNavContext` laadt `GET /workspace/doc`; bij een lege boom seed de client acht standaard hoofdstukken (`workspace-doc-scaffold.ts`). Bij eerste load (of na `COMPANY_HANDBOOK_VERSION`-bump) vult `ensureCompanyHandbookApplied` (`workspace-company-handbook.ts`, nu **v2** — uitgebreide high-level handbook) die acht scaffold-pagina's met Engelse platformdocumentatie; bestaande inhoud op die slugs wordt vervangen. Custom pagina's vallen buiten de handbook. Geen fallback naar per-project docs.
- **Workspace doc Xano-schema:** live tabellen `workspace_doc_pages` en `workspace_doc_blocks` mogen geen PostgreSQL-default `""` op enum/json/uuid-kolommen hebben (inserts geven anders `SQL 22P02`). Blocks sorteren op `position` alleen (`workspace_doc_blocks` heeft geen `created_at`). API-response gebruikt `workspace_doc`, niet `doc` (Xano coerceert `doc` naar integer). Op `workspace_doc_blocks` nooit `return = {type: "list"}` **zonder paging** in `db.query` — kolom `type` botst met de return-key (`HTTP 400 Text filter requires ...`); gebruik `return = {type: "list", paging: {page: 1, per_page: N}}`. `POST /workspace/doc/pages/{id}/blocks` enqueued na batch RAG reindex via worker `POST /workspace/doc/reindex-page` (zelfde patroon als project doc batch). `BlockEditor`, `RevisionPanel` en sidebar `PageTree` gebruiken `docScope: 'workspace'` met CRUD via `/workspace/doc/pages` en block batch via `/workspace/doc/pages/{id}/blocks`. Hoofdstukken zijn top-level `workspace_doc_pages` met kinderen; secties zijn blocks (`heading_*`). Hub-knop **Vraag agent** plaatst `POST /workspace/doc/change-requests` voor de actieve pagina.
...
- **Inbox-zijbalk:** Messages hub folders **External** / **Internal (Agents)** via `?folder=`; queues via `InboxSidebarNav`; footer **Configureren** linkt naar Assistent-instellingen en `/settings/inbox`. Decision cards inline in thread timeline (`DecisionRequestMessage`); resolve via Signal API.
...
- **Workstreams wireframe (mei 2026):** `/project/:id/overview?stream=` is de workstream-cockpit: context bar toont **workstreamnaam + stream-status + worker-status** (niet projectnaam/repo). Horizontale tabnav en worker-status strip zijn op deze route verborgen; streamselectie gebeurt in de hub-sidebar. Standaard opent de pagina op het orchestration-canvas (Input → stappen → Output). **Request a change** hoort op **Blueprint** (`/projects/docs`, knop per pagina), niet op workstream- of project-shell context bars; legacy `/project/:id/request` redirect naar Blueprint.
...
- **Agent types in Workforce-list:** De AI Agents-lijst groepeert agents op type met `PO` bovenaan en `Worker` daaronder. Type wordt als label getoond per agent. PO-detectie accepteert zowel `role_slug=po` als `role_slug=manager` (Bokito PO/orchestrator).
...
- **Unified Messages hub (bokito):** Rail **Messages** (`/messages`) opent internal folder standaard. Sidebar mappen: External (email queues + mailboxes) en Internal (Agents: all, awaiting decision, updates, results, mine, unassigned, pinned). Query `?folder=external|internal` en optioneel `?project_id=` voor project-scoped internal threads. Legacy `/os/communication` en `/project/:id/communication` redirecten naar Messages hub. Agent decisions zijn `SignalMessage` met `kind=decision_request` in dezelfde thread UI (geen aparte Decisions-tab meer).
