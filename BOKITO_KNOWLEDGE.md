...
- **Orchestra foundation:** AgentProfile, Workstream(+steps/runs), mock execution environment, `/api/orchestra/*`.
...
#### Communication-rail met aanpasbare secties (juni 2026, actueel)
De binnenrail van de Communication-hub is herbouwd naar een Intercom-achtig sectiemodel (`MessagesHubNav` + `SidebarPrefsContext` + `SidebarCustomizeDialog`):
...
- **Aanpasbare secties** (volgorde, verbergen, inklappen; per browser gepersist in localStorage key `communication-sidebar-prefs`): **Assistant** (`/communication/assistant`), **Views** (presets Support/Sales/Operations/Partner communication/Agent handoffs op `/communication/view/:key`; gefilterd op tag), **Channels** (e-mail per mailbox-connectie `/communication/channel/email/:connectionId`, Webchat→`widget`, Internal chat→`internal`, Agent messages; WhatsApp/Slack alleen bij bestaand ChannelAccount via `GET /api/channels/accounts`), **Agents** (één rij per toegankelijke company agent `/communication/agent/:agentId` + Agent runs `/communication/runs/:queue` met all/updates/results/awaiting-decision), **Labels** (presets Urgent/VIP/Follow-up/Billing op `/communication/label/:key`; tag-filter) en **Settings** (link-outs naar `/settings/channels` en `/settings/assistant`).
...
- **Settings-secties:** Personal (profile, notifications, access-security), Workspace (general, branding, members), Channels (channels = email, assistant widget = `/ai/assistent/*`, communication agent, help-centers), Integrations (connected, marketplace, mcp), AI (`/settings/llm-keys` = AI keys, owner/admin), Autonomy (`/settings/autonomy` = voormalige GovernPage).
- **Per-tenant LLM keys:** owners/admins kunnen op `/settings/llm-keys` eigen Anthropic (chat) en OpenAI (embeddings) API-keys opslaan. Keys worden encrypted at rest (`TenantSecret` tabel `tenant_secrets`, Fernet via `app.services.crypto`, key afgeleid van `JWT_SECRET`); alleen `last4` + effectieve modus (live/mock) komen terug naar de client. Een tenant met een geldige Anthropic-key draait automatisch live, los van de globale `LLM_MODE`. Resolutie via `resolve_tenant_llm_config(session, tenant_id)` (tenant-secret → env-fallback bij globaal live → mock); doorgeprikt naar `AgentLoop`, workspace-embeddings, interpretation, eval en thread-compaction. API: `GET /api/settings/llm-keys`, `PUT/DELETE /api/settings/llm-keys/{anthropic|openai}`.
- **Model-catalog + token resale:** platform-brede modellen-catalogus (`model_catalog`, staff-managed) bevat Anthropic chat (Sonnet/Haiku/Opus 4), OpenAI chat (GPT-4o, GPT-4o-mini) en OpenAI embeddings (3-small/3-large), met prijs in cents per 1M tokens. Bokito zet eigen fallback-keys (`platform_secrets`, encrypted) zodat tenants zonder eigen key tóch live draaien; dat verbruik is **billable** (token resale met markup `token_markup_multiplier`, default 1.3, in `platform_settings`). Resolutie via `resolve_model_call(session, tenant_id, kind, model_slug)` → keuze-orde key: tenant BYOK (niet billable) → platform key → env-live → mock; modeldata/prijs uit catalog (fallback naar veilige defaults als catalog leeg). Provider-abstractie: `get_chat_provider(provider, api_key)` met `AnthropicLLMProvider` + `OpenAILLMProvider` (vertaalt Anthropic-stijl tools/messages naar OpenAI tool-calling). Metering: `UsageLedger` uitgebreid met `call_type`, `key_source`, `billable`, `provider_cost_micros`, `customer_cost_micros` (micro-USD), `agent_id`/`run_id`/`user_id`; `record_usage(...)` centraal in `AgentLoop.run_chat` + losse callers (triage/eval/compaction/embeddings). Project-tokenverbruik aggregeert nu via `run_id`→`AgentRun.project_id`. Tenant-defaults/allow-list in `tenants.settings_json['models']`. API: tenant `GET/PUT /api/settings/models`, `PATCH /api/workforce/agents/{id}/model` (owner/admin, gevalideerd tegen allowed-list), usage `GET /api/cockpit/usage`; staff `GET/POST/PATCH/DELETE /api/staff/models`, `GET/PUT/DELETE /api/staff/platform-keys/{provider}`, `PUT /api/staff/markup`. UI: Settings > AI > Models (defaults/allow-list + BYOK-vs-billable badges; staff-sectie voor catalog/keys/markup), Model & runtime-kaart op agent-detail en My Assistant, model-badge op agentlijst, Usage-pagina met breakdown per model/agent (BYOK = geen kosten, platform = billable).
...
- **Host:** Hostinger VPS `srv859418` (`31.97.45.44`), Ubuntu 24.04, 2 vCPU / 8 GB. Toegang via de hPanel browser-terminal (root). Docker Engine 29.x staat erop; de compose-plugin ontbrak en is als binary geïnstalleerd in `/usr/libexec/docker/cli-plugins/docker-compose`.
- **Publieke URL:** `https://bokito.chargecars.app` (DNS A-record → `31.97.45.44`). TLS wordt automatisch door de host-Caddy geregeld.
- **Repo op de server:** publiek te clonen via HTTPS; checkout in `/opt/bokito`. Redeploy = `git pull` + de compose-up hieronder.
- **Reverse-proxy topologie (twee Caddy-lagen, bewust):** de host-Caddy (`/etc/caddy/Caddyfile`, systemd) bedient zowel `worker.bokito.ai → localhost:3300` (bestaande dienst, niet aangeraakt) als het nieuwe blok `bokito.chargecars.app { reverse_proxy localhost:8088 }`. De Bokito-container-Caddy (`web`-service) draait HTTP-only met `BOKITO_DOMAIN=:80` en is gepubliceerd op `127.0.0.1:8088` via override `docker-compose.vps.yml` (`ports: !override - "127.0.0.1:8088:80"`), zodat hij niet vecht om 80/443.
- **Stack:** `docker compose --env-file .env.prod -f docker-compose.prod.yml -f docker-compose.vps.yml up -d --build` start `postgres` (pgvector), `redis`, `api` (uvicorn, 1 replica i.v.m. in-process trigger-scheduler), `worker` (arq) en `web` (Caddy + dashboard/widget static). Alle services hebben healthchecks + `restart: unless-stopped`.
- **Config:** `/opt/bokito/.env.prod` met `ENVIRONMENT=prod` (fail-fast checks actief, Secure refresh-cookie), sterke `JWT_SECRET` + `POSTGRES_PASSWORD` (gegenereerd met `openssl rand`), `CORS_ORIGINS=https://bokito.chargecars.app`.
