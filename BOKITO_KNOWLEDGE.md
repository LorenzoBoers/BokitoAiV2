...
- **Integratie-modules (2026-08)**: een **module** is een first-class workspace-schakelaar (schema, verbs, skill, setup), geen marketplace-categorie. Een **connector** is het pakket eronder (KING, Moneybird). Inbox/GitHub blijven kanalen/kinds, geen modules. Enablement staat in `Tenant.settings_json.modules` (`{ "accounting": { "enabled": true } }`). Expliciete flag wint; zonder flag blijft een live pakket de module aan (legacy). Nieuwe tenants starten uit. Uitzetten revoken geen koppelingen; een pakket koppelen zet de parent-module automatisch aan. Registry is `ModuleSpec` in `apps/api/app/modules/catalog.py` (accounting `available`; banking, investing, documents `coming_soon`). `GET /integrations/providers` levert `modules` met `enabled`, `connected` en `tenant_status` (`off` | `on` | `connected` | `coming_soon`). `PATCH /integrations/modules/{slug}` body `{ enabled }` zet de schakelaar. Switchboard: `/settings/modules` (`ModulesPage`); module-home: `/settings/modules/:slug`. Pakket-chips openen de connect-stap (`?connect=&step=setup`). Coming-soon pakketten (Exact, SnelStart) staan als geplande regel, niet als live kaart. Marketplace toont per sectie Off/On/Connected plus de live pakketten. Settings-nav heeft een eigen **Modules**-link (Integraties-match sluit `/settings/modules` uit). Setup Hub (Intelligence) linkt naar `/settings/modules`. Frontend-fallback: `FALLBACK_MODULES` in `integration-modules.ts`.
- **Agent ziet modules alleen als ze aan staan**: tenant-snapshot `Modules:` toont `off` / `on, no package` / `connected`. Coming-soon in één regel “prepared, not connectable”. `filter_tools_for_modules` verbergt `accounting_*` (en later `banking_*` / `investing_*` / `documents_*`) tot de module aan staat. Skill-playbook alleen bij on + geen pakket; volle skill bij on + connected. `accounting_*` geeft eerst `module_off`, daarna `no_connection`. Tools `list_modules` en `recommend_module` blijven ungated. `recommend_module` maakt `enable_module` als de module uit staat, anders `setup_integration`. Goedkeuren van `enable_module` persist in `notifications.py`; de UI navigeert naar de modulepagina. Heartbeat-seed + bestaande heartbeats: accounting **off** → `recommend_module`. `ONBOARDING_SYSTEM_PROMPT`: eerst aanzetten, daarna een pakket koppelen.
...
- **Modules** is a top-level rail item under the Settings group (`TAB_GROUPS`), path `/settings/modules`, Lucide `Boxes` icon. Active highlighting uses `tabFromPath` matching `/settings/modules` before the settings catch-all.
