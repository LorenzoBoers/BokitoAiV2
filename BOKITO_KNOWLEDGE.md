...
- **Intelligence Stack (2026-06 restructure):** Product backbone mapped to Salim Ismail layers — visible in OS Graph pipeline UX (hybrid IA), not as separate rail items per layer.
  ...
  - **Automated / no-reply mail handling (2026-08):** the AI never drafts a reply to automated mail. Detection has two layers in `services/automated_mail.py`: (1) deterministic heuristics — sender local-part patterns (`noreply`, `donotreply`, `notifications@`, `alerts@`, `mailer-daemon`, `postmaster`, `bounce`, `newsletter`, …) plus RFC auto-mail headers captured at sync time (`auto_headers` in message metadata: `Auto-Submitted`, `Precedence: bulk/list`, `X-Auto-Response-Suppress`, `List-Id`/`List-Unsubscribe`, null `Return-Path`); Gmail supplies these from `format=full` headers, Graph only when `internetMessageHeaders` happens to be present; (2) a prompt guardrail — suggest and auto prompts instruct the model to return `NO_REPLY_NEEDED: <summary>` for automated notifications it recognizes (parsed by `extract_no_reply_summary`). Both paths call `inbound_agent.create_action_suggestion`, which raises a "No reply needed" `DecisionRequest` with options **Close thread** (`action_type=close_thread` → new builtin `close_thread` tool: status→closed, `thread_updated` event, thread-update publish), **Create task** (`create_task` with subject-derived title), and **Keep open** (defer). The heuristic path short-circuits before the AgentLoop (no tokens spent, works without a routed agent) and deliberately sends no notification email. Dashboard `DecisionRequestMessage` renders this as a calm neutral card (BellOff icon, "No reply needed"). The third line is a **preview excerpt** of the mail (heuristic path used to hard-cut `body_preview` at 280 characters mid-word); the UI now collapses whitespace, cuts on a word boundary, and always ends with `...` when the text continues. New inbound cards use the same clipping in `clip_with_ellipsis`. Reply-suggestion cards gained an inline ghost "Close thread" button (`patchThread status=closed`).
