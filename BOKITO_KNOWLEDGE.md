...
- **Bron**: `docs/product-help/{en,nl}/{sectie}/{slug}.md` — 26 artikelen per taal in zes vaste secties: `getting-started` (welcome, quickstart, setup-guide, tour, cockpit), `inbox` (communication, channels, contacts, widget), `ai` (agents, decisions, knowledge, projects, agenda), `govern` (govern, autonomy, models), `integrations` (integrations, mcp), `developers` (api-overview, authentication, api-signals, webhooks, mcp-endpoint, widget-embed, rate-limits). Slugs zijn plat en globaal uniek; de map is de sectie. Frontmatter vereist `title/intro/description/keywords/sort` (+ optioneel `related`). Sync naar de verpakte kopie `apps/api/app/data/product_help/` via `python apps/api/scripts/dev/sync_product_help.py` (valideert frontmatter en en/nl-pariteit); `tests/test_product_help.py` faalt op drift. Cursor-rule `.cursor/rules/product-help-docs.mdc` verplicht docs-updates bij featurewijzigingen.
- **Publieke docs-site** `/docs`: landing met hero + zoek + sectiekaarten; artikelen op `/docs/{sectie}/{slug}` met sidebar-nav, ToC uit `##`-koppen (ankers via `lib/docs-toc.ts`, gedeeld met `MarkdownView`), vorige/volgende, "Copy as Markdown", EN/NL-switch (localStorage `bokito.docs.lang`), breadcrumb en SEO-meta (`lib/docs-seo.ts`). Platte `/docs/{slug}` redirect naar het sectiepad. `/docs/api` = Scalar API Reference (CDN-script) op het gefilterde schema.
- **API** (`routers/product_help.py`, rate-limited, unauthenticated): `GET /api/docs` (nav-boom `sections[]` + platte `articles[]`), `GET /api/docs/search?q=&lang=` (keyword-only, geen embeddings publiek), `GET /api/docs/{slug}` (artikel), `GET /api/docs/{slug}.md` (raw markdown met frontmatter), `GET /api/docs/sitemap.xml`, `GET /api/docs/openapi.json` (gefilterd publiek schema via `services/public_openapi.py`: tags public-api/mcp/help-center, alleen gerefereerde component-schema's, bearer-securityscheme). FastAPI-root serveert `/llms.txt` (index met absolute raw-md-URL's) en `/llms-full.txt` (alle en-artikelen geconcateneerd).
